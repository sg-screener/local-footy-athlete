# SEAT INBOX ARCHIVE — everything processed up to 2026-08-10

**Split out of `docs/SEAT_INBOX.md` on 2026-08-10 because that file had
reached 353KB and was being re-read at every terminal stop. Nothing is
...
## Processed 2026-08-10 (seventy-first pass) — MERGE SLICE 2 BUILT AND PHOTOGRAPHED; THE HOT FILES ARE BUDGETED; AND TEN WAYS A DIAGNOSTIC COULD KILL THE APP ARE GONE

Boundaries: `docs/UI_MERGE_SLICE2_BOUNDARY_2026-08-10.md` and
...
## Processed 2026-08-10 (seventy-fourth pass) — FIVE OF SEVEN ORDERS DONE; THE DRIFT CHECK EXISTED AND NOTHING RAN IT

Boundary: `docs/UI_MERGE_SLICE3_BOUNDARY_2026-08-10.md`, addenda 2-5.
ORDERS 1 AND 2 REMAIN OPEN — the knot is hedged not untangled, and
`injury-case` is diagnosed not fixed. The orders follow, verbatim.


**QUEUE TOP-UP. Sam is away. Work top to bottom, report only when it is
all done or you are genuinely blocked — a plan is not a reason to
stop.**

1. **THE KNOT — untangle it so the My status buttons work and the
   off-season box can leave the day screen.** You named it and you were
   right to hold both rather than ship a screen whose buttons lie.
   **Two rules while you do it:** nothing leaves the day screen until
   its replacement can actually do the job, and a control that looks
   live but is not is worse than no control — **so if the untangling
   runs long, make the dead buttons visibly not-yet rather than leaving
   them looking ready.**

2. **THE BROKEN PRACTICE WORLD you found — one setup cannot even
   load.** Fix or retire it, and say which. **A world that cannot load
   is the seed-rot class again**, and the drift check below is what
   stops the next one.

3. **THE 21 SUITES.** 21 of 332 were built on the impossible athlete.
   **Do they still pass on a legal profile?** Fix what that turns up.

4. **THE DRIFT CHECK** — Sam's third alarm. Does each test world still
   match what the generator produces for that profile today?

5. **THE SEEDED WORLD IS NOT DURABLE** — memory four game days, disk
   one. Price it; fix if small, write the price down if not.

6. **UNENFORCED LAWS — keep the count falling.** Priority:
   `LAW-instrumentation-alive`, and `a refusal that cannot be seen is
   not a refusal`.

7. **THE TWO LAYOUT BUGS YOU FOUND BY EYE ARE A FINDING, NOT A
   FOOTNOTE** — a button over the battery icon and a title swamping the
   screen, **neither visible to any assertion.** That is the honest
   limit of the flows: they prove presence, not layout. **Say so in the
   boundary and name what would catch that class** — if the answer is
   "only an eye", then the screenshots are load-bearing and the flows
   must photograph every state they touch. Registry row.

(none)

## Processed 2026-08-10 (seventy-fifth pass) — ORDER 2 FIXED: the witness was right and the ORDER was wrong

Boundary: `docs/UI_MERGE_SLICE3_BOUNDARY_2026-08-10.md` addendum 6.
ORDER 1 (the knot) remains open and is the only live order.
