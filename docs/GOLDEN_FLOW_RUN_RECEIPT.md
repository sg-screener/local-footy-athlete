...
| `mas-session-names-his-own-pace.yaml` | 2026-08-13 | **PARTIAL — the Away control drives GREEN end to end; the final read is OWED and the reason is device contention, not the flow.** Census C2's athlete-visible proof: `deriveMas` had zero production callers, so a card read `Intensity: 110% MAS` and never said 110% of what. **PASSED:** reset seed → week shape → scroll to `home-away-entry` → sheet → leave Tue → return 2026-07-27 → equipment "Yes, same as usual" → **the week returns with Tue/Thu stripped of "+ Team Training" and SATURDAY'S GAME REPLACED BY A CORE CONDITIONING DAY**, which is a receipt for R-075's conditioning half as well as this unit's (`artifacts/ui-walk/pace-away-week-applied.png`). **NOT REACHED:** open Wednesday's session, expand conditioning, assert `Your pace:`. The app dropped into the dev-harness cold-start gate (*"DevE2EClock reload mismatch"*) mid-run twice while the desktop seat was driving the same simulator for the scenarios golden — item 35 records the identical collision from the other side. **The next run needs nothing rewritten, only an uncontended simulator.** TWO HARNESS FACTS ARE WRITTEN INTO THE FILES so nobody re-finds them: a SECOND clear-state launch inside one run never comes ready, and `- back` on iOS is an edge swipe that BACKGROUNDS the app from a stack root rather than returning to the program screen — which is why this is two files and not two arms. |
