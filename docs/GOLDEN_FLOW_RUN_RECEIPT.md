...
| --- | --- | --- |
| `coach-snapshot-dashboard.yaml` | 2026-08-23 UTC (2026-08-24 AEST) | **PASS — redesigned base/empty state on iPhone 17 Pro, iOS 26.3.** Reset `standard-in-season-week`, opened the existing Coach tab, required the weekly hero and its progress track, the four separate status tiles (readiness, load, progress, restrictions), and the existing conversation beneath them, then captured `artifacts/ui-walk/coach-snapshot-dashboard.png`. The approved words remain unchanged. Visual inspection caught lime markers falsely making empty states look positive; they were removed and the final neutral layout passed again. NOT COVERED: non-empty readiness/load/progress/restrictions, a same-screen fact update, and Sam's physical phone. |
