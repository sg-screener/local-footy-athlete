...
| --- | --- | --- |
| `coach-snapshot-dashboard.yaml` | 2026-08-23 UTC (2026-08-24 AEST) | **PASS — load-first base/empty state on iPhone 17 Pro, iOS 26.3.** Reset `standard-in-season-week`, opened the existing Coach tab, required Load directly below the title with the signed shaded sweet-spot zone, required Consistency plus readiness, progress and restrictions in the four-tile grid, required the existing conversation, and captured `artifacts/ui-walk/coach-snapshot-dashboard.png`. Visual inspection confirms the zero-history athlete has the honest `Not enough recorded load yet` state and no marker pretending a current ratio exists. NOT COVERED: the non-empty moving load marker, non-empty readiness/progress/restrictions, a same-screen fact update, and Sam's physical phone. |
