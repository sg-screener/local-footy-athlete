...
> **AUTHORED FINAL — Sam sign-off 2026-07-25/27.**
> Source of truth for the conditioning build; changes require Sam.
> Paired framework: `CONDITIONING_FRAMEWORK_SAM_2026-07-25.md`.

`docs/CONDITIONING_TEMPLATES_FINAL_2026-07-25.xlsx`. Supersedes V2 and V3.
All twenty-one of Sam's rulings applied.
...
| `NO SKI / ROW (flywheel spin-up)` | the 4 machine-admitting Repeat Sprint rows |
...
## Sam's four closing confirmations (rulings 18–21)

**18 — Up-Back Shuttle stays; COD = 4 rows.** Confirms this pass's reading that
"keep ONLY" was scoped to the skim-list rows, so the one Bible-sourced
structured COD session was never in scope. No change to the sheet.

**19 — both 10-second rows stay.** 10 s Max Sprint Repeats (1:5) and 10 s Repeat
Efforts (1:3) are different recovery, different sessions. No merge.

**20 — MetCon stays dead as a conditioning name.** Both copies stay removed and
the name is retired from the conditioning vocabulary. Closes the old Q9.

**21 — Ski/Row excluded from all sub-10-second templates** (flywheel spin-up —
the flywheel cannot come up inside the effort, so the rep produces no real
work). **Air Bike, Bike and Run carry the short templates.** Encoded as the
`NO SKI / ROW (flywheel spin-up)` modality property on the four
machine-admitting Repeat Sprint rows, and as a standing rule in the framework
doc. This closes the old Q10 — **I read the two 10 s rows as inside the rule**,
since the flywheel rationale applies identically at exactly 10 s. It also adds
**Bike** as a named carrier on those rows, where V3 had only Air Bike and Run.
...
- **No open questions remain.** Q9 and Q10 — the last two, which carried no flag
  and so were invisible on the Skim List — are closed by rulings 20 and 21.
