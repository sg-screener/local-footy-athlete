...
## R-119 (written as R-112, then R-114) — COUNT IT
...
R-119 and in commit `ffaabdfb`. **It was written as `R-112`; seat `sessionui`
landed its own `R-112` and `R-113` on `main` while this branch was unmerged, so
the UNLANDED row moved to `R-114` and the landed ones were left alone. The
INTEGRATOR then moved it again to `R-119`, because seat `finish-injury`
independently claimed `R-114` and `R-115` as a matched pair on the same day.** The headline the report must not bury:
