...
The final rcsteps report supersedes older pending-verification prose. Candidate
0bcc3353 passed its release gate and three simulator flows and was built, NOT
installed. Keep installation held until P13 and the revised candidate are
verified. Neither phone may be wiped. The 2 km skip text-tap inconsistency is
OPEN-UNKNOWN QA debt; the successful direct tap did not fix it.

## Options and ownership

Compared (1) name-specific bans / screen fixes with (2) passing the existing
injury decision, including painful triggers, into the existing shared legality
owner and canonical compiler. Choose (2): no new writer, stored plan, or rehab
policy. Likewise use resolved equipment, typed projected components and the
existing scheduler/selector for later chunks, not post-compiler patches.
North star: decisions remain inputs; amended views remain derived.

## Evidence / item ledger

| Items | Current evidence and next owner action |
| --- | --- |
| P13 | Trigger facts now reach shared legality, withholding and the canonical injury stage. New independent movement-family oracle covers pressing/pulling/overhead pressing at eight severity boundaries plus exact exercise triggers. Accumulated male/female journeys combine seven logged weeks, week-8 deload, active readiness, shoulder pressing trigger, restart and Clear. 172/172 injury assertions green. Replacement sets retain the reduced source ceiling. Exact-candidate release and simulator still owed. |
| P11/P12 | rcsteps fixed Row→Bike corruption; retain its accumulated identity witness. |
| P17/P20 | Source-read power caller uses legacy equipment; resolve current answer consistently, preserve explicit unticks. |
| P22 | Prior read-only actual-onboarding probe: 4 Mobility athlete-days across 2 genders; projected recovery, snapshot/menu strength. Trace and repair shared component identity, then Add/Move/Remove/Undo/restart. |
| P02/P05/P10/P21 | Current saved weeks 3/4 still front-load conditioning; existing weekday-prefix receiver selection and global block-index selection identified. |
| P01/P07/P09/P14/P15/P18 | Conditioning review pending; source-read max-machine narrowing retained. Preserve existing sprint credit; distinguish recovery and speed. |
| P03 | Approved new direction removes no-game automatic gendered extras only; manual Add and independent Mobility preserved. |
| P04 | Existing R-116: hide power rest display, preserve domain rest. Verify both UI and year export. |
| P06 | Product choice unresolved: catalogue removal versus automatic fallback; degree of conventional Deadlift preference. No catalogue deletion authorised. |
| P08/P16 | Explicit advanced band-curl / missing-rack unilateral direction, tests and affected registry updates owed. |
| P19 | Landmine press already appears as strength; dedicated upper-power route needs authored-policy review, not a blind retag. |

## Verification strategy

New tests must fail on original defects and in-memory source mutants; exercise
actual onboarding and final rows, accumulated modifiers, accepted edits, Clear,
Undo and production restart. Keep existing identity/readiness/load/fixture
protection. Exact-candidate release and visible simulator proof follow source
freeze; original years remain immutable and corrected-kit years are separate.

## P13 checkpoint evidence

- Baseline `test:injury-recomposition`: 106/106 assertions. Expanded tests first
  failed on preserved painful presses, before the legality owner was changed.
- After: 172/172; injury fallback journey 173/173; session injury review 76/76.
- Trigger-disabled in-memory mutant: 29 failing assertions; restoration green.
  Dose-reset mutant: 2 failing assertions, one per athlete, showing 14 source
  sets expanding to 20 male / 16 female sets. Fixed runs do not expand them.
- Original eight-week driver rerun removed presses but exposed the dose issue;
  that earlier output is diagnostic, NOT the final revised-candidate report.
- Typecheck: zero errors in all three scopes. Canonical compiler baseline chain
  was green before adding the new Mobility/input tests; those additions are
  separate work in progress, not a P13 release receipt.
- Reviewed exactly three changed writer functions in the existing ownership
  census; their classifications and budgets are unchanged. No guard removed.
- P22 new Add journey first failed 38/70 assertions: Mobility was labelled
  Strength and two of six source Mobility rows were diverted out of the main
  list by strength finalisation. The shared component-identity repair now
