...
**Status:** Sam’s approved policy recorded; production behaviour remains unchanged.
...
Forty-five representative weeks were run through the real local production pipeline. Every week was accepted by the current exposure validator and reported no unresolved shortfall. That is **not** proof that every week follows the Bible: the validator checks the code-owned contract, does not generally enforce exposure maxima, and can accept a final week that exceeds a reduced target or is misclassified.
...
5. Recovery cards are currently counted as full-rest days, although approved policy defines them as active recovery. Practice-match code can also reduce pre-season conditioning to 1, below the approved minimum of 3.
...
| Term | Meaning in this report | Important current-code behaviour |
| --- | --- | --- |
| Required minimum | The floor the week must satisfy unless an authorised reduction changes it. | The validator rejects an unfulfilled required or selected target count. |
| Preferred range | A coaching preference, not a compulsory floor or cap. | Some builders use it only as guidance; it is not a generally enforced maximum. |
| Permitted maximum | The highest normally safe exposure count. | The Bible gives several maxima, but the exposure validator generally checks minima, not strength/conditioning/sprint maxima. It explicitly checks the permitted hard-day limit. |
| Planner-selected optional work | Work the planner may add if availability, goals and safety allow. | Once selected, `targetCount` promotes it into an enforced target. In early off-season this can turn optional work into 3 compulsory lifts and 5 compulsory conditioning exposures. |
| Team-training credit | One team session currently counts as one conditioning, running, sprint/COD, fatigue and hard-day anchor. | Credit is automatic; the engine does not confirm that genuine high-speed work occurred. A canonical team workout may also be misclassified as strength. |
| Game/practice-match credit | One game or marked practice match counts as one conditioning, running, sprint/COD, fatigue and hard-day anchor. | The visible-week resolver adds the anchor before final validation. |
| Authorised reduction | A typed reason that explicitly lowers a normal target. | A reduction can be valid in the contract but fail to constrain the canonical final week because the validator accepts overshoot. |
...
| Final effective ledger | Counts from the final visible canonical workouts, after recurring team/game anchors and session classification. | This is the evidence used below, not only the planner’s intended allocation. |
...
| --- | --- | --- |
| Contract construction | `src/rules/weeklyExposureContractBuilders.ts` | Phase/mode minimums, preferred ranges, targets, anchor credit, rest/hard-day policy and typed reductions. |
| Contract resolution/validation | `src/rules/weeklyExposureContract.ts` | Promotes selected planner work into targets, validates the final ledger and reports shortfalls. |
| Planner | `src/utils/coachingEngine.ts` | Day allocation, session tier, patterns, conditioning, speed and power intent. Team anchors are unioned into selected days. |
| Canonical generation | `src/services/api/generateProgram.ts` and `src/data/defaultProgram.ts` | Rebuilds planned intent into visible workouts and exercises. This is where intended and final classification can diverge. |
...
| Exposure classification | `src/rules/sessionClassificationAdapter.ts` and `src/rules/weeklyExposureCounts.ts` | Classifies the canonical workout and produces the effective ledger. |
...
The API path permits Week 1 to use edge-generated content before canonical validation, while Weeks 2–4 use the deterministic fallback builder. The fixed-date evidence below deliberately used `generateProgramLocally`, so all four weeks used the production deterministic canonical path and were comparable. Source inspection confirms both routes converge on canonical workouts and final exposure validation; no remote AI call was used as policy evidence.
...
| Cooked week | Strength frequency may fall, but current readiness wiring can still retain power and the normal anchor-driven hard load. | **Approved:** remove power, reduce app dose first, preserve 2 lifts only when safe, and emit `low_readiness` if frequency falls. |  |
...
| Constraint | Bible intent | Current production choice/evidence | Decision required | Sam decision | Sam notes |
| --- | --- | --- | --- | --- | --- |
...
Approved Bible Section 18 defaults to all four patterns in every healthy relevant week. A rolling two-week balance is only a fallback when spacing, availability, readiness or injury genuinely prevents weekly coverage. Production currently has split ownership:

- In-season contracts require no explicit patterns, although healthy generated weeks often cover all four.
- Off-season and pre-season contracts generally request all four patterns in healthy weeks.
- A single full-body or lower session can credit more than one pattern.
- Injury reductions may remove patterns in the contract, but canonical workouts can restore them.
- The validator checks required patterns when present, but does not catch an explicitly excluded pattern returning.
...
| Decision needed | Current production choice | Risk if not decided | Sam decision | Sam notes |
| --- | --- | --- | --- | --- |
...
| Phase/mode | Current full-rest rule | Current hard-day rule | Evidence concern | Sam decision | Sam notes |
| --- | --- | --- | --- | --- | --- |
...
| Scenario | Clear Bible rule | Current conflict | Policy owner |
| --- | --- | --- | --- |
...
The following read-only production tests were run after the scenario audit:

- `npm run test:week-rebuild`: **60 passed**.
- `src/__tests__/repeatWeekTests.ts` through the repository TypeScript test runner: **33 passed**.
- `npm run test:block-rollover`: **36 passed**.

| Operation | Observed source-of-truth behaviour | Policy drift/risk |
| --- | --- | --- |
| Initial local build | Builds a target contract, plans allocation, creates canonical workouts, resolves visible anchors, then validates the effective ledger. | Planner intent can diverge during canonical fallback/classification. |
...
- This report now records approved policy, but it remains policy-only and does not approve current production behaviour or prescribe an implementation fix.
...
- “No unresolved shortfall” means the current validator accepted its own effective contract. It does not erase a Bible conflict, an unimplemented maximum, a bad anchor assumption or a canonical misclassification.
