...
New to training
ONE EXERCISE CAP FOR EVERY TRAINING AGE (Sam, 2026-07-27). Beginners get the SAME per-session exercise cap as everyone else. The app had been enforcing a beginner-only cap of 3 exercises per strength session, which Sam never authored — "do not overload them with too many exercises" is guidance about dose and complexity, not a hard numeric limit, and a separate beginner cap is exactly the "second beginner/experienced representation" this section forbids. Training age changes the DOSE, the COMPLEXITY and the PROGRESSION SPEED — not the size of the session.
Power rows never count against that cap, at any training age, consistent with power's exclusion from every other count (the counting fence: not a hard exposure, not main strength, no conditioning credit, not a finisher).
