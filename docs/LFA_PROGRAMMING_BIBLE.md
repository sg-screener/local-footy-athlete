...
* No beginner-only optional-session cap or hard-exposure cap. The universal weekly caps in Section 17 B govern every training age.
* The beginner CORE-SESSION cap is a known open question, not abolished. Section 18's authoritative phase table authors "Beginners in mid off-season use 2 strength, 2-3 conditioning and 1 safe sprint exposure with reduced complexity and dose" — a real beginner structural rule. The code applies that MID-OFF-SEASON rule in every phase, which is the actual defect. Held at the authored value until Sam rules on whether the phase-scoped rule stands and gets a phase-scoped home, or whether Section 18 is amended.
...
* Section 11, THE BEGINNER FILE (Sam, 2026-07-27). A sweep of `NEW_ATHLETE_POLICY` found twelve beginner-only limits, of which only two traced to authored text. Sam ruled on every one. SURVIVING and now authored: the 2-3 set / 4-8 rep dose, RPE 6-7 (he blessed the number, so "not close to failure" now has one), a 50% starting-load multiplier (corrected from the code's invented 0.75; load stays athlete-owned per D1), the simple-movement list, regressions, and the power pool's training-age gates. ABOLISHED as never-authored or contradictory: the beginner exercise cap of 3 (see below), the optional-session cap of 1, and the flat 3/3/3 hard-exposure cap — the universal Section 17 B caps govern every training age. NOT abolished, and corrected from the sweep that informed Sam's ruling: the beginner core-session cap of 2 IS authored, by Section 18's phase table ("Beginners in mid off-season use 2 strength..."). The sweep reported no authored beginner core number and was wrong. The genuine defect is that the code applies that mid-off-season rule in every phase; the value is held unchanged pending Sam's ruling. Also abolished: `avoidCombinedStrengthConditioning`, which steered beginners away from combined sessions the Bible PREFERS. CORRECTED: the code capped beginners at 2 sets while this section says 2-3, so it forbade a set Sam authored. Nothing beginner-specific survives outside the list above.
