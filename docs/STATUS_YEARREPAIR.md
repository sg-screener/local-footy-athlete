# STATUS — seat `yearrepair`

Owner: `yearrepair`. Opened 2026-09-01 at
`49eb25469e92656590862bc49ab71a8359569182` for the seven ordered full-year
programming and audit repairs supplied by Sam.

Sam then supplied an eight-part external-review addendum. It is part of this
same task. Every addendum finding must be reproduced on the current checkpoint
before production changes; already-correct behaviour is recorded and left
alone. Its ordered subjects are genuine multiple fixtures, shared robustness,
whole-week deload, running-speed development, Nordic dose, equipment increments,
compact club-night gym work and audit accuracy. Finals and minutes × RPE policy
are explicitly outside this batch.

Sam then supplied a further in-season refinement. Required male and female
weeks share the same football-performance foundation; automatic direct arm and
delt pump work is not required-session filler. Genuine missing calf/soleus,
adductor/groin, hamstring and knee/unilateral exposures may replace redundant
arm-isolation rows without lengthening the session. The male Friday option is
Gunshow and the female Friday option is Primer. Gunshow owns the male direct
arm/delt pump dose and remains low-soreness, away from failure and slow
eccentrics, and responsive to phase, fixture proximity, fatigue, illness and
injury. Final-week, restart and Undo guards are required.

## Boundary and order

- Work sections 1–7 in order and save one path-scoped, stamped checkpoint after
  each completed section.
- Then work addendum sections 1–8 in their supplied order, with the same
  reproduce-first and checkpoint discipline. Where an addendum sharpens an
  original section, the final verification must satisfy both.
- Preserve all unrelated tracked and untracked work. Do not reset the shared
  checkout, wipe either phone, install to a phone or build to a phone.
- Athlete-facing acceptance remains gates-green awaiting a clean Release build
  and Sam's physical-device acceptance.

## Design comparison before section 1

1. Patch every generator, validator and report with its own speed-name check.
   This is smaller at each site but creates several semantic counters and makes
   warm-up/primer and duplicate-row drift inevitable.
2. Keep the existing typed R-261 conditioning-credit owner, project its result
   into the full-year evidence at compiler time, and require audits/reports to
   consume and validate that evidence. This preserves Speed presentation while
   giving generation, validation, restart and reporting one semantic answer.

Choose option 2. Display rows remain presentation; they do not become the
source of exposure credit.

## Section 1 — shared energy-system counting

Implementation and focused verification complete. The typed full-year projection consumes
`classifyVisibleSession`, the same owner read by weekly validation. The focused
R-261 cells were green across seven generated phase/availability coordinates:
49 proper-Speed evidence cells, seven ignored-Speed mutations, seven
warm-up-rider exclusions and seven combined-session single-credit cells. The
full canonical suite then completed at 10,584 passing cells and 18 failing
cells. The denominator is 10,602 cells in that suite, not 18 broken athlete
weeks. The 18 are current-checkpoint defects outside the new evidence
projection: four Pre-season restart-selection mismatches, eight flush-template
rotation/restart failures, two full-kit Primer-option failures, and four
conditioning-diversity failures. They are retained as findings because the
final task requires zero restart mismatch and correct rotation.

The smaller shared-classifier suite reached 46 passing cells and two failing
cells. Both reds pre-date this section and concern whether a workout titled
`Light Team Training` downshifts its stress; the new annual evidence cells all
pass, including the unchanged Team Training anchor, ignored-Speed mutation,
double-credit mutation, Primer exclusion, warm-up exclusion and missing-
evidence refusal. TypeScript compilation, all four changed-script syntax checks
and whitespace checks pass. `test:law-registry` confirms this row is well-
formed and its named guards are in `test:bible`; the suite remains red on the
repo's measured 21 previously `UNENFORCED` process-law rows.

The section checkpoint contains no scheduling or exercise-selection change.
The exact 52-week evidence consumer will run again after the section-2 density
