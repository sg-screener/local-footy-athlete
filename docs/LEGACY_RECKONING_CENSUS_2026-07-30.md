...
| **LR-27** | **One body-part vocabulary, every door.** *Sam QUEUED this as its own unit, 2026-07-30, as option 2 of the injury "Other" ruling — "the convergent fix, 'shin' must mean the same thing at every door."* THREE phrase maps answer one question with different coverage: `guidedInjuryControl.guidedInjuryBucketForArea` (11 patterns), `sessionBuilder.INJURY_BODY_AREA_MAP` (~30 keys, knows `shin`/`glute`/`achilles`), and `programAdjustmentEngine.BODY_PART_TO_BUCKET` (whose own comment says it "mirrors" a fourth in `injuryAdjustmentEngine`). Same word, same athlete, different answers depending on which door they used. | `docs/INJURY_OTHER_PATH_TRACE_2026-07-30.md`; "shin" resolves at onboarding and not in the guided sheet | L-A1, L-B3 | **M** |
