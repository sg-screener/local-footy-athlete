...
## R-393 — Weekly leg coverage, whole-day demand and simple running return (Sam, 2026-09-08)

Owner: legprogram. Sam approved targeted programming repairs, in an isolated branch based on his current code including uncommitted work. These are minimums, not frequency caps: at least one calf-strength prescription and at least one Nordic or hamstring-curl prescription each week throughout the year. Existing meaningful work is credited before adding anything. An RDL cannot satisfy the normal Nordic/curl requirement; a suitable hip-dominant or isometric hamstring alternative counts only when equipment prevents all Nordic/curl choices. Tib raises and light mobility are not calf-strength substitutes. Health/fatigue restrictions and intentional exclusions outrank coverage repairs; repairs use existing automatic strength days and never make optional sessions compulsory.

The planner assesses actual whole-day leg work: strength, running, jumping, hard conditioning, demanding prehab, club and games. It prefers legal arrangements avoiding three/four consecutive demanding leg days while preserving useful training and protected commitments. Ten lower working sets remains the existing soft planning threshold, never a prohibition. Speed may share upper or lower-volume full-body work; no decision assumes sprints first or hours between sessions. Hard off-feet conditioning still contributes leg demand; existing in-season off-feet selection is preserved.

Equipment availability is assessed among choices permitted by the existing experience rules. An experience-blocked unassisted Nordic does not stand in for an unavailable curl machine; suitable hamstring work remains when equipment prevents the eligible choices. This does not relax Nordic experience or injury restrictions. Accepted-week coverage also respects the specific category deliberately removed by the athlete. Moves, unrelated deletions, restored decisions and deletions in another week do not waive the minimum.

After the four-week running layoff, running week one replaces existing field prescriptions with half the normal acceleration repetitions as controlled run-throughs. Week two uses three-quarters of the normal repetitions as faster straight efforts and controlled direction changes. Week three builds toward normal sprint/cutting when tolerated; strength stays. The same two-stage return follows a resolved sprint-blocking injury lasting at least seven days. Returning automatic jump rows reduce repetitions. The shared prescription and jump rep ceiling prevent repeated reduction on save/reopen. The workout's derived return marker describes introductory field exposure, not full-speed performance. The existing field-session slot remains occupied; no extra session is added.

Injury restrictions use dated accepted facts in all sections, including warm-ups, optional work, prehab, primers and saved replacement choices. Clearing the episode reopens legal options. Meaningful prehab contributes to the loaded area's workload regardless of heading. Where that area already has strength work, automatic demanding prehab becomes one light set; on deload its authored sets are halved, rounded down to at least one. Light mobility retains its dose and earns no strength credit. Explicit athlete-authored doses remain intentional edits under R-388.

This refines R-389's section-based exclusions: power contributes jump demand, while meaningful prehab contributes working sets even under a Mobility heading. It supersedes blanket statements that every optional flow is physiologically load-free. The normal Nordic/curl minimum also supersedes broad robustness wording listing hinges as equivalent weekly substitutes. Guard: `LAW-weekly-leg-coverage-and-return` / `test:leg-programming`, plus the existing Saturday Speed, whole-week and 52-week compiler audit checks. No merge, deployment or physical-device acceptance is authorised by this implementation approval.

R-393 verification clarification (legprogram, 2026-09-09): the return prescription is shared by generation and both display paths; the day summary must show the reduced repetitions and controlled work/recovery. Conditioning options keep their authored template identity, so existing run-only modality restrictions still apply. A text/modality rewrite cannot invent machine run-throughs. These are held by the same `test:leg-programming` guard, including both return stages, both field-section types, registered-copy checks and injected display/identity/modality faults.

R-393 repair note (yearfix, 2026-09-09): three defects the sports-science review of the three-day 52-week book found were repaired at their owners, not by date. (1) The Primer's authored Pogo Hops row was untyped, so the knee restriction that strips every other lower jump (`role === 'power'` in the typed-component exposure filter) walked past it on 2027-06-25 and 2027-07-02; the authored jump now carries the same power role, family and evidence as a pool-drawn jump, and no second reading of the constraint exists. (2) One adductor isometric per session: the groin pool's duration-authored holds (Copenhagen variants, Groin Squeeze) are one `adductor_isometric` variation family, so the composer, the automatic selector, the derived warm-up flow and the add/swap doors all refuse a second hold on one day through the existing same-session family rule; six dates paired a warm-up Groin Squeeze with a Half Copenhagen, and the eight-athlete year showed composed pairs too. Dynamic adductor work (Cossack Squat, Lateral Lunge) is not a hold and is untouched. (3) The frontal-plane completion now consumes the dated deload dose exactly as the weekly leg coverage does, and the demanding-prehab reducer reads that same dated policy rather than only a useful-strength contract that a Mixed day does not carry (2027-03-05 kept a 3 x 30 s Copenhagen on a deload Friday). Guard: `test:leg-programming` (cases and injected faults `untype-primer-pogo`, `adductor-holds-not-one-family`, `drop-frontal-completion-dose`, `ignore-dated-policy-for-prehab-reduction`). Receipts: `docs/STATUS_YEARFIX.md`.

## R-394 — In-season Nordics every week, or at minimum every second week (Sam, 2026-09-09)

Owner: yearfix. Sam, on the three-day book review: *"in-season, a couple of sets of Nordics every week, or at minimum every second week. Hamstring curls are fine but do not replace Nordics."* Measured before: Nordics appeared in-season only in weeks 27-29, 39-40, 42 and 51-52; the other in-season weeks carried a Hamstring Curl and satisfied R-393's Nordic-or-curl pair.

In-season the weekly leg coverage answers a Nordic category first, before the ordinary hamstring pair, so the Nordic it places also satisfies R-393. A Nordic is identified by R-312's typed `nordicCurlVariant`, never by the word in a name (Reverse Nordic Curl is a quad exercise). An existing Nordic is credited; a curl is kept but does not stand in. The dose is the automatic R-312 band (two sets at the low end), halved by the dated deload/readiness/illness policy like every other coverage row, which is how "at minimum every second week" is met in a reduced week. R-312's G-2-through-game exclusion, experience gating, dated injury restrictions, equipment and deliberate athlete removals outrank the minimum, as they do for R-393; where no legal day exists the receipt says so. Pre-season and off-season keep R-393's pair unchanged.

Held for every future athlete by the annual verdict: `weekly_inseason_nordic` is a required weekly check in `test:compiler-year` (a Nordic of at least two sets this week, or the previous in-season week, or a legitimately reduced or unavailable week; a week the checker could repair is red). Guard: `LAW-inseason-nordic-minimum` / `test:leg-programming` (injected fault `nordic-satisfied-by-curl`) and `test:compiler-year` (verdict controls `missing_inseason_nordic_check`, `failed_inseason_nordic_check`).

## R-395 — In-season, one Monday/Wednesday session may be harder than a flush (Sam, 2026-09-09)

Owner: yearfix. Sam, on the three-day book review: *"in-season conditioning does not need a big aerobic dose, but one of the Monday or Wednesday sessions may be harder than a flush, on the day the athlete is off club training."* Measured before: every in-season conditioning slot was a flush (4 x 2 min, 5 x 60 s easy, 10 x 30 s), because two club nights plus the game spend the whole game-week count and the G+2 flush (R-265) was the only app conditioning left.

In an in-season game week the scheduler now gives ONE strength-owned gym day off club training a moderate off-feet finisher (category tempo; in-season combined conditioning is off-feet, so the day needs a machine). The day sits outside G-2, G-1, G+1 and the G+2 flush day, so for a Saturday fixture it is Wednesday and for a Sunday fixture also Wednesday; an athlete whose gym days are all club nights or inside the fixture window gets nothing. A week that already carries an app metabolic or Speed component (a one-night club, the no-club fast session of WC-143) gets nothing more. A reduced week (deload, low readiness) authors nothing harder. The in-season game-week contract's permitted conditioning maximum grows by exactly this one session for an athlete with two or more club nights; the required count, the anchors and R-265's flush do not move.

Held for every future athlete by the annual verdict: `weekly_inseason_conditioning` is a required weekly check in `test:compiler-year` (an eligible in-season game week must show a non-flush core conditioning or Speed component on a non-club day). Guard: `LAW-inseason-harder-than-flush` / `test:leg-programming` (injected fault `drop-inseason-moderate-conditioning`) and `test:compiler-year` (verdict controls `missing_inseason_conditioning_check`, `failed_inseason_conditioning_check`).
...
## R-399 — The coach's words are shown as they arrive, and withdrawn if the check fails (Sam, 2026-09-10)

Owner: phonebuild. Sam, on a 10–15 s wait per answer: *"just update it to show the words as they arrive - that will help a lot"*; then, told that this means showing words before the whole-answer check has finished, and offered (1) real streaming with withdrawal or (2) the same wait then a typed-out answer: *"1"*. Amends R-138/F14's "nothing is shown until checked" to: the server streams the message as the model writes it, one JSON line per update; every update forwarded has already passed the word gates (readiness tier, fixture weekday, season phase, quoted door, false change) on the text so far, on the server and again on the phone; the receipt gates (basis, citations, fields, judgement label, length, read-only) run once the whole answer exists, and a failure ends the stream with the same typed refusal as before — the phone withdraws the provisional bubble and shows the signed refusal sentence. A pre-model failure (rate limit, contract version, bad input) is still a plain JSON error with its status. The model's JSON puts `message` first so the words exist before the receipt. Contract version 3. · `WORKING` — `test:coach-chat-streaming` (extractor, line parser, forwarding stops on a tripped gate, a refused final withdraws), `coachChatIntegrationTests` (server streams, gates per word).

---

## R-400 — Every coach failure sentence ends with the feedback ask (Sam, 2026-09-10)

Owner: phonebuild. Sam: *"i think it should have something like tacked onto the end of those - like 'please send a screenshot in the "leave feedback" section of the profile tab. We will fix the issue ASAP!'"*. The three R-140 sentences ("Coach isn't available right now. Try again shortly." / "I can't answer that safely." / "I don't have an answer for that yet.") stay word for word and each now ends with: **"Please send a screenshot through Leave Feedback on the Profile tab and we'll fix it ASAP."** — the button's own label (Profile → Leave Feedback opens an email a screenshot can be attached to). One constant, `COACH_FAILURE_FEEDBACK_ASK` in `coachTabCopy.ts`. · `WORKING` — `coachChatIntegrationTests` (the three sentences pinned with the ask).

---

## R-401 — The no-standing-fixture test profile is removed from every fleet (Sam, 2026-09-10)

Owner: phonebuild. Sam, after the release gate spent an hour on one 52-week replay: *"remove this … profile from all future tests - it's fucking stupid and you're wasting me time"*. `male-6-no-standing-fixture` (six gym days, no club nights, no standing game) is out of every YEAR REPLAY and long journey: `replay: false` in `src/__tests__/compilerYear/catalog.ts`, the replay drivers read `REPLAY_ARCHETYPES` (seven athletes), it is gone from the injury-energy journey worlds, and the compiler slice's "accumulated phase restart" witness, which only that profile drove, is retired. It STAYS in the catalog as a second-long fixture: two dozen quick tests reach it by position and deleting it moved every one onto another athlete (the Add suite went red for exactly that on 2026-09-10). Not returned to any fleet without a new ruling. Coverage lost, stated: the pre-season → in-season phase change with no club training over seventeen logged weeks. The slowness itself (a six-gym-day athlete makes the whole-week planner enumerate far more arrangements) stays on the after-the-build list as a planner performance item. · `BUILT` — the catalog has seven entries.
...
## R-403 — Consecutive conditioning days are allowed; the same modality three days running is not, and focuses alternate (Sam, 2026-09-10)

Owner: phonebuild. Sam, on week 16 of the three-day book (legs Wednesday to Saturday with Tuesday empty): *"I don't think the 3 conditioning day rule is valid... you should be able to do it but they should not all be the same type (unless injury or equipment issues) i.e. not 3 runs in a row, and not 3 bikes in a row. they should also aim to be different focuses i.e. hard conditioning on bike, light aerobic work, sprints on track etc"*. Supersedes the "does not author three app-programmed energy-system days consecutively" sentence of R-303 (the four-day density and the credit rules of R-303 stand). New law: (1) three app conditioning days may run consecutively; (2) not three of the same modality in a row — no three runs, no three bikes — unless an injury restriction or the athlete's equipment leaves no other modality; (3) consecutive conditioning days aim at different focuses (hard bike, easy aerobic, track speed), never hard-hard-hard; (4) the leg-loading run, gym legs and running counted together, stays at three days or fewer — the guard the old sentence protected by accident. Agreed shape for week 16: legs Monday, cuts Tuesday, legs and bike Wednesday, off Thursday, legs Friday, sprint Saturday. · `WRITTEN` — built after the 2026-09-10 merge, proven on the three-day book.

---

## R-404 — One hamstring-isolation exercise per week; the Nordic is it (Sam, 2026-09-10)

Owner: phonebuild. Sam: *"now we have 17 weeks where nordics and hammy curls appear in the same week, adding nordics in more was supposed to reduce the amount of hammy curls not just double up on them"*. Measured on the three-day book: 34 weeks carry a Nordic, 33 a hamstring curl, 17 carry both — every one in-season weeks 30–50, where R-394 made the Nordic weekly. R-394 wrote "a curl does not replace the Nordic"; this row writes the other half: when a Nordic is programmed in a week, no hamstring curl (machine, Swiss ball, band) is programmed that week. The curl fills the hamstring-isolation slot only when the Nordic cannot be done — a knee restriction, no anchor or partner, or a deload dose that removes it. · `WRITTEN` — built after the 2026-09-10 merge; the proof is zero both-weeks on the three-day book.
