...
R-393 repair note (yearfix, 2026-09-09): three defects the sports-science review of the three-day 52-week book found were repaired at their owners, not by date. (1) The Primer's authored Pogo Hops row was untyped, so the knee restriction that strips every other lower jump (`role === 'power'` in the typed-component exposure filter) walked past it on 2027-06-25 and 2027-07-02; the authored jump now carries the same power role, family and evidence as a pool-drawn jump, and no second reading of the constraint exists. (2) One adductor isometric per session: the derived warm-up flow no longer draws an adductor hold (Copenhagen, Groin Squeeze: the groin pool's duration-authored entries) when the prescribed rows already carry one; six dates paired a warm-up Groin Squeeze with a Half Copenhagen. Dynamic adductor work (Cossack Squat, Lateral Lunge) is not a hold and is untouched. (3) The frontal-plane completion now consumes the dated deload dose exactly as the weekly leg coverage does, and the demanding-prehab reducer reads that same dated policy rather than only a useful-strength contract that a Mixed day does not carry (2027-03-05 kept a 3 x 30 s Copenhagen on a deload Friday). Guard: `test:leg-programming` (cases and injected faults `untype-primer-pogo`, `ignore-session-adductor-in-flow`, `drop-frontal-completion-dose`, `ignore-dated-policy-for-prehab-reduction`). Receipts: `docs/STATUS_YEARFIX.md`.
