...
## 2026-08-13 — A BODYWEIGHT-ONLY ATHLETE IS PRESCRIBED A BACK SQUAT, AND THE EQUIPMENT FILTER IS DOUBLY INERT

**Measured on a generated week — off-season, `equipment: ['Bodyweight Only']`:**

    "Lower Squat"  Bicep Curls · Tricep Pushdowns · Tib Raises · Pallof Press · Back Squat

**A barbell back squat and cable tricep pushdowns, to an athlete who told the app
he has nothing.** That is not a slot problem. **It is the equipment answer not
reaching generation at all.**

**TWO INDEPENDENT REASONS, EITHER OF WHICH ALONE MAKES THE FILTER INERT:**

**1. THERE IS NOTHING TO FILTER ON.** `postGenerationConstraintValidation.ts:522`
filters rows by `row.exercise?.equipmentRequired`. `domain.ts:1265` declares that
field REQUIRED, with the comment `// e.g., ['Barbell', 'Dumbbells']` — **and every
production assignment sets it to `[]`**: `workoutCanonicalisation.ts:449`,
`coachActions.ts:617` and `:760`, `applyAdjustmentEvents.ts:739`. An empty
requirement list passes `equipmentRequirementsAreAvailable` unconditionally, so
**the filter removes nothing, ever.**

**2. IT DOES NOT ASK THE PROFILE.** `:515` computes availability only
`if (hasEquipmentConstraint)` — a live temporary equipment FACT. **The athlete's
standing `Bodyweight Only` answer is not a constraint**, so on an ordinary week
the branch is skipped before the empty data even matters.

**THIS IS `pace`'s NAMED PATTERN, FOURTH CENSUS, ONE DISEASE:** *a mechanism that
exists, is typed, is tested, and has no reader on the path that matters.* Beside
`deriveMas` with zero callers, `runningFloorExemption` passed by neither call
site, and `sessionSlotCoverage` with no build-time consumer. **This one is the
most athlete-visible of the four.**

**AND IT SUBSUMES MY OWN EARLIER DIAGNOSIS.** I called the bodyweight leg day a
"selector failure" — it is not. The selector was never told the athlete has no
equipment, so it picked from the full registry and behaved correctly on false
input. **`hinge` and `single_leg_knee` are missing because a barbell Back Squat
took the squat slot and the arms took two more** — not because bodyweight options
were rejected.

**⚠ AND I NEARLY PUBLISHED THIS WRONG.** My first probe called
`equipmentRequirementsAreAvailable('Back Squat', ['bodyweight'])` and got `true`
for every exercise — I was one step from reporting "the gate passes barbell work
to a bodyweight athlete". **The function takes an ARRAY OF EQUIPMENT LABELS, not
an exercise name**, so a string iterates its characters and every one returns
`null` → `continue` → `true`. **The result was an artefact of my argument type.**
Caught by reading the signature before publishing. *A fixture is a claim too* —
and the true finding turned out to be adjacent, larger, and reached only by
finding the real callers.

**NOT FIXED: it needs the exercise registry to carry requirements** (authored
data), **and the profile answer routed to the filter** (code). Two owners, and
the data half is the same stocking job as `single_leg_hip`.


---

## 2026-08-13 — CORRECTION: THE EQUIPMENT DATA EXISTS. IT IS THE MAIN LIFTS THAT ARE OUTSIDE IT.

**I wrote *"there is nothing to filter on"*. That is half wrong and the true shape
is sharper. Measured:**

    src/data/exercisePools.ts    91 entries · 90 CARRY `equipment: EquipmentTag[]`
    src/data/exerciseTags.ts    149 entries · NO equipment field at all

**TWO DISJOINT EXERCISE UNIVERSES, AND THE EQUIPMENT KNOWLEDGE IS IN ONLY ONE.**
`PoolExercise` (`exercisePools.ts:85`) declares `equipment` and 90 of 91 entries
populate it — the model works and the authoring was done. **But every exercise on
the deficient leg day is outside it:**

    Back Squat · Tricep Pushdowns · Bicep Curls · Bodyweight Squat · Glute Bridge
      -> NOT IN ANY POOL

Those pools are the prehab/accessory sets (`UPPER_BACK_PUMP_POOL`,
`GROIN_ADDUCTORS_POOL`, `CALVES_POOL`…). **The MAIN LIFTS — the ones that actually
require a barbell — live in `exerciseTags.ts`, which has no equipment field.**

**SO THE DEFECT IS NOT MISSING DATA, IT IS A SPLIT VOCABULARY.** The app knows a
`Copenhagen Plank` needs nothing and does not know a `Back Squat` needs a barbell,
**because they are catalogued in different files with different shapes.** The
half that needs gating least is the half that carries the gate.

**THAT ALSO RE-SIZES THE FIX.** It is not "author requirements for 149
exercises" — it is **one field on `ExerciseTag`, populated for the lifts that
need equipment**, then carried onto the generated row so
`postGenerationConstraintValidation` has something to read. The pools prove the
model; `exerciseTags` needs the same column.

**⚠ SECOND CORRECTION IN TWO ENTRIES, AND BOTH WERE MINE.** First I nearly
reported the gate as open from a bad probe (wrong argument type). Then I reported
the data as absent when 90 entries carry it. **Both times the instrument was the
problem and both times reading the actual source fixed it in one step.** The
