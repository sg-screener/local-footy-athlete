...
- The Bible gained a **WHICH AUTHORITY WINS** paragraph in Section 8, naming the
  precedence and retiring the two contradictory readings by name (hip thrust for
  a 6-7 knee, supported pulling for a 6-7 shoulder).
- That new rule line is anchored to its enforcer
  (`bibleThresholdAnchors.injury_sheet_outranks_swap_examples`) — **and it had
  to be.** `test:bible-coverage` went GREEN → RED the moment the line landed
  unanchored, because a rule line with no named enforcer is debt. Anchored, the
  debt is back at its ceiling of 995 and held rule lines went 8 → 9.
- The three contradictory test cells now assert the ruling instead of the prose.

**`test:tap-swap-hierarchy`: 16 ok / 3 fail and DYING on `main` → 25 passed,
0 failed, running to completion.** Its last fixture omitted both capacity
answers, so `resolveTapSwapEnvironment` threw `MissingCapacityAnswerError`, the
...
the two doors that mean *"what the athlete SEES"*, so the withholding is a **pure
derivation** applied at the same seam `applyExclusionsToAuthoredDay` sits at — and
for the same reason written beside it: a projection that reaches a canonicaliser
gets written down and stops being a projection. **No new state was added.**

The two projections now sit side by side doing opposite things on purpose: an
exclusion FILTERS the row out, an injury MARKS it
(`WorkoutExercise.unavailableForInjury`).

Measured through the real doors, red-flag hamstring 9/10:

| | before the ruling | now |
| --- | --- | --- |
| Remove decisions created | **5** the athlete never made | **0** |
| rows on the day | `[]` | all five, at their own loads |
| stored accepted program | untouched | untouched |
| recording the day | refused as *"No visible session exists"* | refused, **naming the injury** |
| after clearing | `[]` — empty forever | the **exact** original session |
| after close/reopen | `[]` | the exact original session |
| the resolve said | *"safely recomposed"* over an empty day | *"Your usual exercises are available again"* |

The refusal lands in `sessionOutcomeRecordableRefusal`, which already had two
readers — the write door and `SessionFeedbackPanel`, which asks it before
offering "Save & Finish" — **so the UI lane needs no change to honour the
ruling.**

**Only a RED FLAG blocks.** An ordinary injury substitutes and the athlete
trains; an injury reported today withholds nothing on a day already past.

---

## ONE THING THE UI LANE STILL OWNS

`projectVisibleDay` **blanks the day outright** while a red-flag constraint is
active — measured identically on `main` `9f081efa`, so it predates this unit and
is not a regression. Sam's ruling permits blocking the session ("or block the
session if necessary"), so nothing here fights it. But *"show them as
unavailable/skip"* is only half delivered until that projection renders the
marked rows instead of emptying the day. The typed data is ready and needs no
deriving on the screen; `npm run seed:injury-fallback` prints the five-step glass
handoff verbatim.
...
## QUESTIONS FOR SAM — NONE OPEN

Both questions this seat raised were ruled on 2026-08-20 and are closed above.
The section below is kept as the record of what was asked and why.
