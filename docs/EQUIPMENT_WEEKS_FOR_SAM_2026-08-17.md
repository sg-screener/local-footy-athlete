# THE FIVE WEEKS, SIDE BY SIDE

Written by `npm run trace:equipment-scopes`. Every exercise below came
out of the real generator and the real screen projection.

## HOME — his normal week

```
2026-08-10 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Ab Wheel
2026-08-11 [kit: 19]  Training Day
  Team Training
2026-08-12 [kit: 19]  Training Day
  Strength
    - Leg Press
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Band Pallof Press
2026-08-13 [kit: 19]  Training Day
  Team Training
2026-08-14 [kit: 19]  Rest Day
2026-08-15 [kit: 19]  Game Day
  Game Day
2026-08-16 [kit: 19]  Rest Day
```

## AWAY, normal equipment — he took the trip, kept his gym

```
2026-08-10 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Ab Wheel
  Conditioning
    - Steady Blocks (3×8 min or 4×6 min)
2026-08-11 [kit: 19]  Training Day
  Strength
    - Barbell Row
    - Lat Pulldown
    - Band Pull-Apart
  Conditioning
    - Classic 4×4
2026-08-12 [kit: 19]  Training Day
  Strength
    - Bench Press
    - DB Shoulder Press
    - Banded External Rotation
2026-08-13 [kit: 19]  Rest Day
2026-08-14 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Band Pallof Press
2026-08-15 [kit: 19]  Training Day
  Conditioning
    - Warm-up
    - 30 m Repeats
2026-08-16 [kit: 19]  Training Day
  Conditioning
    - Warm-up
    - 10 m Acceleration Reps
```

## AWAY, dumbbells and bands — leaving Wednesday

```
2026-08-10 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Ab Wheel
  Conditioning
    - Steady Blocks (3×8 min or 4×6 min)
2026-08-11 [kit: 19]  Training Day
  Team Training
2026-08-12 [kit: 3]  Training Day
  Strength
    - Single-Arm DB Floor Press
    - DB Shoulder Press
    - Banded External Rotation
  Conditioning
    - Classic 4×4
2026-08-13 [kit: 3]  Rest Day
2026-08-14 [kit: 3]  Training Day
  Strength
    - Goblet Squat
    - RDLs
    - Cossack Squat
    - Single-Leg RDL
    - Band Pallof Press
2026-08-15 [kit: 3]  Training Day
  Conditioning
    - Warm-up
    - Steady Blocks (3×8 min or 4×6 min)
2026-08-16 [kit: 3]  Rest Day
```

Swapped just for these days — his normal exercise is untouched underneath:

- **Single-Arm DB Floor Press** instead of Bench Press (kit_today)
- **Goblet Squat** instead of Back Squat (kit_today)
- **Cossack Squat** instead of Bulgarian Split Squats (kit_today)

## AWAY with nothing — a hotel room and a floor

**NO WEEK AT ALL — the app refused to build one.**

```
generated_week_refused: Generated week refused (main_strength_planner_selected_target:3)
      findings: [{"clause":"main_strength_planner_selected_target","severity":"blocking","detail":"the planner's own selected main-strength target is not met","expected":4,"actual":3}]
```

## HOME AGAIN — the first week back

```
2026-08-17 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Ab Wheel
2026-08-18 [kit: 19]  Training Day
  Team Training
2026-08-19 [kit: 19]  Training Day
  Strength
    - Back Squat
    - RDLs
    - Bulgarian Split Squats
    - Single-Leg RDL
    - Band Pallof Press
2026-08-20 [kit: 19]  Training Day
  Team Training
2026-08-21 [kit: 19]  Rest Day
2026-08-22 [kit: 19]  Game Day
  Game Day
2026-08-23 [kit: 19]  Rest Day
```

Swapped just for these days — his normal exercise is untouched underneath:

- **Single-Arm DB Floor Press** instead of Bench Press (kit_today)
- **Goblet Squat** instead of Back Squat (kit_today)
- **Cossack Squat** instead of Bulgarian Split Squats (kit_today)

