...
/* ⚠ **THE MIRRORED KEYWORD ESCALATION AUTHORITY WAS DELETED HERE — R-108,
 * 2026-08-20**, in the same commit as `src/utils/injuryClarificationGuard.ts`.
 *
 ...
 * injury/readiness facts and Sam-approved wording."*
 *
 * ⚠ **DELETING ONLY THE CLIENT COPY WOULD HAVE LEFT THE AUTHORITY ALIVE ON THE
 * SERVER.** This file cannot import from `src/`, so it carried an embedded
 * mirror — the same two unsigned medical replies and the same 21 patterns —
 * under a `KEEP IN SYNC` note. A half-deletion here is exactly how the defect
 * comes back: the next reader finds the edge function still escalating and
 * "restores" the client to match. Both halves went together. */
