/**
 * coach-semantic-program-edit-draft edge function.
 *
 * Thin LLM transport for Stage 3D-3B shadow mode. It does not mutate,
 * does not compose a user reply, and does not claim success. The app
 * validates the returned JSON against the strict ProgramEditDraft schema.
 */
...
function getOpenAIModel(): string {
  return Deno.env.get("COACH_SEMANTIC_DRAFT_LLM_MODEL") ||
...
function getAnthropicModel(): string {
  return Deno.env.get("ANTHROPIC_SEMANTIC_DRAFT_MODEL") ||
