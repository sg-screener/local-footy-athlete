...
export interface ConditioningContext {
  /** ISO date string (YYYY-MM-DD) for the session being planned. */
  dateStr: string;
  /** Days until next game. null = no upcoming game / offseason. */
  daysToGame: number | null;
  /** Days since last game. null = no recent game. */
  daysSinceGame: number | null;
  /** Day of week: 0=Sun..6=Sat. */
  dayOfWeek: number;
  /** Current season phase. */
  seasonPhase: SeasonPhase;
  /** Active injuries. Maps body area to severity. */
  activeInjuries: Record<string, 'caution' | 'avoid'>;
}

export interface WeekLog {
  /** Conditioning sessions already placed this week, with their tier. */
  sessions: Array<{
    dateStr: string;
    tier: ConditioningTier;
    exerciseName: string;
    /** Fatigue level of the conditioning session. */
    fatigue: 'low' | 'moderate' | 'high';
  }>;
  /** Strength sessions this week with fatigue info. */
  strengthSessions: Array<{
    dateStr: string;
    /** Overall fatigue of the strength session (derived from heaviest exercise). */
    fatigue: 'low' | 'moderate' | 'high';
  }>;
  /** Number of team training sessions this week (counts toward preseason total). */
  teamTrainingSessions: number;
  /** Is this a bye week? (In-season only — no game this round.) */
  byeWeek: boolean;
  /** Did the athlete miss team training this week? (In-season only.) */
  missedTeamTraining: boolean;
  /**
   * Is this a double game week? (In-season only.)
   * Kept as a boolean because deriving it requires access to all game
   * dates within the week, which the conditioning context doesn't carry.
   */
  doubleGameWeek: boolean;
  /**
   * Consecutive weeks off training before this week.
   * 0 = normal, 1 = returning after 1 week off, 2+ = returning after extended break.
...
export interface ConditioningResult {
  /** The selected conditioning exercise name (key into EXERCISE_TAGS). */
  exerciseName: string;
  /** The tier of the selected exercise. */
  tier: ConditioningTier;
...
interface WeeklyCaps {
  maxTierA: number;
  /** Combined B cap (B-high + B-low). */
  maxTierB: number;
  /** No hard cap on Tier C — effectively unlimited. Use a high number. */
  maxTierC: number;
  /** Total conditioning sessions per week (soft cap). */
  maxTotal: number;
  /** Whether only B-low is allowed (bye week fatigued). */
  bLowOnly: boolean;
}
...
function countByTier(weekLog: WeekLog): Record<ConditioningTier, number> {
  const counts: Record<ConditioningTier, number> = { 'A': 0, 'B-high': 0, 'B-low': 0, 'C': 0 };
...
function hasLowerLimbInjury(injuries: Record<string, 'caution' | 'avoid'>): boolean {
  for (const area of Object.keys(injuries)) {
    if (LOWER_LIMB_AREAS.includes(area.toLowerCase().trim())) return true;
  }
  return false;
}
 ...
 */
function getStackingBlockedTiers(dateStr: string, weekLog: WeekLog): Set<ConditioningTier> {
  const blocked = new Set<ConditioningTier>();
  const target = new Date(dateStr + 'T12:00:00');

  for (const s of weekLog.sessions) {
    // B-low and C don't block anything
    if (s.tier === 'B-low' || s.tier === 'C') continue;
 ...
 */
function getStrengthBlockedTiers(dateStr: string, weekLog: WeekLog): Set<ConditioningTier> {
  const blocked = new Set<ConditioningTier>();
 ...
 */
function getAllowedTiersForDate(ctx: ConditioningContext): Set<ConditioningTier> {
  const allowed = new Set<ConditioningTier>(['A', 'B-high', 'B-low', 'C']);
...
function getEligibleTiers(
  ...
  const eligible: ConditioningTier[] = [];
...
function filterConditioningByInjury(
  ...
  return candidates.filter(name => {
    ...
    if (normInjuries['knee'] === 'avoid') {
      ...
      if (tags && tags.injury.knee === 'avoid') return false;
    }
    ...
    if (tags) {
      for (const [normArea, sev] of Object.entries(normInjuries)) {
        ...
        if (!areaKey) continue;
        const rating = tags.injury[areaKey as keyof typeof tags.injury];
        if (sev === 'avoid' && rating === 'avoid') return false;
        if (sev === 'caution' && rating === 'avoid') return false;
      }
    }
...
export function resolveConditioning(
  ...
  if (dateTiers.size === 0) return null;
  ...
  if (injuryFiltered.length === 0) return null;
  ...
  for (const tier of tierPriority) {
    ...
    if (tierCandidates.length > 0) {
      ...
      if (preferLowImpact && tier !== 'A') {
        ...
        const lowImpact = tierCandidates.filter(n => CONDITIONING_META[n]?.impact === 'low');
 ...
 */
export function getWeeklyConditioningSummary(weekLog: WeekLog): {
  totalSessions: number;
  tierCounts: Record<ConditioningTier, number>;
  highFatigueDays: string[];
} {
  const counts = countByTier(weekLog);
