  ...
  for (const item of items) {
    if (item.kind === 'canonical_injury_episode') {
      ...
      const constraint: ActiveInjuryConstraint = {
        ...
        status: 'active',
        source: 'coach',
        /* Defaults are the values that were hard-coded here until 2026-09-05,
         * so a seed that says nothing gets exactly what it got before. */
        region: item.region ?? 'lower_body',
        severityBand: item.severityBand ?? 'moderate',
        adjustmentLevel: item.adjustmentLevel ?? 'moderate',
        triggers: [...(item.triggers ?? ['Sprinting', 'Running'])],
