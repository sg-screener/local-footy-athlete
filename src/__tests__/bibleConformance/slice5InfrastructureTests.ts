...
check('mutation registry preserves 28 accepted smoke probes', SMOKE_MUTATIONS.length === 28);
...
check('coverage registry has 41 rules and no missing mutation rationale', reportA.rules.length === 41 && reportA.rules.every((rule) => rule.mutationIds.length > 0 || rule.exemptions.length > 0));
