...
import { CONDITIONING_COACHING_COPY, conditioningDisplayLines, conditioningAthletePrescription } from '../../rules/conditioningDisplay';
...
export async function conditioningClarity(storage: Map<string, string>, ok: Check) {
  ...
  let mixed = 0;
  for (const t of templates) for (const machines of [['bike','air_bike','row','ski'],['air_bike'],['row','ski'],['bike','row']] as const) {
    ...
    const rows = composeConditioningRows(t,dateForUnit);
  ...
  for (const gender of ['male', 'female'] as const) {
    ...
    const labels = JSON.stringify(workouts.flatMap(w => buildSessionTemplate(w).items.filter(i => i.kind === 'exercise')
      .map(i => i.kind === 'exercise' ? i.modalityLabel : null)));
