...
const coachingEngine_1 = require("../utils/coachingEngine");
const sessionResolver_1 = require("../utils/sessionResolver");
const sessionBuilder_1 = require("../utils/sessionBuilder");
// ═══════════════════════════════════════════════════
// CONSTANTS
// ═══════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════
function classifyRegion(session, focusOverride) {
    if (!session)
        return 'neutral';
    const focus = (focusOverride || ('focus' in session ? session.focus : '') || ('name' in session ? session.name : '')).toLowerCase();
    const tier = 'tier' in session ? session.tier : ('sessionTier' in session ? session.sessionTier : '');
    if (tier === 'recovery')
        return 'neutral';
    if (focus.includes('full body') || focus.includes('conditioning') || focus.includes('aerobic'))
        return 'neutral';
    if (focus.includes('game') || focus.includes('sprint') || focus.includes('mas ') || focus.includes('metcon'))
        return 'neutral';
    if (focus.includes('flush') || focus.includes('easy ') || focus.includes('tempo'))
        return 'neutral';
    if (focus.includes('flog') || focus.includes('circuit'))
        return 'neutral';
    if (focus.includes('6x1km') || focus.includes('row interval') || focus.includes('ski') || focus.includes('bike'))
        return 'neutral';
    if (focus.includes('lower body') || focus.includes('squat') || focus.includes('hinge') || focus.includes('leg'))
        return 'lower';
    ...
    if (focus.includes('arm') || focus.includes('pump') || focus.includes('bicep') || focus.includes('tricep'))
        return 'upper';
    if (focus.includes('accessor') || focus.includes('prehab') || focus.includes('trunk') || focus.includes('shoulder'))
        return 'upper';
    if (focus.includes('hypertrophy'))
        return 'upper';
    if (focus.includes('mobility') || focus.includes('foam') || focus.includes('recovery') || focus.includes('rest'))
        return 'neutral';
    return 'neutral';
}
...
// ASSERTIONS
// ═══════════════════════════════════════════════════
function runAssertions(plan, resolvedWeek, scenario) {
    const results = [];
    const sorted = [...plan.weeklyPlan].sort((a, b) => DAY_ORDER_MON_FIRST.indexOf(a.dayOfWeek || '') - DAY_ORDER_MON_FIRST.indexOf(b.dayOfWeek || ''));
    ...
    if (isInSeason && gameDay) {
        ...
        if (g1Session) {
            ...
            const ok = isArmsOrPump || isRecovery;
            results.push({
    ...
    if (isInSeason && gameDay && resolvedWeek) {
        const gameDateStr = resolvedWeek.find(d => d.source === 'game')?.date;
        if (gameDateStr) {
            const g1DateStr = (0, sessionResolver_1.addDays)(gameDateStr, 1);
    ...
    if (resolvedWeek) {
        ...
        const templateEmpty = resolvedWeek.filter(d => d.source === 'template' && d.workout &&
...
// ═══════════════════════════════════════════════════
function buildScheduleState(inputs, plan, gameDayOverride, calendarOverrides) {
    const now = new Date().toISOString();
    // Build workouts from the coaching plan (simplified — no AI content)
    const workouts = plan.weeklyPlan.map((s, idx) => {
    ...
    const microcycle = {
        ...
        endDate: (0, sessionResolver_1.addDays)(BLOCK_START, 6),
        miniCycleNumber: 1,
    ...
    if (effectiveGameDay) {
        const gameDates = (0, sessionResolver_1.computeGameDatesForBlock)(effectiveGameDay, BLOCK_START, BLOCK_END);
    ...
    return {
        ...
        manualOverrides: {},
        markedDays,
        athleteContext: sessionBuilder_1.DEFAULT_ATHLETE_CONTEXT,
...
// OUTPUT FORMATTER
// ═══════════════════════════════════════════════════
function printScenario(scenario, plan, resolvedWeek, assertions) {
    ...
    for (const s of sorted) {
        ...
        console.log(`  ${(s.dayOfWeek || '?').padEnd(10)} ${s.tier.padEnd(10)} ${regionLabel(region).padEnd(7)} ${gLabel.padEnd(5)} ${focusTrunc}`);
    }
...
for (const scenario of scenarios) {
    const inputs = (0, coachingEngine_1.onboardingToCoachingInputs)(scenario.onboarding);
    const plan = (0, coachingEngine_1.buildCoachingPlan)(inputs);
    ...
    try {
        ...
        resolvedWeek = (0, sessionResolver_1.resolveWeekWithConditioning)(TEST_MONDAY, schedState);
    }
    catch (err) {
