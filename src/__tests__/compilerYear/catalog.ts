...
export interface Archetype {
  /** false = out of every year replay and long journey (R-401). */
  replay?: boolean;
  id: string;
  gender: 'male' | 'female';
  days: DayOfWeek[];
...
// R-401 (Sam, 2026-09-10): 'male-6-no-standing-fixture' (six gym days, no
// club, no standing game) took an hour per 52-week replay where the others
// took three and a half minutes. It is OUT of every year replay and long
// journey (`replay: false`; replays read `REPLAY_ARCHETYPES`). It stays in
// this list as a quick-test fixture only: two dozen second-long tests reach it
// by position (`ARCHETYPES[6]`) and deleting it moved them all onto another
// athlete. Nothing that replays a year may read `ARCHETYPES` directly.
export const ARCHETYPES: readonly Archetype[] = [
  ...
  { id: 'male-6-no-standing-fixture', replay: false, gender: 'male', days: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'], experience: '5+ years', equipment: 'commercial', initialPhase: 'Pre-season', clubDays: [], gameDay: null, extraGame: true },
...
/** The fleet a year replay may run: every archetype not marked `replay: false` (R-401). */
export const REPLAY_ARCHETYPES: readonly Archetype[] = ARCHETYPES.filter((a) => a.replay !== false);
