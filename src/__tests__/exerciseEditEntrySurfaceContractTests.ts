...
console.log('\n[3] The guided flows behind the buttons are UNCHANGED (ruling 12\'s boundary)');
{
  ...
  // `openExerciseAdd`, `openAddFamily` and `openAddSubcategory` over
  ...
  //
  // ⚠ **`openAddGroup` IS GONE WITH THE FLAT MENU (Sam, 2026-08-20).** It owned
  // ONE level over the generation vocabulary's own 23 groups. Sam's hierarchy is
  // three levels — *"Strength / Conditioning / Mobility-Warm-up"*, then a
  // subcategory, then the choices — so its two successors are pinned here by
  // name. Re-pointing rather than deleting the row is the rule: a gate that
  // stops watching a surface because the surface was renamed is a gate that has
  // quietly stopped watching.
  const flowOwners = [
    'prepareSwap', 'openExerciseAdd', 'openAddFamily', 'openAddSubcategory',
...
console.log('\n[4] Every step the guided flows land on still exists');
{
  ...
  // `add_group` — the ONE flat level — is deleted in its turn (Sam, 2026-08-20).
  // Its three replacements are named here for the same reason `add_kind`'s were.
  const survivingSteps = [
    'pick_exercise', 'confirm_remove', 'confirm_swap',
    'choose_swap', 'add_family', 'add_subcategory', 'add_pick',
