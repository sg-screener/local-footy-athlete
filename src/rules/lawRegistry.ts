...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The onboarding Game Day and usual gym-day pickers show the canonical Monday-through-Sunday week in one horizontal row of compact rounded-square tiles labelled M T W T F S S. Every tile keeps its full weekday as its accessibility label. Game Day says "Select the day you play most often." The usual gym-day picker keeps its existing multi-select cap, dimming and Continue behaviour. The shared day picker keeps its 3-3-1 default and three-letter labels for screens that do not explicitly request the row.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-159 + R-160 + R-168 — Sam supplied the seven-across Game Day reference while retaining the rounded-square shape, required usual gym days to match it, then corrected both rows to first-letter labels.',
    guard: {
      state: 'guarded',
      by: 'test:onboarding-presentation + test:game-anchor',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED UNDER R-160 and R-168 by the same seat. The presentation suite pins the exact Game Day instruction, both screens\' explicit single-row request, the shared picker\'s one-container map over its canonical DAYS list, six-pixel horizontal gap, flex-one square geometry, zero inner padding, absence of a circular radius, first-letter visual labels, full-weekday accessibility labels, and retention of the 3-3-1 default with three-letter labels for non-row consumers. R-159 TEST-FIRST LIVENESS: the four new requirements were the suite\'s only reds against the old 3-3-1 Game Day (89/93); the shared layout variant returned it to 93/93. R-160 TEST-FIRST LIVENESS: the new usual-gym-days row cell was the suite\'s only red against the 3-3-1 screen (93/94); selecting the existing shared variant returned it to 94/94 without changing the cap path. R-168 TEST-FIRST LIVENESS: the new label/accessibility cell was the suite\'s only red against the three-letter row (95/96); deriving one compact label while retaining the full id returned it to 96/96. `test:game-anchor` independently keeps its whole-week picker cell green but remains 13/15 on two pre-existing unrelated generation/reader findings. NOT COVERED: simulator pixels, small-screen clipping, Dynamic Type and the next physical-phone Release.',
    },
  },
  {
