...
export const LAW_REGISTRY: readonly LawRow[] = [
  // ── (a) THE AGENTS.md LAWS ────────────────────────────────────────────────
  {
    ...
    law: 'An ordinary healthy Off-season week plus readiness, illness, injury and target-week fixture adjustments are authored through one canonical weekly compiler: the scheduler, specialist materialiser and connector each have that compiler as their only production caller; generation does not prebuild a rival initial plan or schedule the same week twice; migrated fact families enter as typed semantic directives and no scheduler translator, generator, retained adapter, transaction or read-side resolver independently re-resolves their dose, optionality, fixture identity, injury restrictions or derived plan; injury policy is classified once and drives strength patterns, sprint, conditioning, power and exercise-pool keys; a game moved onto an occupied day takes precedence and moving it back restores the exact accepted week; clearing a temporary injury restores the exact healthy week; standing game answers remain dormant in Off-season; refusal returns no partial plan; and visible prescriptions equal the accepted compiler-authored week.',
    ruledAt: 'docs/STATUS_TESTTRUTH.md steps 4-8 (first compiler slice, readiness, illness, fixture and injury families); Sam, 2026-08-27: "Build the first complete canonical weekly compiler slice" and successive "go for it" instructions.',
    guard: {
      state: 'guarded',
      by: 'test:canonical-weekly-compiler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-27 by seat `testtruth`; extended the same day through readiness, illness, fixtures and injury. INJURY: raw onboarding injuries plus accepted facts are translated once into `CanonicalWeeklyInjuryState`; the compiler supplies that policy to power, both exposure contracts and conditioning, and returns its exercise-pool keys. The real action witness found and closed two further gaps before acceptance: Contract v2 was not receiving injury policy, and onboarding had not recorded the healthy selections temporary facts must preserve. A 7/10 knee now commits through the production door, carries squat/hinge and app-sprint restrictions into the accepted contract, keeps unaffected work, and clearing the exact episode restores the visible week byte-for-byte. Weekly injury rival authors in the guarded production path: 0. Mutations remove both compiler handoffs. LIVE: compiler 102/102; latest-severity 48/48; injury recomposition 41/41; off-feet conditioning 6/6; release 4/4. The diagnostic `test:section18-v2` is independently unrunnable because its imported `utils/section18ProgramObservation` module is absent and did not direct product work. FIXTURE OPTIONS COMPARED: retain targetFixtureDay/targetWeekAvailability interpretation in the scheduler translator plus read-side contract rebuilding and a transaction-owned final replan, or translate accepted calendar availability once into a semantic fixture state and let the compiler own scheduling, connector fields, accepted identity and the final specialist call. The typed compiler route landed because it deletes the disagreement class. TESTS FIRST: fixture expansion printed 66 green / 10 red before production changes; one red assumed a presentation component for Game Day and was corrected to assert the semantic Game Day plus zero training rows. LIVE witness is now 80/80: a real in-season cold start moves Saturday game onto occupied Wednesday through the production transaction, observes Game Day replace Wednesday training and exactly one Wednesday fixture anchor, then moves it back and restores the visible week byte-for-byte. Off-season cells prove saved fixture anchors and adjacent dates are dormant, fixture occupancy is released, and real unavailable blocks survive. Distinct source units: scheduler/materialiser/connector remain 1 caller each in canonicalWeeklyCompiler.ts; weekly-readiness rival authors 0; readiness derived-plan writers outside compiler 0; weekly-illness rival authors 0; weekly-fixture rival authors 0; final fixture specialist production callers 1 and it is the compiler. Scheduler target-fixture arguments, generator connector restatement, read-side fixture identity rebuild and direct transaction specialist invocation are gone. MUTATIONS remove the compiler scheduler call, feasibility marker, readiness dose handover, illness handover, fixture handover and compiler specialist call; each kills its cell. LIVE: compiler 80/80; scheduler group green (102/102 scheduler, 11/11 generated fixtures, 10/10 travel zero-equipment, 9/9 Off-season continuity, 20/20 spare-day options); fixture identity 7/7; G+1 3/3; derived ownership 6/6; release 4/4. The legacy fixture-mutation suite remains unchanged at 10/14 and is classified rewrite_test, so its four old assertions do not direct product changes. NOT COVERED: travel, athlete-edit and scheduled-deload families; global rival-author zero, full-year archetypes, pixels, simulator and physical iPhone.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'Progress headings carry no decorative lime dash. Main lifts always renders Pull-Up, Bench Press, RDL and Back Squat in that order, including an honest No data yet card when a lift has no usable history. Graph values are Brzycki predicted 1RM from the week\'s best completed 1–10-rep set, preferring one real weight-and-reps pair and otherwise using a fully completed prescription; partial work without set detail and sets above ten reps create no point. Pull-Up calculates bodyweight plus added load and displays the predicted added-load maximum with a plus prefix.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-255 — Sam removed Progress heading dashes and required four permanent predicted-1RM lift graphs that remain comparable across 3-, 5- and 8-rep prescriptions.',
    guard: {
      state: 'guarded',
