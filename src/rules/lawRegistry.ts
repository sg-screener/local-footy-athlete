...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The accepted new Program template uses a large Day / Week shape toggle; Day renders no week-date navigation, Week renders one compact previous/range/next row directly below it, and changing or returning weeks leaves every session collapsed.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "SAM EYE PASS 6 — WEEK NAVIGATION BELONGS TO WEEK"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), UPDATED WITH SAM\'S 2026-08-11 DAY / WEEK EYE PASS. FOUNDING CASE: the accepted day template had no week browsing, but the implementation mounted a large previous/range/next bar before the shape toggle so both shapes inherited it. LATEST FINDING: the control still said Today and retained a tiny 12pt compact-pill treatment after Renee\'s larger Day / Week control had been accepted. WHAT THE CELLS HOLD: visible and accessibility copy both say Day / Week; the shared shell is 280pt wide, each half has a 44pt target, and the label is 15pt; the navigator is ordered after the toggle and before Week content, mounts only when !dayFirst, preserves previous/current/next stable doors, contains neither IconButton nor Badge, and pins its small plain proportions. The template has one local expandedWeekIdx; Day reads todayIdx directly; every date and shape transition clears local expansion. WHAT THE TAPE HOLDS: all navigator ids are absent on Day and present after Week is selected; previous and next move adjacent, return to this week, and leave detail collapsed. WHAT IT DOES NOT HOLD: Sam\'s physical iPhone, the old/classic template, adjacent-week Completed styling, or dynamic-type scaling.',
    },
  },
  {
    id: 'LAW-day-timeline-icon-is-marker',
    law: 'On the accepted new Today card, each component icon is its one timeline marker: no separate dot or connector rail; the main headline does not repeat the icon; there is no lime left rail; the owned mobility warm-up appears before every projected component (including conditioning when present); and the headline, counts, badge, session action and quiet change link follow Renee\'s hierarchy.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "SAM EYE PASS 7 — THE SESSION ICON IS THE MARKER" + "SAM EYE PASS 8 — THE DAY REVIEW FINISHES THE MATCH"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/day-card-dropdowns.yaml + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), EXPANDED WITH SAM\'S NEXT EYE PASS. FOUNDING CASE: the interactive day timeline drew a hollow node and connector rail, then a typed component icon beside them, so one component had two markers and its oversized heading/count started a full column too far right. THE NEXT GAP: the card still kept a lime left rail Renee did not have, shrank its main title to 16pt, shouted the change link in lime, and showed only projected strength while the session screen\'s owned optional mobility flow was absent. WHAT THE CELLS HOLD: no duplicate marker or left rail survives; one timelineIconMarker wraps RowIcon and receives saved completion colour; the day headline region contains no duplicate RowIcon; eyebrow, 19pt main title, component heading/count, divider, Start Session and quiet note sizes are pinned; Home asks selectMobilityPrehabFlow — the same owner as the session — once per resolved day, and the shared timeline renders that flow first plus every projected part, so conditioning appears whenever the projection carries it. The flat Week branch consumes the same entries without nested furniture. WHAT THE TAPES HOLD: the standard route renders and captures the collapsed icon-led Today card before traversing Week; the dropdown route now observes and opens the mobility warm-up as well as the projected strength section, then closes both. FIRST-RUN FINDINGS: the marker cell first failed on timelineRail; the expanded cells first failed on the 8pt eyebrow, 16pt title, lime rail/link and absent mobility owner. WHAT IT DOES NOT HOLD: Sam\'s physical iPhone, spoken VoiceOver order, exceptionally long translated headings, the old/classic template.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'A live session uses one shared collapsible section shell and the same square checkbox for every exercise or unit. Mobility / Warm-up, Strength and Team Training use that same shell and checkbox recipe; accessories and prehab are rows inside Strength, and Team Training expands to a plain checklist row rather than a second card. Checked rows visibly recede; those ticks are the sole completion evidence and persist with one whole-session 1-5 effort score. Mobility is prescribed with no optional wording; separately authored Optional Work remains no-penalty.',
    ruledAt: 'NOT STATED IN THE REPO — Sam, 2026-08-11 in this Codex task: "each component of the session should be its own pop down"; exercises are checked as they are done; completion is derived from the ticks; the bottom feedback is 1-5, where 1 is very easy and 5 is very hard. UPDATED by Sam 2026-08-11 in this Codex task: "Don\'t make the mobility warm-up optional - just have it there and they can mark as skipped or not skipped - just like they would do with strength work - so remove optional wording". This guarded registry row is the durable ruling site.',
    guard: {
      state: 'guarded',
      by: 'test:session-execution-checklist',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), UPDATED WITH SAM\'S 2026-08-11 MOBILITY, SHARED-SECTION AND SQUARE-CHECKBOX RULINGS. TWO OPTIONS COMPARED FOR THE CHECKBOX FINDING: copy the ordinary row\'s measurements into Mobility, or put both row renderers behind one shared visual recipe while preserving their existing tap targets. The second removes the styling fork that produced an 18px circle beside 22px squares. WHAT THE CELLS HOLD: the pure plan groups accessory/prehab items into Strength while retaining stable item and component evidence; Mobility is wrapped by the same SessionExecutionSection and its row renderer owns no disclosure state; Mobility and ordinary rows spread the same square checkbox, checked-state and tick recipes; Team Training is a plain exercise-card row with no accent Card language; all sections retain controlled chevrons, checkbox rows, completed styling and full/partial/skipped derivation; unticked mobility remains prescribed and affects the aggregate; Optional Work remains outside it; the accepted result preserves every item and one 1-5 effort score. FIRST RUN: three old cells required the now-rejected separate Accessories measurement and Mobility-owned chevron, and four mobility-flow cells required its parallel disclosure owner; they red immediately and were replaced with ownership cells. WHAT IT DOES NOT HOLD: Sam\'s physical iPhone, VoiceOver traversal order, or analytics/recommendations consuming the evidence.',
    },
  },
  {
