...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'Welcome uses Sam\'s approved BUILT FOR FOOTY copy: one complete program; Your Plan / Train everything that matters / Strength, speed, conditioning and recovery; Your Week / Footy comes first; Your Progress / See how you\'re tracking; Build my program; and Takes about 3 minutes. All three eyebrow labels render through one shared 2px gap before the main heading. The retired Built as one program and Everything works together headings, longer phase-specific sentence, earlier card copy and TRAIN FOR title are absent.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-148 + R-150 + R-153 + R-158 — Sam supplied the complete Welcome copy, restored the compact labels and BUILT FOR FOOTY title, shortened the first card, then replaced its heading with Train everything that matters. R-150 supersedes R-149.',
    guard: {
      state: 'guarded',
      by: 'test:onboarding-presentation',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `welcomecopy`; UPDATED UNDER R-149 by `welcomeeyebrows`; UPDATED UNDER R-150 by `welcomecompact`; UPDATED UNDER R-153 and R-158 by `onboardingtype`. The onboarding presentation cells require every approved title, label, description, CTA and timing line; reject the retired card copy, both old first-card headings, long phase-specific sentence and TRAIN FOR title; require feature.label to render; and anchor the one featureLabelWrap style at marginBottom 2 so all cards share the same compact gap. R-150 TEST-FIRST LIVENESS: the three revised cells were the suite\'s only reds against the label-free TRAIN FOR screen (67/70); restoring the shared labels and BUILT FOR title returned it to 70/70. R-153 TEST-FIRST LIVENESS: the two revised Welcome-copy cells were the suite\'s only reds against the old first card (81/83); the new short card returned it to 83/83. R-158 TEST-FIRST LIVENESS: the two Welcome-copy cells were the suite\'s only reds against Built as one program (86/88); the approved heading returned it to 88/88. NOT COVERED: simulator pixels, small-screen wrapping and the next physical-phone Release.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'The onboarding Game Day and usual gym-day pickers show the canonical Monday-through-Sunday week in one horizontal row of compact rounded-square tiles. Game Day says "Select the day you play most often." The usual gym-day picker keeps its existing multi-select cap, dimming and Continue behaviour. The shared day picker keeps its 3-3-1 default for screens that do not explicitly request the row.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-159 + R-160 — Sam supplied the seven-across Game Day reference while retaining the rounded-square shape, then required the usual gym-day screen to match it.',
    guard: {
      state: 'guarded',
