...
export const LAW_REGISTRY: readonly LawRow[] = [
  // ── (a) THE AGENTS.md LAWS ────────────────────────────────────────────────
  {
    ...
    law: 'An ordinary healthy Off-season week plus readiness and illness adjustments are authored through one canonical weekly compiler: the scheduler, specialist materialiser and connector each have that compiler as their only production caller; generation does not prebuild a rival initial plan or schedule the same week twice; both fact families enter as typed directives and no scheduler translator, generator, retained adapter or read-side resolver independently re-resolves their dose, optionality or derived plan; refusal returns no partial plan; and visible prescriptions equal the accepted compiler-authored week.',
    ruledAt: 'docs/STATUS_TESTTRUTH.md "step 4: first canonical weekly compiler slice", "step 5: readiness family" and "step 6: illness family"; Sam, 2026-08-27: "Build the first complete canonical weekly compiler slice" and successive "go for it" instructions.',
    guard: {
      state: 'guarded',
      by: 'test:canonical-weekly-compiler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-27 by seat `testtruth`; extended the same day through readiness and illness. TWO OPTIONS COMPARED: preserve combined weekMode/weekDeloaded flags plus downstream illness repairs, or translate the active illness fact once and let the compiler author dose, optionality and accepted identity. The typed compiler route landed because it deletes the disagreement class. TESTS FIRST: the illness expansion printed 46 green / 11 red before production changes and 60/60 after. The witness cold-starts a real in-season athlete and executes mild -> clear, moderate -> clear, severe -> clear through the production action and accepted-overlay transaction: mild is exact record-only, moderate reduces real dose while keeping the normal contract, severe keeps offered sessions but makes every survivor optional and authors optional_week, and every clear restores the visible baseline exactly. Distinct source units: scheduler/materialiser/connector remain 1 caller each in canonicalWeeklyCompiler.ts; weekly-readiness rival authors 0; readiness derived-plan writers outside compiler 0; weekly-illness rival authors 0; retired deriveIllnessRecoveryWeekMode production callers 0. Combined compatibility fields, generator illness policy, retained-adapter illness door, derived-contract illness rewrite and session-view illness rewrite are gone. MUTATIONS remove the compiler scheduler call, feasibility marker, readiness dose handover and illness handover; each kills its cell. LIVE: compiler 60/60, readiness/illness 121/121, illness recovery 17/17, fact horizon 14/14, deload law 68/68, scheduler group green, release 4/4. NOT COVERED: fixture, injury, travel, athlete-edit and scheduled-deload families; global rival-author zero, full-year archetypes, pixels, simulator and physical iPhone.',
    },
  },
  {
