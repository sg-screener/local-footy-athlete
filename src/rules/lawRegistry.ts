...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The accepted new Program template uses a large Day / Week shape toggle; Day renders no week-date navigation. Week renders one legible previous/range/next row directly below it with 40pt controls and 13pt range type. A compact three-dot Week-options control sits at the right of that same row with a 44pt physical target while the date navigation stays centred. No separate large Edit this week bar sits above Monday. The row is equally spaced from the toggle above and week cards below, and changing or returning weeks leaves every session collapsed.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11 "SAM EYE PASS 6 — WEEK NAVIGATION BELONGS TO WEEK"; docs/RULINGS_REGISTRY.md R-203 — Sam moved Week editing into compact dots beside the range.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), UPDATED WITH SAM\'S 2026-08-11 DAY / WEEK AND WEEK-NAV EYE PASSES, then R-203 on 2026-08-25 by seat `headeralign`. FOUNDING CASE: the accepted day template had no week browsing, but the implementation mounted a large previous/range/next bar before the shape toggle so both shapes inherited it. LATEST FINDINGS: after the Day / Week control was corrected, the row beneath it still used 28pt controls, 13pt chevrons and 11pt range type on the physical phone; after those sizes were corrected, the row still sat visibly closer to the toggle than the cards. R-203 removed the separate 48pt green-tinted Edit this week bar and put the same menu behind 22pt dots in a 24pt visible button with 10pt hit slop. The date controls stay in their centred child; the dots are absolutely right-aligned in the relative 40pt row, so adding the secondary control cannot shift the range. WHAT THE CELLS HOLD: visible and accessibility copy both say Day / Week; the shared shell is 280pt wide, each half has a 44pt target, and the label is 15pt; the navigator is ordered after the toggle and before Week content, mounts only when !dayFirst, preserves previous/current/next stable doors, contains neither IconButton nor Badge, pins 40pt previous/current/next geometry, 17pt chevrons and 13/18pt range type, uses the same spacing token above and below, owns exactly one Week-only dots ingress, and forbids the large bar styles. The R-203 focused run returned to the suite\'s inherited 54 passed / 2 failed baseline after its new cells passed; the two reds remain mobility review wiring and an absent generated Gunshow fixture. The template has one local expandedWeekIdx; Day reads todayIdx directly; every date and shape transition clears local expansion. WHAT THE TAPE HOLDS: all navigator ids are absent on Day and present after Week is selected; previous and next move adjacent, return to this week, and leave detail collapsed. WHAT IT DOES NOT HOLD: screenshots, simulator interaction, Sam\'s physical iPhone, adjacent-week Completed styling, or dynamic-type scaling.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'Renee\'s compact prototype typography remains the shared type system for the main app. Onboarding retains its larger readable size and line-height scale but uses the same System face and natural heading casing; both stay behind the shared Text owner so the size exception cannot leak into other screens.',
    ruledAt: 'NOT STATED IN THE REPO — Sam, 2026-08-11 in this Codex task first ruled "on all pages everywhere", then superseded that scope after seeing onboarding: "I want the old onboarding back the rest of the app is good". Updated by docs/RULINGS_REGISTRY.md R-192 after Sam found the retained onboarding display face still forced capitals.',
    guard: {
      state: 'guarded',
      by: 'test:prototype-typography + test:onboarding-presentation',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), UPDATED WITH SAM\'S ONBOARDING-ONLY SUPERSESSION AND R-192. WHAT THE CELLS HOLD: the main shared variant owner pins Renee\'s measured compact hierarchy; onboarding\'s separate scale keeps every original heading size and line height while the presentation tape requires all four heading variants to use System and forbids uppercase in that region; one scoped owner wraps the complete onboarding navigator; shared Text reads that scope; AppTextInput keeps the system family; and a source census fails if any athlete-facing screen or ordinary component bypasses shared Text. R-192 FOUNDING CASE: local answer cards already said `textTransform: none`, but the scoped h1-h4 owner still used Bebas Neue plus uppercase, so normal source questions rendered in capitals. TWO OPTIONS COMPARED: screen overrides or correcting the scoped owner. The owner correction landed and the audit also rejects capital-authored question/answer literals across the affected steps. TEST FIRST: the R-192 presentation block was 0/4; AFTER it is 4/4 and the full tape is 105/105. LIVE SIMULATOR: `What should I call you?` is visible and the uppercase node is absent. WHAT IT DOES NOT HOLD: the five inherited athlete-facing shared-Text bypasses named by `test:prototype-typography`, local per-screen size overrides, OS status-bar glyphs, physical iPhone and every later onboarding step viewed manually.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'The onboarding Game Day, Team Training Days and usual gym-day pickers show the canonical Monday-through-Sunday week in one horizontal row of compact rounded-square tiles labelled M T W T F S S. Every tile keeps its full weekday as its accessibility label. Game Day says "Select the day you play most often." Both multi-select screens keep their existing selection, feedback and Continue behaviour. The shared day picker keeps its 3-3-1 default and three-letter labels for any screen that does not explicitly request the row.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-159 + R-160 + R-168 + R-169 — Sam supplied the seven-across Game Day reference, required usual gym days to match it, corrected the row to first-letter labels, then required Team Training Days to match too.',
    guard: {
      state: 'guarded',
      by: 'test:onboarding-presentation + test:game-anchor',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED UNDER R-160, R-168 and R-169 by the same seat. The presentation suite pins the exact Game Day instruction, all three screens\' explicit single-row request, the shared picker\'s one-container map over its canonical DAYS list, six-pixel horizontal gap, flex-one square geometry, zero inner padding, absence of a circular radius, first-letter visual labels, full-weekday accessibility labels, and retention of the 3-3-1 default with three-letter labels for non-row consumers. R-159 TEST-FIRST LIVENESS: the four new requirements were the suite\'s only reds against the old 3-3-1 Game Day (89/93); the shared layout variant returned it to 93/93. R-160 TEST-FIRST LIVENESS: the new usual-gym-days row cell was the suite\'s only red against the 3-3-1 screen (93/94); selecting the existing shared variant returned it to 94/94 without changing the cap path. R-168 TEST-FIRST LIVENESS: the new label/accessibility cell was the suite\'s only red against the three-letter row (95/96); deriving one compact label while retaining the full id returned it to 96/96. R-169 TEST-FIRST LIVENESS: the Team Training row cell was the suite\'s only red against its remaining 3-3-1 layout (96/97); selecting the shared row returned it to 97/97 without changing its selection or Continue code. `test:game-anchor` independently keeps its whole-week picker cell green but remains 13/15 on two pre-existing unrelated generation/reader findings. NOT COVERED: simulator pixels, small-screen clipping, Dynamic Type and the next physical-phone Release.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'Profile names the focus row Main goal/s. Its “Something changed? Tell the coach” control uses the exact same dark edit row, lime text and chevron as Edit player details, Edit equipment and Edit program details. One shared component owns all four actions, with no decorative pencil.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-179 for the label, R-184 for removing the pencil and R-186 superseding R-182\'s lime primary-button treatment with the shared setup-edit row.',
    guard: {
      state: 'guarded',
      by: 'test:profile-reset-ui',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`. R-179 first replaced the flush footer with a dark edit-button treatment. R-182 superseded that visual choice with the shared primary button; R-184 then removed its pencil. UPDATED FOR R-186: Sam compared the Profile CTA on glass with the three edit rows inside its destination and chose their quieter hierarchy. TWO IMPLEMENTATIONS COMPARED were restyling the outer CTA independently, or extracting the identical row into one `SetupEditAction` owner used by all four actions. The shared owner landed, so the outer control cannot drift from the page it opens. TEST FIRST: the exact-shape cell began 175/1 against the lime V2Button; after all four call sites used the shared dark row it returned `test:profile-reset-ui` to 176/0 and the chained wordmark tape to 15/0. The guard counts four call sites, anchors all four labels and pins the shared dark-row/text/chevron styles with no pencil. NOT COVERED: simulator or physical-iPhone pixels, real VoiceOver speech and the next Release rebuild.',
    },
  },
  {
    id: 'LAW-day-change-card-owns-plan-actions',
    law: 'The Day status card asks “Not feeling 100%?” and explains “Tell us what’s changed and we’ll adjust today.” It exposes Tired, Sick and Injured as its only direct controls and owns no scheduling doorway. A programmed non-game session card exposes one compact three-dot plan-options control beside its tier badge; the visible control stays small while its invisible physical tap target is at least 44 by 44 points, and it opens the deterministic Add, Move and Remove menu headed “Session options”, whose first row reads “Add to this session”. Its destructive row reads “Remove session”; its sub-line reads “Make this a rest day” only when the typed remove scope says removal empties the day, otherwise it truthfully says remaining content stays. A rest or Team Training-only day, where there is no programmed-session card to own the dots, retains a direct Add a session doorway. Game Day retains its specialised controls. The active-workout Equipment/Injury/Add hub keeps its own “Need to make a change?” heading. There is no separate Want to change something link and no dedicated whole-session Swap action on Day or Week. Move may trade ordinary occupied sessions, but a Team Training destination is a combine: Team Training stays on its day and is never swapped with strength work. The open-workout hub remains Equipment, Injury and Add because row-level quick actions own exercise swaps and removals.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-181, R-194, R-195, R-196, R-197, R-198, R-199 and R-200 — Sam consolidated Day edits, separated athlete status from scheduling, signed the status wording, moved plan options onto the programmed session card with an invisible 44-point tap target, then signed its heading, Add and Remove labels; Strength ↔ Team Training swaps remain forbidden.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + test:move-scoping + test:accessibility-contracts',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED FOR R-194 the same day and R-195/R-196/R-197/R-198/R-199/R-200 on 2026-08-25. R-196 TWO OPTIONS COMPARED: add a second menu architecture to the session card, or relocate the existing canonical PlanChangeSheet ingress and delete the status-card doorway. The relocation landed because it changes ownership without duplicating policy or writes. TEST FIRST: `test:session-change-hub` was 59 passed / 4 failed against the Edit day implementation, then 63 passed / 0 failed after the dots, status-only owner, 44-point target and no-session Add fallback landed. LIVENESS: mutating hitSlop 10 to 9 made the exact target cell red at 62 passed / 1 failed; restoring returned green. R-197, R-198 and R-199 changed the menu words through signed copy rather than local literals. The R-198 source cells each produced one additional red before implementation; afterwards `test:session-change-hub` is 63/0, `test:day-first-timeline` is back at its inherited 54/2 baseline, and `test:move-scoping` is back at its inherited 17/1 baseline. R-199 is held by `test:accessibility-contracts`, which requires the Day-path SheetHeader to read the signed “Session options” row. Its focused run was 45 passed / 2 failed with the R-199 cell green; both inherited reds name popup-count drift in untouched `SeasonPhaseShiftSheet` / `HomeScreenV2`. R-200 changes the signed Day-status explanation in the same row, held exactly by `test:session-change-hub`. The remove sub-line remains selected by `removeEmptiesTheDay`, so a combined day cannot falsely promise rest. `test:signed-copy-extraction` is 7/7. `test:compile` remains at the concurrent 483-error / 60-worsened-pair baseline and names none of this slice\'s changed product files. LIVE SIMULATOR, NO SCREENSHOTS: home-plan-options was visible, home-edit-day absent, the tap exposed Add/Move/Remove, Wednesday Rest retained Add a session which entered the Add type picker, R-197 showed “Add to this session” with “Add this session” absent, and R-198 showed “Remove session”. The nested subtitle is intentionally not a separate accessibility leaf, so its exact text is held by the source and copy-binding cells rather than falsely claimed from the hierarchy. NOT COVERED: physical-iPhone Release, VoiceOver reading/order, every capability combination, committing a real plan edit and exact pixels on all device widths.',
    },
  },
  {
    id: 'LAW-week-edit-sheet-names-adjustment',
    law: 'Tapping the Week dots opens a sheet headed “Adjust this week”. Its first action-selection step asks “What do you want to change?”. Under “Schedule changes” it reads: “I have a bye” / “Remove this week’s game”; “Add a game” / “Add another game to this week”; and “I’m going away” / “Adjust around travel or time away”. A quiet divider and gap introduce “Training”, whose one row reads “Manage sessions” / “Add, move or remove training”. Its nested step reads: “Add a session” / “Choose a day to add it to”; “Move a session” / “Choose a session and move it to another day”; and “Remove a session” / “Choose a session to remove from the week”. Add a game is the one popup label in both competitive phases.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-201, R-202, R-204, R-205, R-206 and R-207 — Sam supplied the heading/question and option wording, grouped schedule changes apart from training, extended the heading to “Adjust this week”, supplied the nested session-action explanations, then shortened Add to the next choice.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-25 by seat `headeralign`; corrected under the same R-201 minutes later and extended for R-202/R-204/R-205/R-206/R-207. The existing WeekEditSheet remains the one owner; its heading, first-step question, two section labels, four top-level option pairs and three nested session-action pairs read signed copy. The Week-edit cells in `test:day-first-timeline` require every signed value and binding, the requested section order, a quiet divider/gap, and the same action doors. Focused run: the R-207 Add subline cell is green at the suite\'s inherited 54 passed / 2 failed baseline; the two reds still name mobility review wiring and an absent generated Gunshow fixture. NOT COVERED: screenshots, simulator interaction, physical iPhone, translated copy and completing a session mutation.',
    },
  },
];
