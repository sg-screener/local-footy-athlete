...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The Day status card asks “Not feeling 100%?” and explains “Tell us what’s changed and we’ll adjust your training.” It exposes Tired, Sick and Injured as its only direct status controls, followed by one visually separate Edit day doorway into the deterministic Add, Move and Remove menu. The active-workout Equipment/Injury/Add hub keeps its own “Need to make a change?” heading. There is no separate Want to change something link and no dedicated whole-session Swap action on Day or Week. Move may trade ordinary occupied sessions, but a Team Training destination is a combine: Team Training stays on its day and is never swapped with strength work. The open-workout hub remains Equipment, Injury and Add because row-level quick actions own exercise swaps and removals.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-181, R-194 and R-195 — Sam consolidated Day edits, separated athlete status from scheduling and then signed the status-only wording; Strength ↔ Team Training swaps remain forbidden.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + test:move-scoping',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED FOR R-194 the same day and R-195 on 2026-08-25. TWO OPTIONS COMPARED FOR R-195: replace the shared heading globally, or give the active-workout hub its own signed heading while changing only Day. The isolated heading landed because the session hub controls Equipment/Injury/Add and is not a physical-status question. TEST FIRST: two exact-copy cells were red against the previous shared wording; after the signed copy split and Day wording update, `test:session-change-hub` is 61/61. `test:day-first-timeline` remains at its exact inherited 54/2 baseline and `test:signed-copy-extraction` is 7/7. LIVE SIMULATOR, NO SCREENSHOTS: both new Day lines, Tired, Injured and Edit day were visible together. R-194 evidence remains: the revised hub contract produced five named reds against the six-chip implementation, the optional-action default cell caught a simulator skip into Add, and the corrected Edit day interaction rendered Add, Move and Remove together. `test:move-scoping` owns Strength -> Team Training combine behaviour at its inherited 17/1 baseline. NOT COVERED: physical-iPhone Release, VoiceOver order, the active-session heading on glass in this pass and a real committed multi-edit sequence.',
    },
  },
];
