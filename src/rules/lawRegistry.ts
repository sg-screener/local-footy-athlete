...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'A live session uses one shared collapsible section shell and a checkbox for every exercise or unit. Mobility / Warm-up, Strength and Team Training use that same shell; accessories and prehab are rows inside Strength, and Team Training expands to a plain checklist row rather than a second card. Checked rows visibly recede; those ticks are the sole completion evidence and persist with one whole-session 1-5 effort score. Mobility is prescribed with no optional wording; separately authored Optional Work remains no-penalty.',
