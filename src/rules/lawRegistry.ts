...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The Day status card asks “Not feeling 100%?” and explains “Tell us what’s changed and we’ll adjust today.” It exposes Tired, Sick and Injured as its only direct controls and owns no scheduling doorway. A programmed non-game session card exposes one compact three-dot plan-options control beside its tier badge; the visible control stays small while its invisible physical tap target is at least 44 by 44 points, and it opens the deterministic Add, Move and Remove menu headed “Session options”, whose first row reads “Add to this session”. Its destructive row reads “Remove session”; its sub-line reads “Make this a rest day” only when the typed remove scope says removal empties the day, otherwise it truthfully says remaining content stays. A rest or Team Training-only day, where there is no programmed-session card to own the dots, retains a direct Add a session doorway. Game Day retains its specialised controls. The active-workout header exposes its own compact circle-free dots with a 48-point invisible target. Those dots open “Session options” / “Need to make a change?” with Something hurts, Equipment changed when the session has equipment requirements, and Add an exercise; each row enters the established Injury, Equipment or Add handler. The former active-workout change card is absent. There is no separate Want to change something link and no dedicated whole-session Swap action on Day or Week. Move may trade ordinary occupied sessions, but a Team Training destination is a combine: Team Training stays on its day and is never swapped with strength work. Row-level quick actions continue to own exercise Swap and Remove.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-181, R-194, R-195, R-196, R-197, R-198, R-199, R-200 and R-209 — Sam consolidated Day edits, separated athlete status from scheduling, signed the status wording, moved plan options onto the programmed session card, then moved active-workout Injury/Equipment/Add into a compact header menu; Strength ↔ Team Training swaps remain forbidden.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + test:move-scoping + test:accessibility-contracts',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED FOR R-194 the same day, R-195/R-196/R-197/R-198/R-199/R-200 and then R-209 by seat `headeralign` on 2026-08-25. R-196 TWO OPTIONS COMPARED: add a second menu architecture to the session card, or relocate the existing canonical PlanChangeSheet ingress and delete the status-card doorway. The relocation landed because it changes ownership without duplicating policy or writes. TEST FIRST: `test:session-change-hub` was 59 passed / 4 failed against the Edit day implementation, then 63 passed / 0 failed after the dots, status-only owner, 44-point target and no-session Add fallback landed. LIVENESS: mutating hitSlop 10 to 9 made the exact target cell red at 62 passed / 1 failed; restoring returned green. R-197, R-198 and R-199 changed the menu words through signed copy rather than local literals. The R-198 source cells each produced one additional red before implementation; afterwards `test:session-change-hub` is 63/0, `test:day-first-timeline` is back at its inherited 54/2 baseline, and `test:move-scoping` is back at its inherited 17/1 baseline. R-199 is held by `test:accessibility-contracts`, which requires the Day-path SheetHeader to read the signed “Session options” row. R-200 changes the signed Day-status explanation in the same row. R-209 TWO OPTIONS COMPARED: put the existing three-chip hub inside a sheet, or use the established labelled sheet-row primitive and preserve only the three handler pathways. The row menu landed because the requested labels and explanations need readable horizontal space and the old card is removed rather than hidden. The focused `test:session-change-hub` run is 41/41: it pins the 24-point circle-free dots, 48-point invisible target, signed row order, conditional Equipment row, menu-close-before-handler wiring, deleted old hub and retained row-level Quick Swap/Remove. The plan Remove sub-line remains selected by `removeEmptiesTheDay`, so a combined day cannot falsely promise rest. NOT COVERED: screenshots, simulator interaction, physical-iPhone Release, VoiceOver reading/order, every capability combination and completing a real session edit.',
    },
  },
  {
