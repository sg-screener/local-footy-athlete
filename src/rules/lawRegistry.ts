...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    guard: {
      ...
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), AND UPDATED WITH SAM\'S 2026-08-11 MOBILITY RULING. TWO OPTIONS COMPARED FOR THE UPDATE: remove only the optional word while leaving mobility outside aggregate completion, or make the already-tracked mobility section participate in the prescribed-section aggregate. The second removes the contradiction where the summary could say Everything completed while Mobility / Warm-up said skipped. WHAT THE CELLS HOLD: one pure plan groups mobility, power, strength, accessories, conditioning, team and recovery rows; stable item ids; controlled chevron sections and checkbox rows; completed styling; component and section full/partial/skipped derivation; unticked mobility is skipped; skipped mobility makes an otherwise full prescribed session partial; full mobility plus all other prescribed sections makes the session full; the feedback screen and accepted transaction both derive from the same execution-item evidence, with that result preceding the legacy component aggregate; the mobility component contains no optional text or accessibility wording; separately authored Optional Work stays outside the prescribed aggregate; the checklist branch contains no completion-choice chips; the saved canonical result preserves every item and one 1-5 effort score without inventing feel/soreness/reasons; all five choices fit one row and use Sam\'s very-easy/very-hard anchors; a performed session without a valid 1-5 score is refused; the partial result and skipped mobility evidence survive the existing act-then-force-quit/relaunch tape. FIRST RUN: that reload tape expected full while its own fixture had completed Strength and skipped Mobility; it red immediately and now requires partial. WHAT IT DOES NOT HOLD: Sam\'s physical iPhone, VoiceOver traversal order, or analytics/recommendations that consume the new per-section evidence.',
    },
  },
  {
