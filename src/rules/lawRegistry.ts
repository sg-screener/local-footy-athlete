...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    guard: {
      ...
      receipt: 'FOUNDING TEXT, REWRITTEN 2026-08-10 AFTER THE SAME PATTERN APPEARED TWICE IN ONE DAY — this is the strongest argument this project has produced for the law, and it belongs here rather than in a boundary footnote. **A BLOCKED INSTRUMENT DOES NOT HIDE ZERO DEFECTS. IT HIDES AN UNKNOWN NUMBER, AND THE COUNT IS ONLY KNOWABLE BY UNBLOCKING IT.** Run one: the rig\'s first complete pass found THREE defects, each invisible until the one before it was fixed — a harness fingerprinting state the architecture had stopped storing, a seed that never marked onboarding complete, and a profile the app would have refused from a real athlete. Run two, the same day: SIX more, in the same staircase — one day drawn where flows asserted seven; a card whose centre now navigates; a LogBox banner over the tab bar eating every tab tap; a false "legacy world" alarm on every healthy seeded boot; a relaunch with no launch purpose crashing the app on the pad; and a seeded world holding four game days in memory and one on disk. NINE defects from an instrument that had been carried as "dead, cost unknown" since 18 July. The other guards on this row (test:repo-law-guards: no flow names a missing file, no flow is unreachable) would have caught NONE of the nine. Also holding since 2026-08-10: `test:maestro-element-contract`, which proves every element name a flow taps is one product source can emit — mutation-proved, and it PRINTS its own two limits (existence is not reachability; a template hole absorbs a word). Still UNENFORCED because none of these cells proves the flows RAN.',
    },
  },
  {
