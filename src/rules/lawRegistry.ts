...
export const LAW_REGISTRY: readonly LawRow[] = [
  // ── (a) THE AGENTS.md LAWS ────────────────────────────────────────────────
  {
    ...
    law: 'An ordinary healthy Off-season week and its rolling readiness adjustment are authored through one canonical weekly compiler: the scheduler, specialist materialiser and connector each have that compiler as their only production caller; generation does not prebuild a rival initial plan or schedule the same week twice; readiness enters as a typed directive and no scheduler translator, generator or retained adapter independently re-resolves its window or rewrites its derived plan; refusal returns no partial plan; and visible prescriptions equal the accepted compiler-authored week.',
    ruledAt: 'docs/STATUS_TESTTRUTH.md "step 4: first canonical weekly compiler slice" and "step 5: readiness family"; Sam, 2026-08-27: "Build the first complete canonical weekly compiler slice" and "go for it" after choosing readiness as the next family.',
    guard: {
      state: 'guarded',
      by: 'test:canonical-weekly-compiler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-27 by seat `testtruth`; extended the same day with the first complete fact family. TWO OPTIONS COMPARED: keep readiness as a marker interpreted separately by the scheduler translator, generator and retained adapter, or pass one typed readiness directive into the compiler and return compiler-authored per-day dose policy plus the transformed plan. The latter landed because it deletes rival decisions instead of coordinating them. The 36-cell witness drives a healthy Off-season athlete through production onboarding, accepted-state transaction and visible resolver, then executes the real cooked-week athlete action and clears its exact fact. The governed in-window prescriptions reduce, pre-window rows do not change, and clearing restores the visible baseline byte-exact. Source ownership counts are stated in distinct production-source units: scheduleWeek 1/1, materialiseAuthoredSessions 1/1, scheduleToCoachingPlan 1/1, each canonicalWeeklyCompiler.ts; weekly-readiness rival authors 0 and derived-plan writers outside the compiler 0. The scheduler-input translator no longer reads readiness, the retained adapter no longer owns deloadPlanEntry or resolves the readiness window, and the generator consumes the typed compiler handover. MUTATIONS: removing the compiler scheduler call, compiler-to-adapter feasibility marker or per-day dose handover kills its named ownership cell. The release gate caught illness being transported as fatigue and therefore swallowed by the readiness slice; the typed readinessKind discriminator now keeps illness with its own family, and both the 121-cell readiness/illness law and 14-cell durable-horizon witness pass. LIVE RELEASE: 4/4 units green. NOT COVERED: illness, fixture, injury, travel and athlete-edit families have not moved into the compiler; global rival-author zero, full-year archetypes, pixels, simulator and physical iPhone remain outside this unit.',
    },
  },
  {
