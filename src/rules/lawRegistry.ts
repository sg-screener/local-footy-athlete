...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    ruledAt: 'docs/RULINGS_REGISTRY.md R-201, R-202, R-204, R-205, R-206, R-207 and R-244 — Sam supplied the original Week sheet, then consolidated all training and fixture management into one divider-free board.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-25 by seat `headeralign`; UPDATED FOR R-244 on 2026-08-26 by seat `pregamechoice`. TWO OPTIONS COMPARED: keep separate Bye/Move game/Add game commands beside a training manager, or make the board the one surface that manages both content kinds. The unified board landed because it removes duplicated management doors while preserving the canonical plan-change and fixture-transaction owners underneath. TEST FIRST: seven new Week-board cells failed for fixture drop precedence, draggable/removable games and the missing add chooser. AFTER: test:week-board is 75/75; the updated Week-sheet cell passes inside test:day-first-timeline at 55/56, whose one remaining red is the inherited mobility warm-up review binding. The guard requires exactly two top-level rows, no old game rows, no headings, exactly two divider-free options, both competitive phases in the add chooser, and canonical fixture routing for add/move/remove. NOT COVERED: physical-iPhone Release, VoiceOver speech, long-press fixture movement on glass and multiple fixtures on the same date.',
    },
  },
];
