...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The week list uses her one card shape and visual hierarchy for all seven days; today is the same card highlighted, its chevron opens the whole flat session with no nested accordions, and a real active-modifier list puts its compact line above the week.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "RESOLVED AND BUILT — 2026-08-11" + "SAM EYE PASS 2" + "SAM EYE PASS 3"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO). FOUNDING CASE: slice 5 was reported as taking her weekly structure, but the selected/today coordinate still switched into the old hero card and carried Start Session plus the change door. EYE PASSES 2 AND 3 CAUGHT THE GUARD WAS TOO SHALLOW: after the single head landed, 18pt dates, badges with a hidden 24pt line box, an invented GAME badge, a zero-modifier visual world and nested component accordions still differed materially from the prototype. WHAT THE CELLS HOLD NOW: all seven rows call one WeekDayCardHeader selected by SCREEN SHAPE; its date is 27pt; compact badge font AND line-height are pinned; Game Day adds no second badge; the one DayTimeline call chooses interactive Today versus flat Week; the flat branch renders every section and numbered row with no openParts gate, inner button, chevron, rail or icon; the shared modifier component selects the compact lime week treatment. WHAT THE TAPE HOLDS: it creates one equipment modifier through the athlete door, proves the week line, compares collapsed card heads, opens Wednesday once, proves flat rows exist and the inner part button does not, then reaches the remaining days. FIRST-RUN FINDINGS WERE NOT FIXED QUIETLY: off-screen assertion order, TODAY wrapping, XCUITest losing its UI, inherited badge line-height, an acted Monday left open, and a guessed accessory id where the projection carried strength were each reported and the full tape rerun after correction. WHAT IT DOES NOT HOLD: a multi-section flat session on the final device coordinate, the two-modifier plural on a device, prior/next-week Completed treatment, the deferred team-training badge, Sam\'s physical iPhone.',
    },
  },
  {
