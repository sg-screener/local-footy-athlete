...
export const LAW_REGISTRY: readonly LawRow[] = [
  // ── (a) THE AGENTS.md LAWS ────────────────────────────────────────────────
  {
    ...
    guard: {
      ...
      receipt: 'BORN GUARDED 2026-08-27 by seat `testtruth`; extended the same day through readiness, illness, fixtures, injury and availability. INJURY: raw onboarding injuries plus accepted facts are translated once into `CanonicalWeeklyInjuryState`; the compiler supplies that policy to power, both exposure contracts and conditioning, and returns its exercise-pool keys. The real action witness found and closed two further gaps before acceptance: Contract v2 was not receiving injury policy, and onboarding had not recorded the healthy selections temporary facts must preserve. A 7/10 knee now commits through the production door, carries squat/hinge and app-sprint restrictions into the accepted contract, keeps unaffected work, and clearing the exact episode restores the visible week byte-for-byte. Weekly injury rival authors in the guarded production path: 0. Mutations remove both compiler handoffs. FIXTURE: accepted calendar availability is translated once into semantic fixture state; the compiler owns scheduling, connector fields, accepted identity and the final specialist call. A real in-season cold start moves a Saturday game onto occupied Wednesday through the production transaction, observes Game Day replace Wednesday training and exactly one Wednesday fixture anchor, then moves it back and restores the visible week byte-for-byte. AVAILABILITY OPTIONS COMPARED: retain travel, club closure, explicit unavailability and dated-equipment interpretation across the scheduler, fixture translator, profile translator and generator, or translate all accepted availability facts once into `CanonicalWeeklyAvailabilityState` and let the compiler project schedule, coaching, conditioning and composition. The semantic route landed because it deletes the disagreement class. Weekly-availability rival authors in the guarded production path: 0. A real trip witness commits travel plus reduced equipment through the production doors, removes club/game anchors only inside the span, keeps the athlete\'s own work, changes only governed dated-kit output, and resolving both exact facts restores the visible week byte-for-byte. Separate cells prove club closure removes team nights but preserves the athlete-entered game, and explicit unavailability blocks only its own date. Mutations remove both the typed availability handoff and compiler projection. Distinct source units: scheduler/materialiser/connector remain 1 caller each in canonicalWeeklyCompiler.ts; weekly-readiness, weekly-illness, weekly-injury, weekly-fixture and weekly-availability rival authors are each 0; readiness derived-plan writers outside compiler are 0; final fixture specialist production callers are 1 and it is the compiler. LIVE: compiler 125/125; scheduler group green (102/102 scheduler, 11/11 generated fixtures, 10/10 travel zero-equipment, 9/9 Off-season continuity, 20/20 spare-day options); dated equipment 3/3; generated week 36/36; release 4/4. The old availability/conditioning diagnostics named in the test-truth census remain `rewrite_test` and do not direct product changes. NOT COVERED: athlete-edit and scheduled-deload families; global rival-author zero, full-year archetypes, pixels, simulator and physical iPhone.',
    },
  },
  {
