...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'Welcome uses Sam\'s approved BUILT FOR FOOTY copy: one complete program; Your Plan / Built as one program / Strength, speed, conditioning and recovery; Your Week / Footy comes first; Your Progress / See how you\'re tracking; Build my program; and Takes about 3 minutes. All three eyebrow labels render through one shared 2px gap before the main heading. The retired Everything works together heading, longer phase-specific sentence, earlier card copy and TRAIN FOR title are absent.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-148 + R-150 + R-153 — Sam supplied the complete Welcome copy, restored the labels with compact shared spacing and the BUILT FOR FOOTY title, then shortened the first card and replaced its heading. R-150 supersedes R-149.',
    guard: {
      state: 'guarded',
      by: 'test:onboarding-presentation',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `welcomecopy`; UPDATED UNDER R-149 by `welcomeeyebrows`; UPDATED UNDER R-150 by `welcomecompact`; UPDATED UNDER R-153 by `onboardingtype`. The onboarding presentation cells require every approved title, label, description, CTA and timing line; reject the retired card copy, old first-card heading, long phase-specific sentence and TRAIN FOR title; require feature.label to render; and anchor the one featureLabelWrap style at marginBottom 2 so all cards share the same compact gap. R-150 TEST-FIRST LIVENESS: the three revised cells were the suite\'s only reds against the label-free TRAIN FOR screen (67/70); restoring the shared labels and BUILT FOR title returned it to 70/70. R-153 TEST-FIRST LIVENESS: the two revised Welcome-copy cells were the suite\'s only reds against the old first card (81/83); the new short card returned it to 83/83. NOT COVERED: simulator pixels, small-screen wrapping and the next physical-phone Release.',
    },
  },
  {
