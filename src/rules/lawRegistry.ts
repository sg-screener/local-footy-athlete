...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    guard: {
      ...
      by: 'test:exercise-intake',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `sleeperstretch`. The exercise-intake suite binds the source intake to catalogue metadata, dose, cues, video, empty equipment requirement, injury ratings, upper Movement Prep, Mobility, Recovery, manual Add/Swap, Primer exclusion and G-1 eligibility. Red-first, mutation and final receipts plus NOT COVERED: docs/STATUS_SLEEPERSTRETCH.md.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'R-306, refined by R-309: annual workload reporting preserves intentional coaching differences rather than manufacturing equality. Required male and female performance foundations are now shared unless an explicit coaching rule differs; Gunshow versus Primer is the main optional Friday difference. Reporting partitions final main-session rows into main strength, power, core, prehab, mobility/recovery and conditioning, reports optional sessions as an orthogonal subtotal, and makes total athlete-visible rows equal main-session rows plus Movement Prep.',
  ...
  },
  {
    ...
    guard: {
      state: 'guarded',
      by: 'test:programming-hierarchy + test:mobility-accessory-doors + test:weekly-scheduler + test:team-night-content + test:canonical-weekly-compiler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-01 by seat `yearrepair`; STRENGTHENED AFTER FINAL PDF REVIEW. The first focused final-composition run was 19 passing / 5 failing: required split tables still carried automatic arm/pump work and neither sex was guaranteed the four football categories. One typed week-level foundation classifier now allocates only missing categories inside the canonical composer and removes an earlier robustness row when later ordinary lower work already supplies it. The current boundary is 29/29 across male/female in-season final sessions, 13 block histories, restricted kit, injury adjustment and exact restart signatures. Replacing the male robustness seats with triceps killed four named foundation cells. The complete-week pruning first left an interim Reverse Nordic Curl choice in the audit stream; the chained selection-trace gate went red because the selected identity was absent from final rows. Final traces now publish only strength choices surviving on that exact day and the trace is 10/10. The Gunshow boundary is 33/33 across 366 dated builds, excluding the slow negative, requiring three reps in reserve and reducing 2+2+2 to 1+1+1 at 4-7/10 upper injury before pausing at 8-10/10; bypassing that injury dose killed both the direct builder and final generated-week cell. The scheduler is 128/128, including one-game gender, practice match, multi-game, bye, scheduled deload and low-readiness coordinates; removing the reduced-week predicate kills its two reduced-week cells. Final PDF review then exposed a stale system-generated Gunshow/Primer after fixture Remove/Move. The canonical fixture journey began with both new final-state cells red, then proved male and female removal, move-back restoration, restart and exact Undo after fixture-relative offers joined the existing re-derivation boundary; its ownership cells exclude athlete-added offers. The first fresh year reduced seven invalid dates per sex to one and exposed the second compiler path: a Tuesday knee source fact resurrected the old Friday offer in an already accepted bye. A shared accepted-fixture rebase now holds every later source-fact compilation; male and female overlap cells cover the exact sequence and restart, while athlete-added offers remain accepted. The live accumulated team-night tape remains 32/32 and preserves exact content through save, restart and Undo for both sexes. Full receipt and NOT COVERED: docs/STATUS_YEARREPAIR.md.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'R-311 corrected: only run-modality acceleration or maximum-velocity work with full recovery may be written as automatic true Speed. Bike/Air Bike acceleration and incomplete-recovery repeat sprint remain conditioning. Off-season weeks 1-4 contain zero automatic Speed; weeks 5-8 alternate acceleration and progressive build-ups, then small Fly 20/Fly 30 exposures follow. In Pre-season and In-season, team training may supply acceleration but never silently proves top speed; an app top-up is running top-speed only when availability, fixture proximity, accumulated anchor load and reduced-week state permit it.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-311',
    guard: {
      state: 'guarded',
      by: 'test:offseason-deload-conditioning + test:weekly-scheduler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-01 by seat `yearrepair`; CORRECTED 2026-09-01 by seat `elevenfix`. Sam withdrew the newer Week-1 Speed behaviour. The canonical early and mid Off-season contracts now both own a zero target, the old sprint-floor supersession was removed, and the cold-onboarding acceptance gate proves all four final opening weeks contain zero running-Speed exposures. The 12/12 generated-week gate proves Speed begins in week 5 and later running-only quality rules remain. Both gates remain nested in chained `test:offseason-deload-conditioning`; the eleven-item boundary repeats the cold acceptance. NOT COVERED: physical iPhone acceptance.',
    },
  },
  {
  ...
  },
  {
    ...
    guard: {
      ...
      by: 'test:full-body-balance',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-01 by seat `weeklyselector`. TEST FIRST: 200 final composer-declared athlete-weeks across 50 real generated profile worlds produced 12 repeated exact identities, 52 repeated real anchor-family findings and 44 dedicated Lower Hinge findings containing single-leg knee work. TWO OPTIONS COMPARED: clean final sessions after selection, or give the existing whole-week composer one selector that judges canonical delivered identities and real catalogue movement classification before authorship. The selection-time owner landed and later source-fact remainder/injury compilation consumes the same state. Its focused boundary is 13/13 across required squat/hinge/upper examples, dedicated-day ownership, the complete fallback ladder, Mobility/Prehab semantics, 50 generated worlds, a bodyweight generation, mid-week delivered history, athlete additions and exact restart. The complete 104-athlete-week working diagnostic has zero exact, real-family or dedicated-day findings and 104/104 successful restart reconstructions. AMENDED 2026-09-02 by seat `fivebridge`: red-first exposed Dumbbell Kickback repeated in every male Gunshow week plus Rotational Medicine-Ball Slam and Box Jumps repeated in every female Primer week. The same selector now reaches all automatic gym families, typed authorship survives rebuild/restart, and athlete-added rows remain outside it. Focused, fixture repair, mutation and NOT COVERED receipt: docs/STATUS_FIVEBRIDGE.md. Original annual receipt: docs/STATUS_WEEKLYSELECTOR.md.',
    },
  },
  {
    ...
    guard: {
      ...
      by: 'test:full-body-balance',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `compoundcap`. TEST FIRST: the seven-cell focused tape was 0/7 because canonical classification, completeness, session state, fallback and the final cap audit did not exist; a real full-upper generation delivered Bench Press, Chest-Supported DB Row, Seated DB Press, Pull-Ups, Push-ups and Lat Pulldown as six compounds. TWO OPTIONS COMPARED: trim final rows after their choices had already changed weekly and durable state, or classify once in exercise metadata and refuse illegal support at the existing selection boundary. The selection-time owner landed. The canonical governed scope contains 84 distinct identities: 49 compounds and 35 isolations, with zero missing. AFTER: the original focused tape was 7/7; the founding upper case delivers four compounds followed by Tricep Pushdown and Face Pull isolation support. Removing Back Squat classification and injecting a fifth automatic compound each kill their named cell, while an athlete-authored fifth remains legal. CORRECTED GROUPING: the tape is 8/8 after adding the exact 13-name Lower Accessories — Isolation contract derived from existing pools. Temporarily changing Copenhagen Plank (Half) to compound made it 5/8 and killed the corrected grouping, exact boundary and classification split cells; restoration returned 8/8. Focused compiler, restart and NOT COVERED receipts: docs/STATUS_COMPOUNDCAP.md.',
    },
  },
  {
    id: 'LAW-typed-exercise-movement-planes',
    law: 'R-327: Sam-supplied exercise movement planes live in one typed catalogue equal to the Exercise Master workbook. Unlisted identities remain unanswered questions and are never inferred. Plane preservation and missing-useful-plane tie-breaking run only inside the legal equal cohort after injury, equipment, experience, season/game proximity, purpose/family and weekly limits; accepted current-block history still restores first. Weekly and annual checks require meaningful lower-body primary-frontal work plus at least one meaningful gym transverse/multiplanar strength, prehab, power or trunk exercise; a secondary transverse plane counts and Team Training never earns gym credit. Athletic COD exposure remains separately typed: Off-season Weeks 1–4 have no target, then each complete healthy/running-eligible rolling 14-day window requires one qualifying exposure. Team Training and the approved typed work count; straight-line acceleration does not. The existing fortnightly late-off-season/Christmas COD exchange may satisfy the window without adding a day or conditioning credit, while injury, illness, travel, deload, readiness, running and game-proximity safety outrank it. Generation and annual audit consume the same typed rule. The trunk rotation/anti-rotation check remains softer at 7–14 days. No equal-percentage target or athlete-facing plane label is introduced.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-327',
    guard: {
      state: 'guarded',
      by: 'test:full-body-balance',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `movementplane`; rolling-window correction guarded 2026-09-02 by seat `fivebridge`. TEST FIRST: the focused command could not load either the plane catalogue or programming audit; the correction then failed its two new cells because the shared phase/window function did not exist. TWO OPTIONS COMPARED: keep a weekly audit exemption beside the scheduler, or give generation and audit one typed phase/window/safety rule. The shared rule landed. The focused tape pins Sam\'s exact named set, metadata invariants, workbook equality in both directions, the complete unanswered-question list, same-plane substitution, accepted-history priority, lower-frontal exclusions, typed athletic credit, Off-season Weeks 1–4, complete rolling 14-day windows, every safety override and the soft trunk window. The existing selector owns the tie-break and the existing annual audit owns findings; no screen reads the fields. Final tests, mutation and NOT COVERED receipt: docs/STATUS_MOVEMENTPLANE.md and docs/STATUS_FIVEBRIDGE.md.',
    },
  },
  {
  ...
  {
    id: 'LAW-lower-hinge-purpose-relevant-support',
    law: 'R-336: a dedicated Lower Hinge automatically uses only hinge-purpose work: the main hinge, single-leg hip, hamstring, glute and back-extension/posterior-chain work and the posterior-chain prehab that supports it, with calf or general football-robustness work only after legal unused posterior-chain options are exhausted. Squats, lunges, step-ups and step-downs, Bulgarian split squats, leg extensions, reverse Nordics, Spanish squat work and other knee-dominant exercises are refused on Lower Hinge whether offered as strength or prehab, judged by typed movement slots and signed primary muscles, never by name. The useful-session minimum, one automatic appearance per exercise per week, the four-compound ceiling, equipment/injury/game safety, athlete-added freedom and Lower Squat\'s separate ownership are unchanged.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-336',
    guard: {
      state: 'guarded',
      by: 'test:full-body-balance',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `hingecod` from exact checkpoint 8d68d6dd. RED FIRST: the checkpoint\'s own accumulated 34-week male/female journeys carried 27 knee-dominant automatic rows on dedicated Lower Hinge days (Slant Board Step-Down on the 12 sessions the annual audit named plus Standing Knee Extension, Reverse Nordic Curl and Spanish Squat Hold on 15 more), all delivered through the football-robustness seat, because the weekly selector judged dedicated-day ownership only for the strength route and only by the single-leg-knee slot, so a prehab-authored lunge and quad-led isolation passed. TWO OPTIONS COMPARED: filter named exercises or the named weeks after composition, or give the one weekly selector a typed purpose rule shared by both routes and every rebuild path. The typed rule landed: real movement slot (squat / single-leg knee) or signed primary muscle (Quads / Knee) refuses the row, hinge-family slots or Hamstrings / Glutes / Low back name posterior-chain support, the composer applies weekly legality before choosing a robustness category and opens the posterior bench first on hinge days, and weekly frontal completion asks with the composed day shape. Focused guard cells: ten knee-dominant strength and prehab identities refused by conflict, live selector and final audit; thirteen hinge-purpose and general identities admitted with Lower Squat unchanged; the fallback bench skips knee-dominant prehab and spends posterior work before calf; every dedicated Lower Hinge across 50 generated worlds carries zero knee-dominant automatic rows, no exact repeat and a met or typed useful-session receipt. MUTATION: removing the hinge branch of the conflict rule reds three cells (18/21). AFTER, on regenerated 34-week journeys per athlete: 31 Lower Hinge days each, zero knee-dominant rows, every useful-session receipt met or typed, zero exact/main-family repeats, zero dedicated-ownership findings, 34/34 successful restarts. Chain neighbours unchanged: composer 6/6, generated-week 30/30, minimum-useful 7/7, movement planes 13/13, budget 5/5, variety 5/5. Inherited reds reproduced identically at the untouched checkpoint and not repaired: compound-session-selection 7/8, slot-coverage 81/84, the typecheck gate\'s five pre-existing file errors. Receipt: docs/STATUS_HINGECOD.md.',
    },
  },
  {
    id: 'LAW-final-year-audit-credits-typed-athletic-exposure',
    law: 'R-336: the final-year strength-budget analyzer credits athletic transverse exposure only from typed identity: the club-training role, the authored conditioning quality reached through a template name or through the typed section rows a combined session renders (the R-331 Change of Direction session: Speed warm-up, Low-Intensity Deceleration Drills, 45-Degree Cut Reps, Up-Back Shuttle), and the power pool\'s declared exposure. Display titles never decide. COD programming, cadence, placement and prescriptions are unchanged.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-336',
    guard: {
      state: 'guarded',
      by: 'test:programming-hierarchy',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `hingecod`. RED FIRST: on the checkpoint\'s accumulated male journey the analyzer reported missing_athletic_transverse for off-season weeks 6 and 7 although the combined Change of Direction session sat in weeks 5 and 7, because it resolved only a standalone template name per row and the combined session renders its typed sections as rows. TWO OPTIONS COMPARED: add a new export field naming the selected template, or resolve the existing typed section identity already exported per row back to its authored template. The section resolver landed in the conditioning-selection owner, the exposure vocabulary in the movement-plane owner, and one canonical day projection in the audit-projection owner that the analyzer now calls. Guard cells: the four-row combined session earns exactly cod_decel, a single section still earns it while the day\'s display label is ignored, and club, power and plain aerobic rows credit only what they are. MUTATION: removing the typed section recognition reds the projection cell and the analyzer again reports male weeks 6 and 7. AFTER: zero missing_athletic_transverse findings across both regenerated 34-week journeys; the same analyzer on the untouched checkpoint\'s export also credits them, so no export change was needed. Receipt: docs/STATUS_HINGECOD.md.',
    },
  },
  {
    id: 'LAW-week-budgeted-in-stimuli',
    law: 'R-337: the phase energy-system target is a list of stimuli (from normal Off-season week 5 and Pre-season without a fixture: Speed, one hard, one tempo, at most one easy), not a count of days. A planned automatic Speed day is one of those stimuli and the metabolic budget is what remains after it. With enough legal receivers Speed owns its day and carries no metabolic block; it shares a day with tempo or intervals only when receivers are short. A stimulus goes onto an existing training day before opening a new one. R-303\'s four-day maximum, the no-three-consecutive rule, WC-143, P15, club top-ups, safety, readiness and every strength rule are unchanged. A strength session whose only energy-system component is Speed keeps its session name.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-337',
    guard: {
      state: 'guarded',
      by: 'test:weekly-scheduler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `hingecod`. RED FIRST: on the checkpoint\'s accumulated 34-week male and female journeys every normal Off-season build week (5-7) used five training days — Speed rode Saturday\'s tempo and ticked one day, the day-count budget found a gap that was not real, the top-up could use only free days, Friday would have made three consecutive energy days, so a Sunday bike sprint was added. Three new scheduler cells (six-day build: four stimuli on four distinct days and an empty Sunday; the Speed day carries no metabolic work; no accepted no-club matrix coordinate stacks Speed) were red at 128/131. TWO OPTIONS COMPARED: patch the Sunday top-up and the Saturday stack separately, or budget stimuli — a typed overlay field says whether Speed sits inside the target, the metabolic budget subtracts it, the receiver selection excludes the Speed day when room exists and retries with stacking only when spacing cannot seat the budget, the stale prefer-a-receiver-after-Speed score is silent when Speed owns its day, and existing training days are preferred over new ones before even spacing. The stimulus budget landed. AFTER: weekly scheduler 131/131, fresh Speed 14/14, persistence 2/2, fortnightly COD 19/19, scheduler fixtures 11/11, off-season continuity 9/9, spare-day options 22/22, generated-week 30/30, final composition 6/6, minimum-useful 7/7, selection 21/21, planes 13/13, template equality 170/170, weekly-plan display green with the strength-name cell. Regenerated 34-week journeys per athlete: weeks 5-7 now four training days (Mon lower + easy off-feet, Tue upper + hard, Thu hinge + tempo, Sat upper + Speed only), zero Sunday energy days, zero stacked Speed days, 31 Lower Hinge days with zero knee-dominant rows, zero repeats, zero dedicated-ownership or required plane findings, 34/34 restarts each. MUTATION: turning the normal-build overlay\'s speedInsideConditioningTarget off reds the three cells (128/131); file restored byte-identical. Inherited reds reproduced identically at the untouched checkpoint: travel-zero-equipment 8/10, session-classification 48/50, canonical-weekly-compiler one duplicate-row cell, compound-session-selection 7/8, slot-coverage 81/84, the typecheck gate\'s five pre-existing files. Receipt: docs/STATUS_HINGECOD.md.',
    },
  },
  {
  ...
  },
  {
    ...
    guard: {
      state: 'guarded',
      by: 'test:week-checker-allowances',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-09-02 by seat `hingecod`. RED FIRST at 4712a17a: 19/32 — the home-kit knee weeks carried planner_selected_target_miss:conditioning 4 vs 3 with no reduction and an empty "Aerobic Conditioning" card; a practice match on a five-day home week left the second upper day with no main (3 vs 2); an added Monday game left the one full-body day with Leg Press, RDLs, Bench Press and a band pull-apart (no pull main); a game moved onto a club night let the app sprint make four high-speed exposures (maximum 3). After: 32/32 on the same worlds through the real doors (injury report, fixture add/move) plus the budget and coverage-order units. Receipt: docs/STATUS_HINGECOD.md.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'The authored normal-gym Gunshow is ordinarily 2 biceps + 2 triceps + 2 shoulders at 2-3 sets, draws only from the exact signed 8/7/8 pools, and never selects a forbidden same-pattern pair. Automatic content excludes slow-eccentric Chin-Up Negatives, stays three reps from failure, and follows the upper-body injury bands: 1+1+1 at 4-7/10 and paused at 8-10/10.',
    ...
    guard: {
      ...
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-21; UPDATED FOR R-309 on 2026-09-01 by seat `yearrepair`. A0 pins the exact three ordered pools. A1-A4 hold the ordinary 2+2+2 normal-gym shape, no cross-family top-up and authored 2-3 sets. A1b walks 366 dated automatic Gunshows, excludes the named slow-eccentric identity and requires three reps in reserve. A1c drives the real injury-aware builder: 5/10 yields three rows and 8/10 yields zero. A6 pins the forbidden pairs and requires every automatic-eligible signed candidate to rotate. Bypassing the injury dose makes A1c and the generated final-week injury cell red; the earlier pair-rule mutation still kills A6.',
    },
  },
  {
  ...
  },
  {
    ...
    guard: {
      ...
      by: 'test:weekly-scheduler',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-26 by seat `gymfill`; UPDATED FOR P03 AND R-309 by seat `yearrepair`. The original existing-owner design remains: the scheduler owns the gendered G-1 offer and the post-acceptance top-up owner owns Mobility. The current final-generation tape is 22/22 across three-through-six gym days, both sexes, no-game Pre-season, Off-season, equipment-free Mobility and a moderate-injury reduced Gunshow. The direct scheduler is 125/125 and now pins one-game In-season, Pre-season practice-match, multi-game, bye, scheduled-deload and low-readiness coordinates. Removing the reduced-week predicate kills exactly the scheduled-deload and low-readiness cells. Manual Add remains outside this automatic placement gate. NOT COVERED: physical-iPhone Release acceptance, every injury/equipment combination, athlete-added multi-game offers, and long-term adherence to optional sessions.',
    },
  },
  {
  ...
  },
  {
    ...
    law: 'R-275/R-218f: every fatigue answer is one dated fact. Bit tired records only; Pretty flat slightly reduces that date; Totally cooked removes that date session. Any two consecutive reported calendar dates in any tier order deload from the second date through Sunday and tell the athlete “That’s two tired days in a row.” Same-date repeats do not form a streak, a gap breaks it, cooked remains rest, Clear removes the report from derivation and expiry preserves factual history. The missed-session notice offers Yes log, No skip and No move. Move opens the existing Week board, where only that prompted past unlogged item may move to today/future without swapping future content into the past; directly dragging that named box is itself the explicit move answer. The board locks continuously to the nearest measured day/slot, prefers an available future slot for a past source, displays magnetic target feedback, and retains occupied-slot swaps for ordinary future drags. The Week editor has no redundant Sessions / games heading; real game-picker instructions remain, an unchanged editor can exit through Day / Week, and a changed editor still requires Save changes.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-275 and R-218f — Sam settled the fatigue combinations, cooked rest, exact notice and past unlogged move route on 2026-08-29/30, removed the redundant Week-editor heading on 2026-08-31, and required magnetic whole-day drag targets plus the direct Monday-to-Wednesday missed move on 2026-09-01.',
    guard: {
      state: 'guarded',
  ...
  { id: 'LAW-weekly-leg-coverage-and-return',
    ...
    guard: {state:'guarded',by:'test:leg-programming',chainStatus:'in_chain',
      receipt:'Owner legprogram. Red-first RDL coverage failure; isolated real onboarding, eight-week progression, injury/clear, equipment changes and reopen; in-memory fault controls. Existing annual compiler audit includes final weekly coverage. Returning field display is held across both stages and section kinds, including actual repetitions, registered copy, template identity and invalid modality rewrites. Results and remaining failures are recorded in docs/STATUS_LEGPROGRAM.md; physical device acceptance remains outstanding. Repairs 2026-09-09 (seat yearfix, three-day book review): the Primer\'s authored Pogo Hops is typed power so the one exposure filter strips it under a knee; one adductor isometric per session (the groin pool\'s holds are one variation family for the composer, selector, warm-up flow and doors); the frontal-plane completion and the prehab reducer consume the dated deload policy. Injected faults untype-primer-pogo, adductor-holds-not-one-family, drop-frontal-completion-dose and ignore-dated-policy-for-prehab-reduction are caught.'},
  },
  { id: 'LAW-inseason-nordic-minimum',
    law: 'R-394: in-season, a couple of sets of Nordics every week or at minimum every second week; hamstring curls are fine but do not replace Nordics. The weekly leg coverage answers a typed Nordic first in-season, credits an existing one, keeps R-312\'s dose band, game-proximity and experience rules, and halves the dose under the dated reduced-week policy. Pre-season and off-season keep R-393\'s Nordic-or-curl pair.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-394',
    guard: { state: 'guarded', by: 'test:leg-programming', chainStatus: 'in_chain', receipt: 'Owner yearfix. Domain cases for in-season addition, credit, pre-season silence and the G-1 exclusion; injected fault nordic-satisfied-by-curl caught. Required annual weekly check weekly_inseason_nordic with verdict controls in test:compiler-year. Receipts: docs/STATUS_YEARFIX.md.' },
  },
  { id: 'LAW-inseason-harder-than-flush',
    law: 'R-395: in-season conditioning needs no big aerobic dose, but one Monday/Wednesday-style gym session off club training, outside the fixture window and with a machine on the day, may be harder than a flush. The scheduler places one moderate off-feet finisher outside the game-week budget; the game-week contract permits exactly that one extra session for a two-or-more-night club athlete. Reduced weeks, weeks already carrying app metabolic or Speed work, and athletes with no eligible day receive nothing more.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-395',
    guard: { state: 'guarded', by: 'test:leg-programming', chainStatus: 'in_chain', receipt: 'Owner yearfix. Scheduler cases for Saturday and Sunday fixtures, deload, low readiness, no machine, no club, no eligible day and pre-season; injected fault drop-inseason-moderate-conditioning caught. Required annual weekly check weekly_inseason_conditioning with verdict controls in test:compiler-year. Receipts: docs/STATUS_YEARFIX.md.' },
  },
