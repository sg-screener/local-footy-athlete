...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The Day Need to make a change card exposes Tired, Sick and Injured as its only direct status controls, followed by one visually separate Edit day doorway into the deterministic Add, Move and Remove menu. There is no separate Want to change something link and no dedicated whole-session Swap action on Day or Week. Move may trade ordinary occupied sessions, but a Team Training destination is a combine: Team Training stays on its day and is never swapped with strength work. The open-workout hub remains Equipment, Injury and Add because row-level quick actions own exercise swaps and removals.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-181 and R-194 — Sam first consolidated Day edits, then separated athlete status from day scheduling behind one Edit day doorway; Strength ↔ Team Training swaps remain forbidden.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + test:move-scoping',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED FOR R-194 the same day. TWO OPTIONS COMPARED: keep six equal-weight chips and rely on labels to distinguish status from scheduling, or retain three immediate status controls and route one separate Edit day doorway into the already capability-driven Add/Move/Remove menu. The second landed because it removes mixed concepts without creating a second mutation system. TEST FIRST: the revised `test:session-change-hub` produced five named reds against the six-chip implementation; after the shared hub gained its separate edit doorway and Home removed the three plan chips it returned 60/60. The extra cell was born after the first simulator run caught the optional action silently defaulting to Add; removing that default made Edit day land on the menu. `test:day-first-timeline` pins the three-chip set, signed Edit day copy, deleted legacy link and menu ingress; it remains at its exact inherited 54/2 baseline. `test:move-scoping` continues to own Strength -> Team Training combine behaviour at its inherited 17/1 baseline. `test:signed-copy-extraction` is 7/7 and `test:compile` remains at the concurrent 483-error / 60-worsened-pair baseline, naming none of this slice\'s changed product files. LIVE SIMULATOR, NO SCREENSHOTS: Tired, Sick and Injured were reachable, the direct Add id was absent, Edit day was present, and its tap rendered Add, Move and Remove together. NOT COVERED: physical-iPhone Release, VoiceOver order and a real committed multi-edit sequence.',
    },
  },
];
