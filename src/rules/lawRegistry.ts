...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'Conditioning keeps work-to-rest ratios internal. Athlete-facing titles use plain duration words, Work and Recovery state explicit times, and the projected count agrees with the concrete count materialised on the row. A merged workbook row may retain alternatives internally, but its athlete projection selects one already-authored branch and never shows “or” or “variant” in Work, Recovery or count lines. One conditioning option renders directly as its row; only two or more genuine equivalents use a Choose one control. A standalone row does not print a second rest line beneath its structured Recovery line. The historical Classic 4×4 identity displays as 4×4 VO₂ Max, prescribes three minutes of complete rest with no jogging, and cues only “Choose a pace you can repeat across all 4 rounds.” No conditioning card renders a Heart rate line. One shared renderer bolds Work, Recovery, the applicable count label and Intensity across standalone and genuine multi-option rows, with readable line height and a small gap below the title.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-238, R-239 and R-240 — Sam distinguished 30:30 seconds from 1:1 minutes, rejected mixed titles, duplicate parts and menu-like prescriptions, corrected the 4×4 VO₂ Max name and complete-rest prescription, then required concise cards with no heart-rate line and bold structured labels.',
    guard: {
      state: 'guarded',
      by: 'test:conditioning-templates + test:session-template',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-26 by seat `condfix`. FOUNDING CASE: a real conditioning-showcase generation held one Classic 4×4 row while buildSessionTemplate wrapped it in a one-option card titled Hard Intervals, making one prescription look like two parts. The signed workbook also carried workToRest 1:1 beside explicit 30 s / 30 s fields and four merged rows projected mutually exclusive alternatives. TWO OPTIONS COMPARED: rewrite the signed workbook and its typed mirror, or retain internal physiology and make the existing shared athlete projection choose one plain title and one already-authored branch. The projection design landed; placement, fatigue and progression are untouched. TEST FIRST: session-template began 77/78 with the one-option row still emitted as choice:1; the conditioning guard first stopped on the missing display-title owner. Sam then caught the dated source\'s wrong Classic 4×4 / easy-jog card on glass; the exact 4×4 VO₂ Max / complete-rest cell began red at 101/103. His next glass review rejected the long HR-plus-cue card: the global no-HR cell and exact concise cue began red, while three new renderer cells began red for the absent shared emphasis owner. AFTER: conditioning-templates is 106/106 and session-template 85/85. The 55-template sweep requires plain non-colon titles, no or/variant in Work/Recovery/count lines, no Heart rate line, exact 30 s work plus 30 s recovery, exact 4×4 VO₂ Max title plus complete rest and concise cue, and the one-minute flush title/times; the session guard requires one option to render directly while retaining the two-option picker, forbids a second rest renderer, and requires both row paths to use the one structured-copy component that bolds Work, Recovery, count and Intensity with readable line height and title separation. C14 drives all seven requestable categories across twelve mini-cycles: each advances beyond its first eligible template and remains stable inside a block. MUTATION/LIVENESS: restoring the literal Flush Intervals 1:1 title killed two named C13 cells; forcing the real selector to index zero killed C14 and printed all seven stuck categories; the absent emphasis owner is the test-first 80/83 failure; restoring the old single-option wrapper is the original 77/78 test-first failure. The real iOS Simulator generation now shows one direct 4×4 VO₂ Max row with bold, vertically separated Work, Recovery, Rounds and Intensity, three minutes complete rest, the one-sentence cue, no generic wrapper, no easy jog and no Heart rate. NOT COVERED: physical-iPhone Release, every persisted legacy title, VoiceOver reading order and athlete choice among genuinely multi-option blocks.',
    },
  },
  {
