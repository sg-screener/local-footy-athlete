...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The week list uses her one card shape and visual hierarchy for all seven days; today is the same card highlighted, opening changes only the details, and a real active-modifier list puts its compact line above the week.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "RESOLVED AND BUILT — 2026-08-11" + "SAM EYE PASS 2 — THE STRUCTURE WAS RIGHT AND THE PROPORTIONS WERE NOT"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO). FOUNDING CASE: slice 5 was reported as taking her weekly structure, but the selected/today coordinate still switched into the old hero card and carried Start Session plus the change door. SAM EYE PASS 2 CAUGHT THE GUARD WAS TOO SHALLOW: after the single head landed, 18pt dates, badges with a hidden 24pt line box, an invented GAME badge and a zero-modifier visual world still differed materially from the prototype. WHAT THE CELLS HOLD NOW: all seven rows call one WeekDayCardHeader selected by SCREEN SHAPE; its date is 27pt; week category/Today badges select compact sizes whose font AND line-height are pinned; Game Day adds no second badge; week expansion is details-only; the one shared modifier component selects the compact lime week treatment and reads registered singular/plural copy. WHAT THE TAPE HOLDS: it begins with the honest zero state, creates one equipment modifier through the athlete door, proves the week line appears, compares the collapsed card heads, opens Wednesday without importing day controls, then reaches the remaining days. FIRST-RUN FINDINGS WERE NOT FIXED QUIETLY: off-screen assertion order, TODAY wrapping, the XCUITest invalid-element instrument failure, inherited badge line-height, and an acted Monday left open were each reported and the full tape rerun after correction. WHAT IT DOES NOT HOLD: the two-modifier plural on a device, prior/next-week Completed treatment, the deferred team-training badge, Sam\'s physical iPhone.',
    },
  },
  {
