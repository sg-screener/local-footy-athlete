...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The accepted new Program template uses a large Day / Week shape toggle; Day renders no week-date navigation, Week renders one legible previous/range/next row directly below it with 40pt controls and 13pt range type, and changing or returning weeks leaves every session collapsed.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "SAM EYE PASS 6 — WEEK NAVIGATION BELONGS TO WEEK"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO), UPDATED WITH SAM\'S 2026-08-11 DAY / WEEK AND WEEK-NAV EYE PASSES. FOUNDING CASE: the accepted day template had no week browsing, but the implementation mounted a large previous/range/next bar before the shape toggle so both shapes inherited it. LATEST FINDING: after the Day / Week control was corrected, the row beneath it still used 28pt controls, 13pt chevrons and 11pt range type on the physical phone. WHAT THE CELLS HOLD: visible and accessibility copy both say Day / Week; the shared shell is 280pt wide, each half has a 44pt target, and the label is 15pt; the navigator is ordered after the toggle and before Week content, mounts only when !dayFirst, preserves previous/current/next stable doors, contains neither IconButton nor Badge, and pins 40pt previous/current/next geometry, 17pt chevrons and 13/18pt range type. The template has one local expandedWeekIdx; Day reads todayIdx directly; every date and shape transition clears local expansion. FIRST RUN: the old guard red because it explicitly required the rejected 28pt/11pt proportions, so it was rewritten to hold the accepted size rather than softened. WHAT THE TAPE HOLDS: all navigator ids are absent on Day and present after Week is selected; previous and next move adjacent, return to this week, and leave detail collapsed. WHAT IT DOES NOT HOLD: Sam\'s physical iPhone, the old/classic template, adjacent-week Completed styling, or dynamic-type scaling.',
    },
  },
  {
