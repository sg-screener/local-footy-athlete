...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The Day status card asks “Not feeling 100%?” and explains “Tell us what’s changed and we’ll adjust your training.” It exposes Tired, Sick and Injured as its only direct controls and owns no scheduling doorway. A programmed non-game session card exposes one compact three-dot plan-options control beside its tier badge; the visible control stays small while its invisible physical tap target is at least 44 by 44 points, and it opens the deterministic Add, Move and Remove menu. A rest or Team Training-only day, where there is no programmed-session card to own the dots, retains a direct Add a session doorway. Game Day retains its specialised controls. The active-workout Equipment/Injury/Add hub keeps its own “Need to make a change?” heading. There is no separate Want to change something link and no dedicated whole-session Swap action on Day or Week. Move may trade ordinary occupied sessions, but a Team Training destination is a combine: Team Training stays on its day and is never swapped with strength work. The open-workout hub remains Equipment, Injury and Add because row-level quick actions own exercise swaps and removals.',
    ruledAt: 'docs/RULINGS_REGISTRY.md R-181, R-194, R-195 and R-196 — Sam consolidated Day edits, separated athlete status from scheduling, signed the status wording, then moved plan options onto the programmed session card with an invisible 44-point tap target; Strength ↔ Team Training swaps remain forbidden.',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + test:move-scoping',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED 2026-08-24 by seat `onboardingtype`; UPDATED FOR R-194 the same day and R-195/R-196 on 2026-08-25. R-196 TWO OPTIONS COMPARED: add a second menu architecture to the session card, or relocate the existing canonical PlanChangeSheet ingress and delete the status-card doorway. The relocation landed because it changes ownership without duplicating policy or writes. TEST FIRST: `test:session-change-hub` was 59/4 against the Edit day implementation, then 63/63 after the dots, status-only owner, 44-point target and no-session Add fallback landed. LIVENESS: mutating hitSlop 10 to 9 made the exact target cell red at 62/1; restoring returned green. `test:day-first-timeline` remains at its exact inherited 54/2 baseline and `test:signed-copy-extraction` is 7/7. `test:compile` remains at the concurrent 483-error / 60-worsened-pair baseline and names none of this slice\'s changed product files. LIVE SIMULATOR, NO SCREENSHOTS: home-plan-options was visible, home-edit-day absent, the tap exposed Add/Move/Remove, and Wednesday Rest retained Add a session which entered the Add type picker. `test:move-scoping` continues to own Strength -> Team Training combine behaviour at its inherited 17/1 baseline. NOT COVERED: physical-iPhone Release, VoiceOver reading/order, every capability combination, committing a real plan edit and exact pixels on all device widths.',
    },
  },
];
