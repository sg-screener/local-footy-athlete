...
export const LAW_REGISTRY: readonly LawRow[] = [
  ...
  },
  {
    ...
    law: 'The week list uses her one card shape and visual hierarchy for all seven days; it starts fully collapsed while today is independently highlighted, its chevron opens the whole flat session with no nested accordions, and a real active-modifier list puts its compact line above the week.',
    ruledAt: 'docs/UI_PROTOTYPE_DIVERGENCE_2026-08-11.md "RESOLVED AND BUILT — 2026-08-11" + "SAM EYE PASS 2" + "SAM EYE PASS 3" + "SAM EYE PASS 4"',
    guard: {
      state: 'guarded',
      by: 'test:day-first-timeline + .maestro/golden/standard-program-week.yaml',
      chainStatus: 'in_chain',
      receipt: 'BORN GUARDED (LAW ZERO). FOUNDING CASE: slice 5 was reported as taking her weekly structure, but the selected/today coordinate still switched into the old hero card and carried Start Session plus the change door. EYE PASSES 2–4 CAUGHT THE GUARD WAS TOO SHALLOW: 18pt dates, badges with a hidden 24pt line box, an invented GAME badge, a zero-modifier visual world, nested component accordions and today starting open all still differed materially from the prototype. WHAT THE CELLS HOLD NOW: all seven rows call one WeekDayCardHeader selected by SCREEN SHAPE; date and compact badge proportions are pinned; Game Day adds no second badge; entering Week clears the transient selected expansion while Card highlight independently reads day.isToday; the one DayTimeline call chooses interactive Today versus flat Week; the flat branch renders every section and numbered row with no inner interaction; the shared modifier component selects the compact lime week treatment. WHAT THE TAPE HOLDS: it creates one equipment modifier through the athlete door, enters Week, proves no timeline is visible before any day tap, compares collapsed card heads, opens Wednesday once into flat rows, then reaches the remaining days. FIRST-RUN FINDINGS WERE NOT FIXED QUIETLY: off-screen assertion order, TODAY wrapping, XCUITest losing its UI, inherited badge line-height, an acted Monday left open, a guessed accessory id where the projection carried strength, and the selected/open collision were each reported and the full tape rerun after correction. WHAT IT DOES NOT HOLD: Week → open → Today → Week again on a device, a multi-section flat session on the final coordinate, the two-modifier plural on a device, prior/next-week Completed treatment, the deferred team-training badge, Sam\'s physical iPhone.',
    },
  },
  {
