...
export const FEATURE_REGISTRY: readonly FeatureRow[] = [
  ...
  },
  {
    ...
    proof: {
      ...
      receipt: '`test:coach-snapshot` proves the pure Snapshot, the one-adapter ownership, one screen construction, five dashboard readers and the conversation sharing `snapshot.visibleWeek`. It also proves signed sweet-spot ownership and the load-first hierarchy, including a mutation that swaps Load with Consistency. BASE GLASS IS PAID: `.maestro/golden/coach-snapshot-dashboard.yaml` passed on iPhone 17 Pro / iOS 26.3 and `artifacts/ui-walk/coach-snapshot-dashboard.png` shows Load as the hero with its shaded signed zone, no dishonest empty-state marker, and Consistency in the four-tile grid. The non-empty and live-update states named above remain unproven, so this row stays BUILT rather than WORKING.',
    },
  },
  {
