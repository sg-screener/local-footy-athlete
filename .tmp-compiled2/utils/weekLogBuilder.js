...
exports.buildWeekLog = buildWeekLog;
exports.conditioningToWeekLogEntry = conditioningToWeekLogEntry;
 ...
 */
function buildWeekLog(resolvedDays, markedDays, readiness = 'medium', conditioningPlaced = []) {
    ...
    for (const [date, type] of Object.entries(markedDays)) {
        if (type === 'game' && weekDates.has(date))
            gamesThisWeek++;
    }
...
function intensityToFatigue(intensity) {
    switch (intensity) {
