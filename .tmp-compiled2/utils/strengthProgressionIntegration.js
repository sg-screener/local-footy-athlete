...
exports.DEFAULT_PROGRESSION_CONTEXT = void 0;
exports.feedbackFeelingToSessionFeeling = feedbackFeelingToSessionFeeling;
exports.classifyExerciseRole = classifyExerciseRole;
exports.isLowerBodyExercise = isLowerBodyExercise;
exports.roundToIncrement = roundToIncrement;
exports.applyStrengthProgression = applyStrengthProgression;
exports.buildProgressionContext = buildProgressionContext;
const exerciseTags_1 = require("../data/exerciseTags");
const progressionRules_1 = require("./progressionRules");
 ...
 */
const FEEDBACK_TO_SESSION_FEELING = {
    very_easy: 'Strong',
    easy: 'Good',
    good: 'Good',
    hard: 'Sore',
...
const EXCLUDED_MOVEMENTS = new Set([
    'isolation_upper', 'core', 'conditioning', 'carry', 'plyo',
]);
/**
 ...
 */
function roundToIncrement(weight, incrementKg) {
    if (incrementKg <= 0)
        return weight; // safety: avoid division by zero
    return Math.round(weight / incrementKg) * incrementKg;
}
/**
 * Map a ProgressionOutput to concrete prescription deltas.
 */
function outputToPrescriptionDelta(output) {
    const delta = {
        setsChange: 0,
        repsMinChange: 0,
        repsMaxChange: 0,
 ...
 */
function applyDelta(exercise, delta, loadIncrementKg = DEFAULT_LOAD_INCREMENT_KG) {
 ...
 */
function applyStrengthProgression(workout, ctx, lastPerformedWeights) {
    // Only process strength sessions
    if (workout.workoutType !== 'Strength')
        return workout;
    const rpe = (0, progressionHelpers_1.feelingToRPE)(ctx.sessionFeeling);
    const results = {};
    ...
    const newExercises = workout.exercises.map(ex => {
        ...
        if (!role)
            return ex;
        // Build progression input for this exercise
        const exposures = (0, progressionHelpers_1.extractExposureHistory)(ctx.workoutHistory, ex.exerciseId, 3);
        const trend = (0, progressionHelpers_1.deriveTrend)(exposures);
        // Derive completion quality from most recent workout containing this exercise
        let completionQuality = 'full';
        ...
        for (const w of ctx.workoutHistory) {
            ...
            if (matchingSets.length > 0) {
                completionQuality = (0, progressionHelpers_1.deriveCompletionQuality)(matchingSets, ex.prescribedSets, ex.prescribedWeightKg);
                ...
                if (completionQuality === 'full') {
                    ...
                    for (let i = 1; i < ctx.workoutHistory.length; i++) {
                        const olderSets = ctx.workoutHistory[i].sets.filter(s => s.workoutExerciseId === ex.exerciseId);
                        if (olderSets.length === 0)
                            break;
                        const olderQ = (0, progressionHelpers_1.deriveCompletionQuality)(olderSets, ex.prescribedSets, ex.prescribedWeightKg);
                        if (olderQ === 'full') {
        ...
        const output = (0, progressionRules_1.resolveProgression)(input);
        results[name] = output;
        ...
        if (ctx.adaptationBlockProgression) {
            if (delta.weightMultiplier > 1.0)
                delta.weightMultiplier = 1.0;
            if (delta.repsMinChange > 0)
                delta.repsMinChange = 0;
            if (delta.repsMaxChange > 0)
                delta.repsMaxChange = 0;
        }
        ...
        if (lastPerformedWeights && ex.exerciseId in lastPerformedWeights) {
            const performedWeight = lastPerformedWeights[ex.exerciseId];
            baseEx = {
                ...ex,
 ...
 */
function buildProgressionContext(seasonPhase, readiness, gameDates, dateStr, injuries, markedDays, workoutHistory = [], feedbackFeeling, recentFeedback = [], adaptation) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (markedDays[checkStr] === 'game')
            gamesThisWeek++;
    }
    const doubleGameWeek = gamesThisWeek >= 2;
    // Injury avoid flag
    const injuryAvoidFlag = injuries.some(i => i.severity && i.severity.toLowerCase() !== 'mild');
    // Determine session feeling: adaptation override > explicit feedback > default
    let sessionFeeling = 'Good';
    ...
    if (adaptation) {
        adjustedReadiness = (0, feedbackAdapter_1.applyReadinessBias)(adjustedReadiness, adaptation);
    }
    const baseCtx = {
        seasonPhase,
    ...
    const patternSummary = (0, feedbackPatterns_1.analyzeFeedbackPatterns)(recentFeedback);
    return (0, feedbackPatterns_1.applyPatternBiases)(baseCtx, patternSummary);
}
