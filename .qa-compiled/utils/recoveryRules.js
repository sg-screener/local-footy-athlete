...
exports.getMaxRecoveryPerWeek = getMaxRecoveryPerWeek;
// ─── Frequency Caps ───
function getMaxRecoveryPerWeek(phase) {
    switch (phase) {
 ...
 */
function resolveRecovery(daysToGame, daysSinceGame, seasonPhase, readiness, weekRecoveryCount, recentHighTier) {
    // ── Frequency guard ──
    const maxRecovery = getMaxRecoveryPerWeek(seasonPhase);
    if (weekRecoveryCount >= maxRecovery)
        return null;
