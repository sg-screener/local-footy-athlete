...
exports.RUN_LOAD_SHIFT_NOTE = exports.MODALITY_FLEX_NOTE = exports.SPEED_SPRINT_TEMPLATES = exports.CONDITIONING_DURATION_CAP = exports.TEMPLATE_CATEGORY = exports.CATEGORY_INTENSITY = exports.CONDITIONING_CATEGORIES = exports.WEEKLY_CATEGORY_CAPS = exports.DEFAULT_ATHLETE_CONTEXT = void 0;
exports.inferEquipment = inferEquipment;
exports.buildDerivedSession = buildDerivedSession;
exports.buildTagAwareSession = buildTagAwareSession;
exports.substituteExercise = substituteExercise;
exports.conditioningDateHash = conditioningDateHash;
exports.condEx = condEx;
exports.conditioningFlavourToExerciseName = conditioningFlavourToExerciseName;
exports.getTemplateCategory = getTemplateCategory;
exports.conditioningCategoryToExerciseName = conditioningCategoryToExerciseName;
exports.flavourToCategory = flavourToCategory;
exports.categoryToFlavour = categoryToFlavour;
exports.buildConditioningTemplate = buildConditioningTemplate;
exports.buildCombinedConditioningTemplate = buildCombinedConditioningTemplate;
exports.isModalityFlexibleConditioning = isModalityFlexibleConditioning;
exports.isRunningBasedConditioning = isRunningBasedConditioning;
exports.tagAsShiftedFromRun = tagAsShiftedFromRun;
exports.convertToOffFeetWithNote = convertToOffFeetWithNote;
exports.switchToOffFeetModality = switchToOffFeetModality;
exports.buildAerobicFlushFinisher = buildAerobicFlushFinisher;
exports.buildConditioningSession = buildConditioningSession;
exports.conditioningWorkoutType = conditioningWorkoutType;
const exercisePools_1 = require("../data/exercisePools");
const exerciseTags_1 = require("../data/exerciseTags");
const exerciseFilter_1 = require("./exerciseFilter");
const exerciseScorer_1 = require("./exerciseScorer");
const loadEstimation_1 = require("./loadEstimation");
const masCopy_1 = require("./masCopy");
/** Default context when no profile data is available. */
exports.DEFAULT_ATHLETE_CONTEXT = {
    injuries: [],
...
function injuriesToTags(injuries) {
    const tags = new Set();
...
function inferEquipment(trainingLocation) {
 ...
 */
function filterPool(pool, injuryTags, equipmentTags) {
    return pool.filter(ex => {
        // Exclude if any contraindication matches an active injury
        if (ex.contraindications.some(c => injuryTags.has(c)))
            return false;
...
function poolExerciseToWorkoutExercise(pe, workoutId, order) {
 ...
 */
function buildDerivedSession(type, dateStr, microcycleId, reason, athlete, weekCategoryUsage) {
    ...
    for (const slot of slots) {
        ...
        const cap = exports.WEEKLY_CATEGORY_CAPS[slot.category];
        ...
        const pool = exercisePools_1.POOL_REGISTRY[slot.category] || [];
    ...
    const finalExercises = athlete.onboardingData
        ? (0, loadEstimation_1.applyLoadEstimates)(exercises, athlete.onboardingData)
        : exercises;
    const now = new Date().toISOString();
 ...
 */
function buildTagAwareSession(templateWorkout, dateStr, gameDates, athlete, inSeason, weekExercises = new Set()) {
    // Build filter context from date + game data + injuries
    const injuries = athlete.injuries.map(i => ({
        bodyArea: i.bodyArea,
        severity: i.severity,
    }));
    const filterCtx = (0, exerciseFilter_1.buildFilterContext)(dateStr, gameDates, injuries, inSeason);
    // Get all tagged exercises and apply hard + injury filters
    const allTagged = (0, exerciseTags_1.getAllTaggedExercises)();
    const candidates = (0, exerciseFilter_1.applyHardFilters)(allTagged, filterCtx);
    // Build intent from the template workout's name/type
    const exerciseCount = templateWorkout.exercises.length || 5;
    const intent = (0, exerciseScorer_1.buildIntent)(templateWorkout.name, templateWorkout.workoutType, exerciseCount);
    // Score and select
    const selected = (0, exerciseScorer_1.selectExercises)(candidates, intent, filterCtx, weekExercises);
    // Build WorkoutExercise array from selected exercises
    const workoutId = `tag-${templateWorkout.id}-${dateStr}`;
    const now = new Date().toISOString();
    const exercises = selected.map((name, idx) => {
 ...
 */
function substituteExercise(workout, exerciseName, dateStr, gameDates, athlete, inSeason) {
    const injuries = athlete.injuries.map(i => ({
        bodyArea: i.bodyArea,
        severity: i.severity,
    }));
    ...
    const currentNames = new Set(workout.exercises.map(e => e.exercise?.name).filter(Boolean));
    const availableCandidates = candidates.filter(c => !currentNames.has(c));
    const substitute = (0, exerciseScorer_1.findSubstitute)(exerciseName, availableCandidates, filterCtx);
    if (!substitute)
        return workout;
    const now = new Date().toISOString();
    ...
    const newExercises = workout.exercises.map(e => {
        if (e.exercise?.name !== exerciseName)
            return e;
        return {
            ...e,
...
// ═══════════════════════════════════════════════════════════════
const conditioningRules_1 = require("./conditioningRules");
const conditioningProgressionRules_1 = require("./conditioningProgressionRules");
const progressionHelpers_1 = require("./progressionHelpers");
/** Simple date hash for deterministic variety (same date = same workout). */
function conditioningDateHash(dateStr) {
    let hash = 0;
    for (let i = 0; i < dateStr.length; i++) {
        hash = ((hash << 5) - hash + dateStr.charCodeAt(i)) | 0;
    }
...
function condEx(id, name, order, sets, repsMin, repsMax, rest, notes) {
    const now = new Date().toISOString();
    return {
        id,
 ...
 */
function conditioningFlavourToExerciseName(flavour, dateStr) {
...
exports.CONDITIONING_CATEGORIES = [
...
function getTemplateCategory(exerciseName) {
    return exports.TEMPLATE_CATEGORY[exerciseName] ?? null;
}
/**
 * Pick a concrete template name for a given category using the date
 * hash for deterministic variety. This is the category-aware sibling
 * of `conditioningFlavourToExerciseName` and is preferred when the
 * planner has assigned a category directly.
 */
function conditioningCategoryToExerciseName(category, dateStr) {
    const hash = conditioningDateHash(dateStr);
    switch (category) {
        case 'aerobic_base':
 ...
 */
function flavourToCategory(flavour) {
    switch (flavour) {
 ...
 */
function categoryToFlavour(category) {
    switch (category) {
...
function buildConditioningTemplate(exerciseName, dateStr, opts) {
    ...
    if (opts?.combined) {
        const category = getTemplateCategory(exerciseName);
        if (category) {
            const scaled = buildCombinedConditioningTemplate(category, dateStr, opts.strengthRegion, opts.feel, opts.ergModality);
            if (!exports.SPEED_SPRINT_TEMPLATES.has(exerciseName) && isModalityFlexibleConditioning(exerciseName)) {
                return appendNoteToHeadline(scaled, exports.MODALITY_FLEX_NOTE);
            }
            return scaled;
        }
    ...
    if (!exports.SPEED_SPRINT_TEMPLATES.has(exerciseName) && isModalityFlexibleConditioning(exerciseName)) {
        return appendNoteToHeadline(exercises, exports.MODALITY_FLEX_NOTE);
    }
    return exercises;
}
// ════════════════════════════════════════════════════════════════════
 ...
 */
exports.CONDITIONING_DURATION_CAP = {
 ...
 */
function buildCombinedConditioningTemplate(category, dateStr, 
/**
...
ergModality) {
    ...
    const legSparingMods = ['ski', 'row', 'bike'];
    ...
    const combinedNote = 'Combined S+C day — abbreviated conditioning dose.\n' +
    ...
    switch (category) {
        case 'aerobic_base': {
            ...
            return [
                condEx(`${prefix}-aero`, `${duration}min zone 2 ${mod}`, 1, 1, 1, 1, 0, `${duration}min zone 2 steady on ${modLabel}.\n` +
        ...
        case 'sprint': {
            ...
            if (isLowerPairing) {
                ...
                if (ergFeelIdx === 1) {
                    return [
                        condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps).`),
                        condEx(`${prefix}-grindsprint`, `8 × 15s hard (${modLabel})`, 2, 8, 1, 1, 45, `8 reps: 15s hard on ${modLabel}, 45s rest between (1:3).\n` +
                ...
                if (ergFeelIdx === 2) {
                    return [
                        condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps).`),
                        condEx(`${prefix}-pyramid`, `Pyramid 10/15/20/15/10s × 2 rounds (${modLabel})`, 2, 2, 1, 1, 120, `2 rounds of 10s / 15s / 20s / 15s / 10s on ${modLabel}.\n` +
                ...
                return [
                    condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps).`),
                    condEx(`${prefix}-powersprint`, `${reps} × 10s all-out power sprint (${modLabel})`, 2, reps, 1, 1, 90, `${reps} × 10s max-wattage on ${modLabel}.\n` +
            ...
            if (sprintFeelIdx === 1) {
                ...
                return [
                    condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '8min: light jog, dynamic prep, 3 × 20m gradual accelerations (50% → 70% → 90%). Must feel sharp.'),
                    condEx(`${prefix}-ladder`, 'Acceleration ladder × 3 rounds', 2, 3, 1, 1, 120, '3 rounds of 10m / 20m / 30m accelerations.\n' +
            ...
            if (sprintFeelIdx === 2) {
                ...
                return [
                    condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '8min: light jog, dynamic prep, 3 × 20m gradual accelerations (50% → 70% → 90%). Must feel sharp.'),
                    condEx(`${prefix}-shuttle`, '6 × 20m shuttle sprints', 2, 6, 1, 1, 60, '6 reps: 20m out / 20m back, hard turn.\n' +
            ...
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '8min: light jog, dynamic prep, 3 × 20m gradual accelerations (50% → 70% → 90%). Must feel sharp.'),
                condEx(`${prefix}-sprints`, `${reps} × max-velocity sprints`, 2, reps, 1, 1, 180, `${reps} reps: 20m build → 20m max velocity.\n` +
        ...
        case 'vo2': {
            ...
            const variantIdx = feel === 'grindy' ? 0
                : feel === 'sharp' ? 1
                    : feel === 'flowing' ? 2
                        : hash % 3;
            const [reps, workSec, restSec, label, feelNote] = variantIdx === 0 ? [3, 180, 90, '3 × 3min', 'Grindy: long work, short recovery — sustained ceiling.']
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 2 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-vo2`, `${label} VO2 intervals (${modLabel})`, 2, reps, 1, 1, restSec, `${label} at ~90% HRmax on ${modLabel} (${workSec}s work).\n` +
                    `${Math.round(restSec / 60 * 10) / 10}min easy recovery between reps.\n` +
                    'Intensity: 8-9/10.\n' +
                    `${feelNote}\n` +
                    `${combinedNote}${legSpareNote}`),
            ];
        ...
        case 'glycolytic': {
            ...
            if (variantIdx === 0) {
                ...
                return [
                    condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                    condEx(`${prefix}-grind`, `4 × 2min hard (${modLabel})`, 2, 4, 1, 1, 120, `4 reps: 2min hard on ${modLabel}, 2min easy between.\n` +
            ...
            if (variantIdx === 1) {
                ...
                return [
                    condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                    condEx(`${prefix}-sharp`, `10 × 30s hard (${modLabel})`, 2, 10, 1, 1, 90, `10 reps: 30s hard on ${modLabel}, 90s easy between (1:3).\n` +
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-flow`, `40:20 intervals × 2 rounds (${modLabel})`, 2, 2, 1, 1, 120, `40s on / 20s off × 4min block on ${modLabel}.\n` +
                    '2 rounds total. Rest 2min between blocks.\n' +
                    `\n${masCopy_1.MAS_FALLBACK_NOTE}\n` +
                    `\nTarget pace: ${(0, masCopy_1.masIntensityLabel)(40)} on each 40s work rep.\n` +
        ...
        default:
            return [
                condEx(`${prefix}-aero`, `20min zone 2 ${mod}`, 1, 1, 1, 1, 0, `20min zone 2 steady on ${modLabel}.\n` +
                    'Intensity: 5-6/10.\n' +
                    `${combinedNote}${legSpareNote}`),
            ];
    }
 ...
 */
function buildSprintMicroDose(dateStr, combined) {
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    const reps = 3 + (hash % 2); // 3 or 4
    const note = combined
    ...
    return [
        condEx(`${prefix}-warmup`, 'Sprint warm-up (short)', 1, 1, 1, 1, 0, '6min: light jog, dynamic prep, 2 × 20m gradual accelerations (50% → 70% → 90%).\n' +
            'Must feel sharp — stop immediately if anything tight.'),
        condEx(`${prefix}-micro`, `${reps} × 10s flying sprint (micro-dose)`, 2, reps, 1, 1, 180, `${reps} reps: 15m gradual build → 10s flying sprint.\n` +
 ...
 */
function buildSprintReducedVolume(dateStr, combined, ergModality, strengthRegion) {
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    ...
    if (isLower || ergModality) {
        ...
        return [
            condEx(`${prefix}-warmup`, `${modLabel} warm-up (short)`, 1, 1, 1, 1, 0, `4min easy ${mod}, 2 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps).`),
            condEx(`${prefix}-reduced`, `${reps} × 10s power sprint (${modLabel}, reduced)`, 2, reps, 1, 1, 120, `${reps} × 10s all-out on ${modLabel} — reduced volume.\n` +
    ...
    const reps = 3;
    return [
        condEx(`${prefix}-warmup`, 'Sprint warm-up (short)', 1, 1, 1, 1, 0, '6min: light jog, dynamic prep, 2 × 20m gradual accelerations (50% → 70% → 90%). Must feel sharp.'),
        condEx(`${prefix}-reduced`, `${reps} × 20m flying sprint (reduced)`, 2, reps, 1, 1, 180, `${reps} reps: 20m build → 20m max velocity.\n` +
...
function appendNoteToHeadline(exercises, note) {
 ...
 */
function buildConditioningTemplateRaw(exerciseName, dateStr, feel, ergModality) {
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    ...
    switch (exerciseName) {
        ...
        case '1km Repeat Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Run warm-up', 1, 1, 1, 1, 0, '10min easy jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, butt kicks. Finish with 2 × 200m strides.'),
                condEx(`${prefix}-intervals`, `${reps} × 1km repeats`, 2, reps, 1, 1, 0, `${reps} × 1km — start every ${startEvery} min.\n` +
                    'Intensity: 8-9/10 — VO2 effort.\n' +
                    'Consistent pacing — not all-out sprinting.\n' +
                    'Walk/jog in remaining time before next rep.'),
                condEx(`${prefix}-cooldown`, 'Cool-down jog', 3, 1, 1, 1, 0, '5min easy jog, stretch'),
            ];
        }
        case '4x4 VO2': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Conditioning warm-up', 1, 1, 1, 1, 0, '10min easy bike/row, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)'),
                condEx(`${prefix}-vo2`, `${reps} × 4min VO2 intervals`, 2, reps, 1, 1, rest * 60, `${reps} × 4min at high aerobic intensity (~90% HRmax).\n` +
                    `Intensity: 8-9/10.\n` +
                    `${rest}min easy recovery between intervals.\n` +
                    'Controlled hard effort — not a sprint. Bike or row preferred.'),
                condEx(`${prefix}-cooldown`, 'Easy cool-down', 3, 1, 1, 1, 0, '5min easy spin/row, stretch'),
            ];
        }
        // ════════════════════════════════════════════════
        ...
        case 'MAS 15:15 Blocks': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Interval warm-up', 1, 1, 1, 1, 0, '10min jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, hip openers. Finish with 3 × 80m strides at 80%.'),
                condEx(`${prefix}-intervals`, `15:15 intervals × ${rounds} rounds`, 2, rounds, 1, 1, 120, `15s on / 15s off × 4min block.\n` +
                    `Rest 2min between blocks.\n` +
                    `${rounds} rounds total.\n` +
                    `\n${masCopy_1.MAS_FALLBACK_NOTE}\n` +
                    `\nTarget pace: ${(0, masCopy_1.masIntensityLabel)(15)} on each 15s work rep.\n` +
        ...
        case 'Tabata Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Interval warm-up', 1, 1, 1, 1, 0, '5min easy bike/row, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)'),
                condEx(`${prefix}-tabata`, `Tabata 20:10 × ${rounds} rounds`, 2, rounds, 1, 1, 120, `20s on / 10s off × 4min block.\n` +
                    `Rest 2min between blocks.\n` +
                    `${rounds} rounds total.\n` +
                    'Intensity: 8-9/10 — glycolytic.\n' +
                    'Max effort on working intervals. Bike/row/ski.'),
                condEx(`${prefix}-cooldown`, 'Easy cool-down', 3, 1, 1, 1, 0, '3min easy spin/row, stretch'),
            ];
        }
        case 'Inverse Tabata': {
            ...
            const rounds = 4 + (hash % 3); // 4–6 rounds
            return [
                condEx(`${prefix}-warmup`, 'Interval warm-up', 1, 1, 1, 1, 0, '5min easy bike, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)'),
                condEx(`${prefix}-inv-tabata`, `Inverse Tabata 10:20 × ${rounds} rounds`, 2, rounds, 1, 1, 120, `10s on / 20s off × 4min block.\n` +
                    `Rest 2min between blocks.\n` +
                    `${rounds} rounds total.\n` +
                    'Intensity: 8-9/10 — quality > volume. Air bike preferred.'),
                condEx(`${prefix}-cooldown`, 'Easy cool-down', 3, 1, 1, 1, 0, '3min easy spin, stretch'),
            ];
        }
        // ════════════════════════════════════════════════
        // ⚡ 3. SPRINT / ALACTIC
        ...
        case 'Max Effort Sprint Accumulation': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '5min easy bike, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)'),
                condEx(`${prefix}-sprints`, `Sprint accumulation (${fmt.reps} × ${fmt.dur})`, 2, fmt.reps, 1, 1, fmt.rest, `Accumulate 60s total max sprinting.\n${fmt.note}\n` +
                    'Intensity: 9-10/10 — quality-based, full recovery.'),
                condEx(`${prefix}-cooldown`, 'Easy spin', 3, 1, 1, 1, 0, '3min easy spin, stretch'),
            ];
        }
        case 'Free Sprint Session': {
            ...
            const fmt = formats[hash % formats.length];
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '12min jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, butt kicks, straight-leg bounds, ankling. Finish with 4 × 20m gradual accelerations (50% → 70% → 80% → 90%).'),
                condEx(`${prefix}-sprints`, `Free sprints (${fmt.dist})`, 2, 1, 1, 1, 0, `20min total session.\n${fmt.note}\n` +
                    'Intensity: 9-10/10 — quality-based.\n' +
                    'Quality > volume. Stop if any rep feels sluggish.'),
                condEx(`${prefix}-cooldown`, 'Cool-down jog + stretch', 3, 1, 1, 1, 0, '5min easy jog, hamstring/hip flexor/calf stretch'),
            ];
        }
        case 'Flying Sprints': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '15min easy jog, then dynamic prep: leg swings, walking lunges, hip openers, high knees, butt kicks, A-skips, B-skips, straight-leg bounds, pogo hops.\n' +
                    '4 × 20m gradual accelerations (50% → 70% → 80% → 90%).\n' +
                    'Must feel sharp before first rep.'),
                condEx(`${prefix}-flying`, `Flying sprints (${reps} × ${buildup}m build + ${maxV}m max)`, 2, reps, 1, 1, rest * 60, `${reps} reps:\n` +
        ...
        case '200m/400m Repeat Runs': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Run warm-up', 1, 1, 1, 1, 0, '8min easy jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, butt kicks. Finish with 2 × 100m strides.'),
                condEx(`${prefix}-repeats`, `${dist} repeats (${duration}min)`, 2, 1, 1, 1, 0, `${dist} every ${startEvery}min for ${duration}min.\n` +
        ...
        case 'Footy Fartlek': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Dynamic warm-up', 1, 1, 1, 1, 0, '10min: jog, high knees, butt kicks, A-skips, lateral shuffles, 3 × 20m gradual accelerations (50% → 70% → 90%)'),
                condEx(`${prefix}-fartlek`, `Footy fartlek (${rounds} rounds × ${reps} reps)`, 2, rounds, reps, reps, 150, `Goal-to-goal sprint (200m)\n` +
                ...
                condEx(`${prefix}-cooldown`, 'Cool-down jog + stretch', 3, 1, 1, 1, 0, '5min easy jog, full body stretch'),
            ];
        }
        // ════════════════════════════════════════════════
        ...
        case 'Long Nasal Run': {
            ...
            if (ergModality === 'bike') {
                ...
                const pair = modVariant === 0 || modVariant === 1
            ...
            const structVariant = feel === 'flowing' ? 0
                : feel === 'sharp' ? 1
                    : feel === 'grindy' ? 2
                        : hash % 3;
            const structBlock = structVariant === 0
            ...
            return [
                condEx(`${prefix}-run`, `${duration}min zone 2 ${modLabel}`, 1, 1, 1, 1, 0, `${structBlock}\n` +
                    `${modNote}\n` +
                    'Intensity: 5-6/10 — conversational pace.\n' +
        ...
        // ════════════════════════════════════════════════
        case 'Flush Run':
            return [condEx(`${prefix}-flush`, 'Flush run', 1, 1, 1, 1, 0, '20min easy run at conversational pace (Zone 1–2). Purpose: flush metabolites, promote blood flow, aid recovery.')];
        case 'Easy Bike':
            return [condEx(`${prefix}-bike`, 'Easy bike', 1, 1, 1, 1, 0, '20min easy spin (Zone 1–2). Low resistance, high cadence (80–90rpm). Recovery only.')];
        case 'Easy Row':
            return [condEx(`${prefix}-row`, 'Easy row', 1, 1, 1, 1, 0, '15–20min easy row (Zone 1–2). Stroke rate 18–22. Smooth technique, not power.')];
        case 'Easy Ski':
            return [condEx(`${prefix}-ski`, 'Easy SkiErg', 1, 1, 1, 1, 0, '15–20min easy SkiErg (Zone 1–2). Smooth, relaxed rhythm. Recovery focus.')];
        case 'Easy Swim':
            return [condEx(`${prefix}-swim`, 'Easy swim', 1, 1, 1, 1, 0, '20–30min easy swim. Mix of freestyle and backstroke. Recovery pace.')];
        case 'Light Circuits':
            return [condEx(`${prefix}-circuits`, 'Light recovery circuit', 1, 2, 1, 1, 60, '2 rounds — easy pace, no rushing:\n• 2min easy bike\n• 10 bodyweight squats\n• 10 push-ups\n• 30s plank\n• 2min easy row\n60s rest between rounds. Purpose: blood flow, not fitness.')];
        default:
            return [
 ...
 */
exports.SPEED_SPRINT_TEMPLATES = new Set([
    'Flying Sprints',
    'Free Sprint Session',
    'Max Effort Sprint Accumulation',
]);
 ...
 */
function isModalityFlexibleConditioning(exerciseName) {
    if (exports.SPEED_SPRINT_TEMPLATES.has(exerciseName))
        return false;
    const meta = exerciseTags_1.CONDITIONING_META[exerciseName];
    if (meta) {
 ...
 */
function isRunningBasedConditioning(exerciseName) {
    const meta = exerciseTags_1.CONDITIONING_META[exerciseName];
    if (meta)
        return meta.modality === 'run';
 ...
 */
exports.MODALITY_FLEX_NOTE = 'Can also be completed on bike, rower, SkiErg, or Assault Bike. ' +
...
exports.RUN_LOAD_SHIFT_NOTE = 'Shifted to non-running modality to manage weekly run load.';
/**
 ...
 */
function tagAsShiftedFromRun(exercises) {
    if (exercises.length === 0)
        return exercises;
 ...
 */
function convertToOffFeetWithNote(exerciseName, dateStr) {
    if (exports.SPEED_SPRINT_TEMPLATES.has(exerciseName))
        return null;
 ...
 */
function switchToOffFeetModality(exerciseName, dateStr) {
    if (!isRunningBasedConditioning(exerciseName))
        return null;
    // Flying Sprints are top-end speed exposure — NEVER convert off-feet.
    if (exerciseName === 'Flying Sprints')
        return null;
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    const modalities = ['bike', 'row', 'ski'];
    ...
    switch (exerciseName) {
        ...
        case 'MAS 15:15 Blocks': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-intervals`, `15:15 intervals × ${rounds} rounds (${modLabel})`, 2, rounds, 1, 1, 120, `15s on / 15s off × 4min block on ${modLabel}.\n` +
                    `Rest 2min between blocks.\n${rounds} rounds total.\n` +
                    `\n${masCopy_1.MAS_FALLBACK_NOTE}\n` +
                    `\nTarget pace: ${(0, masCopy_1.masIntensityLabel)(15)} equivalent on each 15s work rep.\n` +
                    'Intensity: 9/10 — glycolytic repeat effort.\n' +
                    'Must stay sharp — stop if form collapses.'),
                condEx(`${prefix}-cooldown`, `Easy ${mod}`, 3, 1, 1, 1, 0, `5min easy ${mod}, stretch`),
            ];
        }
        case 'Tabata Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-tabata`, `Tabata 20:10 × ${rounds} rounds (${modLabel})`, 2, rounds, 1, 1, 120, `20s on / 10s off × 4min block on ${modLabel}.\n` +
                    `Rest 2min between blocks.\n${rounds} rounds total.\n` +
                    'Intensity: 8-9/10 — glycolytic.\n' +
                    `Max effort on working intervals.`),
                condEx(`${prefix}-cooldown`, `Easy ${mod}`, 3, 1, 1, 1, 0, `3min easy ${mod}, stretch`),
            ];
        }
        case 'Inverse Tabata': {
            const rounds = 4 + (hash % 3);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-inv-tabata`, `Inverse Tabata 10:20 × ${rounds} rounds (${modLabel})`, 2, rounds, 1, 1, 120, `10s on / 20s off × 4min block on ${modLabel}.\n` +
                    `Rest 2min between blocks.\n${rounds} rounds total.\n` +
        ...
        case 'Max Effort Sprint Accumulation': {
            ...
            const fmt = formats[hash % formats.length];
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-sprints`, `Sprint accumulation (${fmt.reps} × ${fmt.dur}, ${modLabel})`, 2, fmt.reps, 1, 1, fmt.rest, `Accumulate 60s total max effort on ${modLabel}.\n${fmt.note}\n` +
                    'Intensity: 9-10/10 — quality-based, full recovery.'),
                condEx(`${prefix}-cooldown`, `Easy ${mod}`, 3, 1, 1, 1, 0, `3min easy ${mod}, stretch`),
            ];
        }
        case 'Free Sprint Session': {
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-sprints`, `Free sprints on ${modLabel}`, 2, 1, 1, 1, 0, `20min session: 6–8 × 10–15s max effort on ${modLabel}.\n` +
                    'Intensity: 9-10/10 — quality-based.\n' +
        ...
        case 'Flying Sprints': {
            ...
            const rest = 2 + (hash % 3);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-flying`, `${modLabel} max velocity sprints (${reps} × 10–15s)`, 2, reps, 1, 1, rest * 60, `${reps} reps: 5s progressive build → 10–15s max effort on ${modLabel}.\n` +
        ...
        case '1km Repeat Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, then dynamic prep: leg swings, walking lunges, hip openers, high knees, ankling. Finish with 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps).`),
                condEx(`${prefix}-intervals`, `${reps} × ${workTime} intervals (${modLabel})`, 2, reps, 1, 1, 0, `${reps} × ${workTime} at 1km-repeat intensity on ${modLabel}.\n` +
                    'Intensity: 8-9/10 — VO2 effort.\n' +
        ...
        case '4x4 VO2': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-vo2`, `${reps} × 4min VO2 intervals (${modLabel})`, 2, reps, 1, 1, rest * 60, `${reps} × 4min at ~90% HRmax on ${modLabel}.\n` +
                    `Intensity: 8-9/10.\n` +
                    `${rest}min easy recovery between intervals.\n` +
        ...
        case '200m/400m Repeat Runs': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `8min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-repeats`, `${workTime} repeats on ${modLabel} (${duration}min)`, 2, 1, 1, 1, 0, `${workTime} hard / easy recovery on ${modLabel} for ${duration}min.\n` +
                    'Intensity: 8-9/10 — glycolytic repeat effort.\n' +
        ...
        case 'Footy Fartlek': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 3 × 5-10s gradual accelerations (easy → moderate → fast, easy between reps)`),
                condEx(`${prefix}-fartlek`, `Fartlek intervals on ${modLabel} (${rounds} rounds × ${reps} reps)`, 2, rounds, reps, reps, 150, `Hard 30s / easy 30s × ${reps} reps per round on ${modLabel}.\n` +
        ...
        case 'Long Nasal Run': {
            ...
            return [
                condEx(`${prefix}-steady`, `${duration}min zone 2 ${mod}`, 1, 1, 1, 1, 0, `${duration}min zone 2 steady state on ${modLabel}.\n` +
                    'Intensity: 5-6/10 — conversational pace.\n' +
        ...
        default:
            ...
            return [
                condEx(`${prefix}-session`, `${exerciseName} (${modLabel})`, 1, 1, 1, 1, 0, `${exerciseName} adapted to ${modLabel}. See coach notes.`),
            ];
    }
}
// ════════════════════════════════════════════════════════════════════
 ...
 */
function buildAerobicFlushFinisher(conditioningName, dateStr) {
    // Guard: skip for high-fatigue conditioning
    if (HIGH_FATIGUE_CONDITIONING.has(conditioningName))
        return [];
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    ...
    const modLabel = mod === 'bike' ? 'Assault Bike' : mod === 'row' ? 'Rower' : 'SkiErg';
    return [
        condEx(`${prefix}-flush-finisher`, `Aerobic flush (${duration}min ${modLabel})`, 99, 1, 1, 1, 0, `${duration}min aerobic flush on ${modLabel}:\n` +
 ...
 * @param microcycleId    - ID to stamp on the workout
 */
function buildConditioningSession(dateStr, gameDates, athlete, seasonPhase, weekLog, microcycleId) {
    ...
    const result = (0, conditioningRules_1.resolveConditioning)(ctx, weekLog);
    if (!result)
        return null;
    ...
    const progressionInput = {
        ...
        weeklyLoad: (0, progressionHelpers_1.calculateConditioningLoad)(weekLog.sessions),
    ...
    const progression = (0, conditioningProgressionRules_1.resolveConditioningProgression)(progressionInput);
    // Apply progression adjustments
    const adjustedDuration = Math.max(15, baseDuration + (progression.adjustment.durationDelta || 0));
    return {
        id: workoutId,
        microcycleId,
        dayOfWeek,
...
function conditioningWorkoutType(name) {
    const map = {
...
function conditioningDuration(tier) {
    switch (tier) {
        case 'A': return 30;
...
function conditioningIntensity(tier) {
    switch (tier) {
