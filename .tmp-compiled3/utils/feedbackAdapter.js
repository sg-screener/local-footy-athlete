...
exports.findMatchingFeedback = findMatchingFeedback;
exports.deriveAdaptation = deriveAdaptation;
exports.applyReadinessBias = applyReadinessBias;
 ...
 */
function normalizeWorkoutCategory(workoutType) {
    const t = (workoutType || '').toLowerCase();
    if (t.includes('lower'))
        return 'lower';
    if (t.includes('upper') && t.includes('push'))
        return 'upper_push';
    if (t.includes('upper') && t.includes('pull'))
        return 'upper_pull';
    if (t.includes('upper'))
        return 'upper';
    if (t.includes('full'))
 ...
 */
function findMatchingFeedback(targetType, allFeedback, allWorkouts, // date → workoutType
currentDate, lookbackDays = 14) {
...
function difficultyFromFeeling(feeling) {
    switch (feeling) {
 ...
 */
function applyReadinessBias(readiness, adaptation) {
