...
exports.detectStaleOverrides = detectStaleOverrides;
exports.isOverrideStale = isOverrideStale;
 ...
 */
function hasNearbyGame(date, gameDates, range = 2) {
    for (let offset = -range; offset <= range; offset++) {
        if (gameDates.has((0, sessionResolver_1.addDays)(date, offset)))
            return true;
    }
    return false;
}
 ...
 */
function classifyProximityIntent(workout) {
    const text = `${workout.name} ${workout.description}`.toLowerCase();
    // Specific relationships first
    if (text.includes('pre-game') || text.includes('g-1'))
        return 'pre-game';
    if (text.includes('post-game') || text.includes('g+1'))
        return 'post-game';
    // Generic proximity keywords
    if (text.includes('g-2') || text.includes('game day'))
        return 'generic';
    if (text.includes('arms / pump') || text.includes('arms/pump'))
        return 'pre-game';
    ...
    if (hasRecoveryKeyword && hasGameReference)
        return 'post-game';
    return null;
}
// ─── Core Detection ───
 ...
 */
function detectStaleOverrides(manualOverrides, overrideContexts, markedDays) {
    const warnings = [];
    ...
    for (const [date, workout] of Object.entries(manualOverrides)) {
        ...
        if (!context) {
            ...
            if (intent) {
                ...
                if (intent === 'pre-game') {
                    // Pre-game implies G-1: game should be the next day
                    const gameNextDay = gameDates.has((0, sessionResolver_1.addDays)(date, 1));
                    if (!gameNextDay) {
                        isStale = true;
                    ...
                    // Post-game implies G+1: game should be the previous day
                    const gamePrevDay = gameDates.has((0, sessionResolver_1.addDays)(date, -1));
                    if (!gamePrevDay) {
                        isStale = true;
 ...
 */
function isOverrideStale(date, manualOverrides, overrideContexts, markedDays) {
    if (!manualOverrides[date])
        return null;
    const warnings = detectStaleOverrides({ [date]: manualOverrides[date] }, overrideContexts, markedDays);
