...
function buildConditioningTemplate(exerciseName, dateStr, opts) {
    ...
    if (opts?.combined) {
        ...
        if (category) {
            const scaled = buildCombinedConditioningTemplate(category, dateStr, opts.strengthRegion);
    ...
    const exercises = buildConditioningTemplateRaw(exerciseName, dateStr);
...
strengthRegion) {
    ...
    const mod = modPool[hash % modPool.length];
    ...
    switch (category) {
        ...
        case 'sprint': {
            ...
            if (isLowerPairing) {
                // Ergo power-sprint session. Short all-out efforts, full recovery.
                const reps = 6 + (hash % 3); // 6 / 7 / 8
                return [
                    condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 10s build-up sprints.`),
                    condEx(`${prefix}-powersprint`, `${reps} × 10s all-out power sprint (${modLabel})`, 2, reps, 1, 1, 90, `${reps} × 10s max-wattage on ${modLabel}.\n` +
                        ...
                        'Intensity: 9–10/10 — quality-based, neural alactic.\n' +
                        `${combinedNote}${legSpareNote}`),
                ];
            }
            const reps = 3 + (hash % 2); // 3 or 4
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '8min: light jog, dynamic prep, 3 × build-ups. Must feel sharp.'),
        ...
        case 'vo2': {
            // VO2 dose — rotate interval structure for variety across weeks.
            //   hash 0 → 3 × 2min @ 90% HRmax  (classic)
            //   hash 1 → 4 × 90s               (shorter, more reps)
            //   hash 2 → 2 × 3min              (longer, fewer reps)
            const variant = hash % 3;
            const [reps, workSec, restSec, label] = variant === 0 ? [3, 120, 120, '3 × 2min']
                : variant === 1 ? [4, 90, 90, '4 × 90s']
                    : [2, 180, 180, '2 × 3min'];
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 2 × build-ups`),
        ...
        case 'glycolytic': {
            // Glycolytic dose — rotate rest scheme for variety.
            //   hash 0 → MAS 15:15 × 2 rounds
            //   hash 1 → 30:30 × 2 rounds
            //   hash 2 → 40:20 × 2 rounds  (longer work, shorter rest)
            const variant = hash % 3;
            const [label, scheme] = variant === 0 ? ['MAS 15:15', '15s on / 15s off × 4min block']
                : variant === 1 ? ['30:30 repeats', '30s on / 30s off × 4min block']
                    : ['40:20 repeats', '40s on / 20s off × 4min block'];
            const rounds = 2;
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × short build-ups`),
                condEx(`${prefix}-mas`, `${label} × ${rounds} rounds (${modLabel})`, 2, rounds, 1, 1, 120, `${scheme} on ${modLabel}.\n` +
                    `${rounds} rounds total. Rest 2min between blocks.\n` +
...
function buildConditioningTemplateRaw(exerciseName, dateStr) {
    const hash = conditioningDateHash(dateStr);
    const prefix = `cond-${dateStr}`;
    switch (exerciseName) {
        // ════════════════════════════════════════════════
        ...
        case '4x4 VO2': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Conditioning warm-up', 1, 1, 1, 1, 0, '10min easy bike/row, build-ups'),
        ...
        case 'MAS 15:15 Blocks': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'MAS warm-up', 1, 1, 1, 1, 0, '10min jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, hip openers. Finish with 3 × strides at 80% MAS.'),
                condEx(`${prefix}-mas`, `MAS 15:15 × ${rounds} rounds`, 2, rounds, 1, 1, 120, `15s on / 15s off × 4min block.\n` +
        ...
        case 'Tabata Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Interval warm-up', 1, 1, 1, 1, 0, '5min easy bike/row, 3 × 15s build-ups'),
        ...
        case 'Inverse Tabata': {
            ...
            const rounds = 4 + (hash % 3); // 4–6 rounds
            return [
                condEx(`${prefix}-warmup`, 'Interval warm-up', 1, 1, 1, 1, 0, '5min easy bike, 3 × 10s build-ups'),
        ...
        case 'Max Effort Sprint Accumulation': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '5min easy bike, 3 × progressive build-ups (50% → 70% → 90%)'),
        ...
        case 'Free Sprint Session': {
            ...
            const fmt = formats[hash % formats.length];
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '12min jog, then dynamic prep: leg swings, walking lunges, A-skips, high knees, butt kicks, straight-leg bounds, ankling. Finish with 4 × build-ups (50% → 70% → 80% → 90%).'),
        ...
        case 'Flying Sprints': {
            ...
            return [
                condEx(`${prefix}-warmup`, 'Sprint warm-up', 1, 1, 1, 1, 0, '15min easy jog, then dynamic prep: leg swings, walking lunges, hip openers, high knees, butt kicks, A-skips, B-skips, straight-leg bounds, pogo hops.\n' +
                    '4 × progressive build-ups (50% → 70% → 80% → 90%).\n' +
                ...
                condEx(`${prefix}-flying`, `Flying sprints (${reps} × ${buildup}m build + ${maxV}m max)`, 2, reps, 1, 1, rest * 60, `${reps} reps:\n` +
                    `• ${buildup}m progressive build-up to ~95%\n` +
        ...
        case 'Footy Fartlek': {
            ...
            const reps = 3 + (hash % 2); // 3–4 reps per round
            const rounds = 3; // fixed — keeps cap-compliant
            return [
                condEx(`${prefix}-warmup`, 'Dynamic warm-up', 1, 1, 1, 1, 0, '10min: jog, high knees, butt kicks, A-skips, lateral shuffles, build-ups'),
        ...
        case 'Long Nasal Run': {
            ...
            const [modLabel, modNote] = modVariant === 0 || modVariant === 1
            ...
            // Structure variant: 0 = pure steady, 1 = 2-block with surge, 2 = fartlek
            const structVariant = hash % 3;
...
function switchToOffFeetModality(exerciseName, dateStr) {
    ...
    switch (exerciseName) {
        ...
        case 'MAS 15:15 Blocks': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 3 × progressive build-ups`),
                condEx(`${prefix}-mas`, `MAS 15:15 × ${rounds} rounds (${modLabel})`, 2, rounds, 1, 1, 120, `15s on / 15s off × 4min block on ${modLabel}.\n` +
                    `Rest 2min between blocks.\n${rounds} rounds total.\n` +
                    'Intensity: 8–9/10 — glycolytic repeat effort.\n' +
                    'Must stay sharp — stop if form collapses.'),
        ...
        case 'Tabata Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 15s build-ups`),
        ...
        case 'Inverse Tabata': {
            const rounds = 4 + (hash % 3);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × 10s build-ups`),
        ...
        case 'Max Effort Sprint Accumulation': {
            ...
            const fmt = formats[hash % formats.length];
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 3 × progressive build-ups (50% → 70% → 90%)`),
        ...
        case 'Free Sprint Session': {
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `5min easy ${mod}, 4 × progressive build-ups (50% → 70% → 80% → 90%)`),
        ...
        case 'Flying Sprints': {
            ...
            const rest = 2 + (hash % 3);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} sprint warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, 4 × progressive build-ups`),
        ...
        case '1km Repeat Intervals': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, then dynamic prep: leg swings, walking lunges, hip openers, high knees, ankling. Finish with 2 × build-ups.`),
        ...
        case '4x4 VO2': {
            ...
            const rest = 2 + (hash % 2);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, build-ups`),
        ...
        case '200m/400m Repeat Runs': {
            ...
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `8min easy ${mod}, build-ups`),
        ...
        case 'Footy Fartlek': {
            ...
            const rounds = 3 + (hash % 3);
            return [
                condEx(`${prefix}-warmup`, `${modLabel} warm-up`, 1, 1, 1, 1, 0, `10min easy ${mod}, progressive build-ups`),
