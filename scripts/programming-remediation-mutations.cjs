...
const mutants = {
  ...
  injury_duplicate_addition: ['src/rules/canonicalWeeklyInjuryCompiler.ts', 'const adjustment = deriveInjurySessionAdjustment({ workout: substitutedVisible, environment,\n    profile: args.profile, bodyPart: stage.bodyPart,\n    redFlag: isRedFlagInjurySeverity(stage.seriousSymptoms, stage.severity),\n    weekExerciseNames: substitutedWeekNames,', 'const adjustment = deriveInjurySessionAdjustment({ workout: visible, environment,\n    profile: args.profile, bodyPart: stage.bodyPart,\n    redFlag: isRedFlagInjurySeverity(stage.seriousSymptoms, stage.severity),\n    weekExerciseNames: args.weekExerciseNames,', 'simple_injury', 'substitutions and additions never duplicate an exercise on one day'],
  injury_manual_medical_stop: ['src/utils/programEditRiskAssessment.ts', 'const hardStop = isRedFlagInjurySeverity(constraint.seriousSymptoms, constraint.severity);', 'const hardStop = false;', 'simple_injury', 'serious symptoms retain the medical stop for manual edits'],
  injury_lower_balance: ['src/utils/injurySessionAdjustment.ts', "safeHalf === 'lower'\n    ? leaves.flatMap(legal).sort", "false\n    ? leaves.flatMap(legal).sort", 'simple_injury', 'eligible lower replacement favours the uncovered hinge'],
  injury_balance_authority: ['src/rules/section18SafetyPolicy.ts', 'contract.strengthPatterns.intentionalImbalanceReason ??=\n      `injury_restriction:${prohibited.join(\',\')}`;', 'void 0;', 'simple_injury', 'offered unaffected swap is accepted without relaxing weekly balance'],
  injury_pull_requirement: ['src/rules/canonicalWeeklyInjuryState.ts', 'if (restrictions.length) for (const pattern', 'if (false) for (const pattern', 'simple_injury', 'offered unaffected swap is accepted without relaxing weekly balance'],
  injury_severe_blanket_stop: ['src/utils/programEditRiskAssessment.ts', 'const hardStop = isRedFlagInjurySeverity(constraint.seriousSymptoms, constraint.severity);', 'const hardStop = constraint.severity >= 8 || isRedFlagInjurySeverity(constraint.seriousSymptoms, constraint.severity);', 'simple_injury', 'offered unaffected swap is accepted without relaxing weekly balance'],
