...
const frequency = athlete => {
  ...
  return Object.fromEntries([...counts].sort().map(([name, t]) => [name,
    { prescribedRowPlacements: t.placements, distinctAthleteDays: t.dates.size }]));
};
...
const rowHTML = row => `<li><b>${esc(row.name)}</b> ${esc(row.dose)} ${esc(row.load)}${row.optional ? ' <small>Optional</small>' : ''}${row.role === 'power' ? ' <small>Power</small>' : ''}${row.modalityLabel ? `<p class="mode">${esc(row.modalityLabel)}</p>` : ''}${row.rest ? ` · ${esc(row.rest)}` : ''}${row.notes ? `<pre>${esc(row.notes)}</pre>` : ''}${row.choices ? row.choices.map(c => `<p>${esc(c.name)} — ${esc(c.modalityLabel)} ${esc(c.description)}</p><ul>${c.rows.map(rowHTML).join('')}</ul>`).join('') : ''}</li>`;
...
for (const world of ['original-partial', 'corrected-commercial']) {
  ...
  for (const athlete of data.athletes) {
    ...
    const total = { world, gender: athlete.gender, revision: data.revision, weeks: athlete.weeks.length, athleteDays: days.length,
      prescribedRowPlacements: rows.length, distinctExerciseDisplayNames: new Set(rows.map(r => r.name)).size,
...
const count = f => Object.values(f).reduce((n, v) => n + v.prescribedRowPlacements, 0);
