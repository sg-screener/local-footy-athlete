...
git_case() {
  ...
  # THE HALT lives in the repo the hook actually RUNS in, so the cell must place
  # it THERE. The first version of this cell touched `.claude/HALT` in the real
  # checkout — it halted the live terminal, could not be undone on a mount that
  # cannot delete, and proved nothing. Exactly the class this suite exists to
  # catch: a probe measuring a different world from the one it reports on.
  [ -n "${HALT_THIS_CASE:-}" ] && { mkdir -p "$repo/.claude"; echo halt > "$repo/.claude/HALT"; }
...
# it no door. These two cells are the wall AND its liveness: the first proves a
# halted repo stops being blocked, the second proves the halt is not simply
# always-on. Delete the check from the hook and the first cell reds.
HALT_THIS_CASE=1 git_case "a HALT file ALLOWS the stop even with a full queue" allow \
  "docs(seat): anything at all" "" ""
unset HALT_THIS_CASE
git_case "and with the HALT gone the same queue BLOCKS again" block \
  "docs(seat): anything at all" "" ""
