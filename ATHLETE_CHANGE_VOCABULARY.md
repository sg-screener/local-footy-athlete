...
- Conditioning → Light / Hard / Sprint (DEFERRED — on-legs running
  ...
  date-hash rotation.
- Strength → Upper / Lower / Full body / Accessories. BUILT 2026-07-04:
  dynamic registry templates generated by the real engine
  (buildTagAwareSession for splits with tag scoring / game proximity /
  injury filters; buildDerivedSession for Gunshow + Prehab). Athlete
  picks the bucket; producer picks the split (Upper avoids whichever of
  push/pull the week already has). Context arrives via ONE seam
  (coachRevisionTemplateContext); policy computes per-date signatures
  for dynamic templates. KNOWN LIMITS: one-off prescriptions still use
  the tag-builder default rx (polish later); strength does not stack
  onto occupied days yet (single strength section per projected day).
...
BUILT 2026-07-04: tired → readiness signal for today (existing resolver
constraint machinery, self-healing tomorrow); sniffle → today softens to
Recovery Flow; bed-ridden → confirm → shutdown_week (atomic, games
untouched, same pipeline); pretty-rough + injured → coach pre-loaded.
